...
| GIS Sprint 4 | Controlled Fixture Adapter | Future non-production fixture adapter proof of provider-boundary behavior without external calls or live data. | `NOT_AUTHORIZED` | Requires separate authorization; no live providers, provider credentials, acquisition, persistence, retrieval, runtime, downstream, or customer activation |
