...
2. `PROPERTY_PUBLIC_RECORD_FACTS`: situs or property address, property characteristics, lot size, building area, year built, property type or use, assessor-related facts, APN/FIPS/legal-description fields, valuation/tax fields, ownership-related fields where permitted, and tax/public-record attributes. Documentation-supported endpoint families include Geocoding, Addresses, Parcels, Assessment, Structures, APN Lookup, and LightBox ID Lookup; exact app entitlement remains `ACCESS_UNKNOWN` until static portal scope is confirmed.
3. `ZONING`: zoning district classifications, setbacks, FAR, maximum building height, lot area/density metrics, geography, description, effective/freshness posture where present, and parcel-to-zoning relationship. Documentation-supported endpoint families include `GET /zoning/_on/parcel/{countryCode}/{id}`, site/property variants, and address lookup; exact app entitlement remains `ACCESS_UNKNOWN` until static portal scope is confirmed.
