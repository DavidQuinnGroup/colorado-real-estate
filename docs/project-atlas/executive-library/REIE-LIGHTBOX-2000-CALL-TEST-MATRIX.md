...
The goal is not to spend all 2,000 calls. Planned test ceiling is 1,340 calls with 660 reserve calls held through the final days. Public documentation now supports concrete first-pass endpoint families and selected fields for Geocoding, Addresses, Parcels, Assessment, Structures, and Zoning. Exact app entitlement, Key + Secret handling, complete response fields, rate limits, and full call-accounting behavior remain unresolved until static portal scope/authentication confirmation and later authorized Day 1 validation.
...
| Property/public-record facts | Does it add verified non-MLS property attributes? | Compare APN, FIPS, legal description, lot size, building area, year built, property type/use, assessor valuation/tax facts, and ownership-sensitive fields where available and permitted. | High priority; documented Assessment and Parcel field families now guide first-pass inspection, with rights still unresolved. |
...
Public documentation supports the following first-pass endpoint and field mapping. Any field not listed remains `PROVIDER_DOC_REQUIRED`, and actual app entitlement remains `ACCESS_UNKNOWN` until static portal scope is confirmed:
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Parcel/APN | Geocoding `GET /addresses/search`; Parcels `GET /parcels/{countryCode}/{id}`; Parcels geometry `GET /parcels/{countryCode}/geometry`; APN Lookup if entitled | Parcel LightBox ID, APN/assessor fields where present, related Parcel reference from geocoding | Parcel identity | Missing in `Property`; public-record evidence says parcel number missing. | Net-new if reliable. | String | County/APN normalization. | Address, APN, LightBox Parcel ID, coordinates. | High. | Source/freshness/authority required; exact field-level provenance still requires response inspection. | Storage/display/indexing rights unresolved. | Potentially high only after rights. | High. | Preserve ambiguous or multi-parcel cases. |
| Parcel geometry | Parcels `GET /parcels/{countryCode}/geometry`; tile/bbox endpoints not Day 1 | `location.geometry`, WKT/polygon geometry, recordSet bbox/metadata where returned | Parcel geometry | Geographic contracts exist; no active public parcel geometry. | Net-new. | Geometry/WKT | Coordinate system and geometry simplification; docs indicate EPSG 4326 metadata in some responses. | LightBox Parcel ID, coordinates/WKT. | Medium/high. | Source lineage and date required; response metadata must be inspected. | GIS display/transformation rights unresolved. | Potentially high. | High. | Geometry conflicts preserved. |
| Situs address | Addresses `GET /addresses/{countryCode}/{id}`; Addresses `GET /addresses/_on/parcel/{countryCode}/{id}`; Geocoding `GET /addresses/search` | LightBox Address ID/UAID, Parcel ID, normalized address components, confidence/change flags where present | Official property address | Existing listing address. | Verification/overlap. | String/object | Address parsing and normalization. | Address, LightBox Address ID, Parcel ID. | High. | Source identity required; docs list parcel, assessment, structures, USPS, e911, localized authoritative sources. | Display and retention rights unresolved. | Medium. | High. | Conflict against listing address. |
| Lot size | Assessment `GET /assessments/us/geometry`; Assessment ID/Parcel ID endpoint path requires static confirmation; Parcels fields may include property/tax characteristics | `assessedLotSize`, `lot.lotSize`, parcel-derived geometry area if present | Lot-size verification | Existing listing `lotSize`. | Verification/overlap. | Numeric/unit | Unit conversion; docs show units metadata in some responses. | APN, Assessment ID, Parcel ID, address. | Medium. | Source/effective date required. | Derived display rights unresolved. | Medium. | High. | Conflict against listing value. |
| Building area | Assessment; Structures `GET /structures/{countryCode}/geometry`; Structures `GET /structures/_on/parcel/{countryCode}/{id}` | Building square footage, structure footprint area, stories, height/elevation where present | Square footage reconciliation | Existing listing `sqft`. | Verification/overlap. | Numeric/unit | Measurement-type mapping. | Assessment ID, Structure ID, Parcel ID, address. | Medium. | Source/method required. | Display caveats unresolved. | Medium. | High. | Preserve source/method discrepancy. |
| Year built | Assessment endpoint family | Exact year-built field remains `PROVIDER_DOC_REQUIRED`; inspect assessment response if present | Year-built verification | Existing listing `yearBuilt`. | Verification/overlap. | Number | None/minor. | APN/address/Assessment ID. | Medium. | Source date required. | Display rights unresolved. | Medium. | High. | Conflict review. |
| Property type/use | Parcels; Assessment | `propertyType`, `propertyClass`, `landUse.code`, `landUse.description`, `landUse.normalized.*` where present | Property type/use | Existing listing `propertyType`; legal use not established. | Potential net-new legal-use evidence, not conclusion. | Coded/string | Map provider codes to REIE labels. | APN/address/Parcel ID. | Medium. | Source/code list required. | Legal-use conclusion risk. | Low/medium. | High. | Do not infer legal compliance. |
| Ownership-related fields | Parcels and Assessment; Assessment owner portfolio is explicitly out of Day 1 | Ownership information and owner portfolio capability where present | Ownership where permitted | Not active; owner identity display blocked. | Net-new but high-risk. | String/entity | Privacy and entity normalization. | APN/address/Assessment ID/Parcel ID. | High. | Source and legal basis required. | Privacy, display, retention, redistribution unresolved; customer display blocked. | Blocked unless approved. | Potential internal value. | Fail closed. |
| Tax/public-record attributes | Parcels; Assessment | Assessed/taxable/market values, tax rates, exemptions, tax history, tax attributes, book/page, legal description where present | Tax/public-record context | Boulder treasurer/tax source blocked. | Net-new if rights permit. | Numeric/date/string | Tax-year and amount normalization. | APN/tax account/address/Assessment ID. | High. | Source/freshness required. | Tax advice and display restrictions unresolved. | Low/medium. | High. | Do not imply payoff/current tax advice. |
| Zoning district/code | Zoning `GET /zoning/_on/parcel/{countryCode}/{id}`; Zoning site/property/address variants; Parcels/Assessment may include assessor zoning | Zoning district classifications, setbacks, FAR, max heights, lot area/density, `zoning.assessment` where present | Zoning evidence | Municipal planning is reference-only. | Net-new if parcel-linked. | Coded/string/object | Jurisdiction-specific code mapping. | Parcel ID, address, site/property IDs. | High. | Source/effective date required. | Legal-use conclusion risk. | Medium only with caveats. | High. | Do not infer allowed use. |
| Jurisdiction identifiers | Assessment; Parcels; Geocoding; Addresses | FIPS, county/locality/region components, jurisdictional references where present | County/city/jurisdiction attribution | Listing city and geographic records. | Verification/augmentation. | String/code | FIPS/local ID mapping if present. | Address/APN/coordinates/Parcel ID. | Medium. | Source lineage required. | Display and derivative rights unresolved. | Medium. | High. | Boundary conflicts preserved. |
