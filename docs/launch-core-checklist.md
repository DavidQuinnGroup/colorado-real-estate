...
- Alert worker startup, alert dry-runs, digest dry-runs, CRM scheduler reporting, Typesense reindexing, and live seed writes must stop at their database preflight while `npm run supabase:check` is failing.
