...
Latest new-chat handoff, July 25, 2026 PROJECT ATLAS GIO 1.0 Wave 1 Closure and Wave 2 Charter:
...
- Current verified baseline before GIO documentation closure edits: branch `main`, `HEAD = origin/main = 632f106f412a43722afa2bb0fcc1517a21dd45b3`.
...
- Discovery result: the repository has property-centered geography, city/neighborhood/static market content, search/map/SEO/property-page geographic usage, geocode/logistics helpers, source-health patterns, EIA provenance patterns, and enterprise repository-governance patterns, but it does not yet have canonical Geographic Intelligence Objects with governed identity, source/freshness metadata, geometry provenance, typed relationship metadata, lifecycle, eligibility, conflict preservation, or trust-aware inheritance.
- Expanded discovery deliverables now include current geographic capability inventory, existing object catalog, GIO alignment matrix, relationship inventory, Real Estate Data Tools source matrix, architecture risk register, recommended minimum implementation baseline, proposed implementation waves, and explicit deferral register.
- Wave 2 charter recommends a generalized additive GIO persistence foundation with companion alias, relationship, source registry, observation/evidence, eligibility, and property-geographic relationship contracts.
- Initial canonical object scope is limited to `Municipality`, `Neighborhood`, `MarketArea`, `ZipCode`, and `Subdivision`.
- `Property` remains the production runtime anchor and must not be converted into a GIO object during initial implementation; it should connect through additive property-geographic relationships.
...
- Next recommended authorization: Wave 3 additive persistence-foundation package, documentation-reviewed first, with no runtime integration unless explicitly authorized.
- Do not begin runtime implementation, database migrations, route/API changes, ingestion changes, search/map changes, public-page changes, vendor integration, scraping, production data mutations, form submissions, CRM mutations, email sends, queue retries, MLS requests, Typesense reset/reindex, deployment, commit, or push without explicit authorization.
