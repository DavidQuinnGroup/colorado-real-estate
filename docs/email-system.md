...
- Alert and digest dry-runs require `npm run supabase:check:json` readiness; current aggregate notification readiness is `watch` with property-inquiry notification readiness `ready`, explicit sender/reply-to configured, and 197 pending saved-search alert rows requiring final live policy review before live processing.
...
- June 29, 2026 14:41 MDT `npm run supabase:check:json` reported `readiness.level="ready"` with no failed checks. URL/key shape, placeholder detection, project-ref consistency, Postgres URL shape, Supabase project DNS, Postgres DNS, Postgres TCP, Prisma `SELECT 1`, and Supabase REST all passed. The suggested Typesense reindex next command was not run because reindexing remains out of scope unless explicitly requested.
- June 29, 2026 14:43 MDT `npm run typesense:collections:check` completed the collections-only Typesense readiness check. Canonical `properties` and `listings` schemas were ready with 32 fields, 23 facets, 7 canonical sortable fields, and default sort `price`; live collections reported 32 fields, 23 facets, 16 sortable fields, and default sort `price`. No reset or reindex was run.
- June 29, 2026 14:48 MDT `npm run check:fast` passed. Worker output rebuilt; property-inquiry missing-recipient and dry-run suppression helpers passed; saved-search alert readiness stayed `watch` with 197 pending / 0 failed / 0 processing rows; consolidated notification readiness stayed `blocked` by missing property-inquiry routing; strict-contract validation passed; bounded MLS sync dry-run completed with `executed=false` and no MLS Grid request; `npm run typecheck` and `npm run lint` passed.
- June 29, 2026 14:51 MDT `npm run build` passed. Next compiled successfully, completed lint/type validation in the build pipeline, collected page data, generated 130 static pages, finalized page optimization, and collected build traces. The build left the tracked tree clean.
- June 29, 2026 14:54 MDT `npm run smoke:mls-status` and `npm run smoke:search` passed against a temporary local Next dev server. MLS status returned HTTP 200 with operational readiness `watch`, search-index health `healthy` with 750 attempted / 750 succeeded / 0 failed recent index updates, inventory freshness still degraded at 100% stale by `lastIntelligenceSync`, and media diagnostics `busy` / `watch`; public search returned HTTP 200 with Typesense source, 5 returned / 5 mapped / 0 coordinate-filtered results, and `meta.smoke.ready=true`. The temporary server was stopped after the checks.
- June 29, 2026 15:01 MDT `npm run smoke:ops` passed against a temporary local Next dev server. Public-experience smoke, admin/control/intake/CRM/dead-letter/retry/alert inspection metadata, public search, and notification readiness checks passed structurally; MLS status stayed `busy` / `watch` with healthy search-index health and busy media diagnostics; alert status stayed `caution` with 197 pending / 0 failed rows; aggregate notification readiness stayed `blocked` because property-inquiry recipient routing is missing. The temporary server was stopped after the check.
- June 29, 2026 15:22 MDT `npm run smoke:property-inquiry` was reclassified as a mutating route smoke, not a read-only notification readiness gate. The earlier smoke passed and sent no email because the fixture is low-priority research intent (`sent=false`, `reason="not-high-priority"`, `attempted=false`, `required=false`), but it creates temporary smoke property, user, CRM task, user interaction, and lead interaction rows before cleanup. A follow-up residue check found one leftover smoke `LeadInteraction`; it was deleted by exact ID and verified gone. The smoke script now explicitly cleans smoke lead interactions before parent row cleanup and prints `sendsEmail=false`, `mutatesRows=true`, `cleanupAttempted=true`, `mutationScope`, and `initialCleanup` metadata.
- June 29, 2026 15:28 MDT `npm run check:property-inquiry-notification:readiness` rebuilt worker output, sent no email, mutated no rows, and failed closed as expected with `success=false` and `readiness.level="blocked"`. The only blocker remained `property_inquiry_recipient_missing` for `PROPERTY_INQUIRY_NOTIFY_TO` and fallback `REIE_INTERNAL_EMAIL`; disabled or unset `PROPERTY_INQUIRY_NOTIFICATION_DRY_RUN`, Resend API key shape, sender, and public site URL checks passed, while missing `RESEND_REPLY_TO_EMAIL` remained a warning.
- June 29, 2026 15:31 MDT `npm run check:notification-readiness` rebuilt worker output, sent no email, mutated no rows, and returned `success=true` with overall readiness still `blocked`. Saved-search alert notification readiness stayed `watch` with sender fallback, missing reply-to, and 197 pending-row warnings; property-inquiry and aggregate launch notification readiness stayed `blocked` by `property_inquiry_recipient_missing`.
- June 29, 2026 15:35 MDT `npm run check:notification-readiness:strict` rebuilt worker output, sent no email, mutated no rows, and failed closed as expected with `success=false`, `strictMode=true`, `commandSuccess=true`, and overall readiness still `blocked`. Saved-search alert notification readiness stayed `watch`; property-inquiry and aggregate launch notification readiness stayed `blocked` by `property_inquiry_recipient_missing`.
- June 29, 2026 15:42 MDT `npm run check:notification-readiness:strict-contract` rebuilt worker output, sent no email, mutated no rows, and passed. It confirmed the current environment fails closed, the dummy-recipient plus property-inquiry dry-run environment fails closed, both direct property-inquiry and aggregate dry-run blockers are detected, and the aggregate launch-readiness reply-to warning remains aligned.
- June 29, 2026 16:05 MDT `npm run check:launch-readiness` rebuilt worker output, sent no email, mutated no rows, and failed closed as expected with `success=false`, `readiness.level="blocked"`, and one blocked launch gate. Supabase connectivity stayed `ready`; saved-search alert email stayed `watch` with 197 pending / 0 failed / 0 processing rows; property-inquiry notification email stayed `blocked` by `property_inquiry_recipient_missing`.
- June 29, 2026 16:13 MDT `npm run check:alert-notification-readiness` rebuilt worker output, sent no email, mutated no rows, returned `success=true`, and reported saved-search alert notification readiness at `watch`. Alert queue counts stayed 197 pending / 0 failed / 0 processing; sender fallback, missing reply-to, and pending-row warnings remain before any live alert processing.
- June 29, 2026 16:26 MDT `npm run run:alerts:dry -- --limit 50` completed a dry-run preview with 50 scanned, 0 sent, 0 skipped, 0 failed, and 50 masked rows marked `preview` / `Ready to send.` The execution plan stayed `caution`; review preview rows before any live execution, and do not run live alert sends or recurring email traffic until property-inquiry recipient routing is configured and strict/aggregate readiness clears.
- June 29, 2026 18:18 MDT `npm run run:queue-dashboard -- --limit=5 --timeout-ms=3000` completed inspection with no diagnostics, no failed jobs, no open dead-letter jobs, and no stale active jobs. Recovery stayed `caution` because `reie-alerts` had 273 waiting / 0 active / 0 failed jobs; MLS sync, MLS page, listings, and dead-letter queues had 0 waiting / 0 active / 0 failed. The surfaced alert-worker command remains blocked by launch policy until property-inquiry recipient routing is configured and strict/aggregate readiness clears.
- June 29, 2026 18:31 MDT `npm run supabase:check:json` passed with `readiness.level="ready"` and no failed checks. Supabase URL/key checks, project-ref consistency, DNS, Postgres TCP, Prisma `SELECT 1`, and Supabase REST all passed. The surfaced `npm run typesense:reindex` command was not run and remains blocked unless explicitly approved.
- June 29, 2026 18:35 MDT `npm run typesense:collections:check` passed in `check=true`, `reset=false`, `collectionsOnly=true` mode. Canonical `properties` and `listings` schemas validated, and existing Typesense `properties` and `listings` collections were ready; no reset or reindex was run.
- June 29, 2026 18:43 MDT `npm run check:fast` passed. Worker output rebuilt; property-inquiry missing-recipient and dry-run suppression checks passed; saved-search alert readiness stayed `watch`; consolidated notification readiness stayed `blocked`; strict notification readiness contract validation passed; bounded MLS sync dry-run completed with `executed=false` and no MLS Grid request; typecheck and lint passed.
- June 29, 2026 18:49 MDT `npm run build` passed. Next.js 15.1.6 compiled successfully, linted and checked types, collected page data, generated 130 static pages, finalized optimization, and collected build traces.
- June 29, 2026 18:54 MDT a masked local env-presence check reconfirmed `PROPERTY_INQUIRY_NOTIFY_TO` is unset, fallback `REIE_INTERNAL_EMAIL` is unset, and `PROPERTY_INQUIRY_NOTIFICATION_DRY_RUN` is unset. No recipient values were printed, no email was sent, and no rows were mutated.
- June 29, 2026 19:00 MDT `npm run check:property-inquiry-notification:readiness` rebuilt worker output, stayed non-sending and non-mutating, and failed closed as expected with `readiness.level="blocked"`. The only blocker remained `property_inquiry_recipient_missing` for `PROPERTY_INQUIRY_NOTIFY_TO` and fallback `REIE_INTERNAL_EMAIL`; `RESEND_REPLY_TO_EMAIL` remained a warning.
- June 29, 2026 19:02 MDT `npm run check:notification-readiness` rebuilt worker output, stayed non-sending and non-mutating, returned `success=true`, and summarized overall notification readiness as `blocked`. Saved-search alert readiness stayed `watch`; property-inquiry notification and aggregate launch notification readiness stayed `blocked` by `property_inquiry_recipient_missing`.
- June 29, 2026 19:19 MDT `npm run check:notification-readiness:strict` rebuilt worker output, stayed non-sending and non-mutating, returned `strictMode=true`, `commandSuccess=true`, and failed closed as expected with `success=false` because overall notification readiness remains blocked by missing property-inquiry recipient routing.
- June 29, 2026 19:22 MDT `npm run check:notification-readiness:strict-contract` rebuilt worker output, stayed non-sending and non-mutating, and passed. It confirmed the current environment fails closed, the dummy-recipient plus property-inquiry dry-run scenario fails closed with direct and aggregate dry-run blockers detected, and aggregate launch-readiness reply-to warning alignment remains true.
- June 29, 2026 19:30 MDT `npm run check:launch-readiness` rebuilt worker output, stayed non-sending and non-mutating, and failed closed as expected with `readiness.level="blocked"` and one blocked launch gate. Supabase connectivity stayed `ready`; saved-search alert email stayed `watch` with 197 pending / 0 failed / 0 processing rows; property-inquiry notification email stayed blocked by missing recipient routing.
- June 29, 2026 19:52 MDT `npm run check:alert-notification-readiness` rebuilt worker output, stayed non-sending and non-mutating, returned `success=true`, and reported saved-search alert notification readiness at `watch`. Alert queue counts stayed 197 pending / 0 failed / 0 processing; sampled recipients were unsubscribed=false; sender fallback, missing reply-to, and pending-row dry-run review remained warnings.
- June 29, 2026 20:16 MDT `npm run run:alerts:dry -- --limit 50` completed in dry-run preview mode with 50 scanned rows, 0 sent, 0 skipped, 0 failed, and 50 masked rows marked `preview` / `Ready to send.` The execution plan stayed `caution`; no live alert send was run.
- June 29, 2026 20:29 MDT `npm run run:queue-dashboard -- --limit=5 --timeout-ms=3000` completed queue inspection cleanly. Diagnostics were empty; failed jobs, open dead-letter jobs, and stale active jobs were all clear; MLS sync, MLS page, listings, and dead-letter queues were healthy; `reie-alerts` remained `busy` with 273 waiting / 0 active / 0 failed jobs. No worker, retry, or live alert processing was run.
- June 29, 2026 20:35 MDT `npm run supabase:check:json` passed with `readiness.level="ready"` and no failed checks. Supabase URL/key checks, placeholder detection, project-ref consistency, Postgres URL shape, Supabase project DNS, Postgres DNS, Postgres TCP, Prisma `SELECT 1`, and Supabase REST all passed. The surfaced `npm run typesense:reindex` command was not run because reindexing remains out of scope unless explicitly requested.
- June 29, 2026 20:47 MDT `npm run typesense:collections:check` passed in collections-only mode. Canonical `properties` and `listings` schemas validated, and existing Typesense `properties` and `listings` collections were ready; no reset or reindex was run.
- June 30, 2026 02:59 MDT `npm run check:fast` passed. Worker output rebuilt; property-inquiry missing-recipient and dry-run suppression helpers passed; saved-search alert readiness stayed `watch`; consolidated notification readiness stayed `blocked`; strict-contract validation passed; bounded MLS sync dry-run completed with `executed=false` and no MLS Grid request; typecheck and lint passed.
- June 30, 2026 03:13 MDT `npm run build` passed. Next.js 15.1.6 compiled successfully, linted and checked types, collected page data, generated 130 static pages, finalized page optimization, and collected build traces; no live sync, worker, email, CRM, OpenAI, MLS Grid, reindex, reset, or queue retry operation was run.
- June 30, 2026 04:17 MDT `npm run check:property-inquiry-notification:readiness` rebuilt worker output, stayed non-sending and non-mutating, and failed closed as expected with `readiness.level="blocked"`. The only blocker remained `property_inquiry_recipient_missing` because `PROPERTY_INQUIRY_NOTIFY_TO` and fallback `REIE_INTERNAL_EMAIL` are unset; missing `RESEND_REPLY_TO_EMAIL` remained a warning.
- June 30, 2026 04:42 MDT `npm run check:notification-readiness` rebuilt worker output, stayed non-sending and non-mutating, returned `success=true`, and summarized overall notification readiness as `blocked`. Saved-search alert readiness stayed `watch`; property-inquiry notification and aggregate launch notification readiness stayed blocked by missing property-inquiry recipient routing.
- June 30, 2026 05:16 MDT `npm run check:notification-readiness:strict` rebuilt worker output, stayed non-sending and non-mutating, returned `strictMode=true`, `commandSuccess=true`, and failed closed as expected with `success=false` because overall notification readiness remains `blocked` by missing property-inquiry recipient routing.
- June 30, 2026 05:44 MDT `npm run check:notification-readiness:strict-contract` rebuilt worker output, stayed non-sending and non-mutating, and passed. It confirmed the current environment fails closed, the dummy-recipient plus property-inquiry dry-run scenario fails closed with direct and aggregate dry-run blockers detected, and aggregate launch-readiness reply-to warning alignment remains true.
- June 30, 2026 06:00 MDT `npm run check:launch-readiness` rebuilt worker output, stayed non-sending and non-mutating, ran read-only Prisma connectivity and alert-queue count checks, and failed closed as expected with `readiness.level="blocked"` and one blocked launch gate. Supabase connectivity stayed `ready`; saved-search alert email stayed `watch` with 197 pending / 0 failed / 0 processing rows; property-inquiry notification email stayed blocked by missing recipient routing.
- June 30, 2026 07:00 MDT `npm run check:alert-notification-readiness` rebuilt worker output, stayed non-sending and non-mutating, ran read-only Prisma connectivity, alert-queue count, and sampled-recipient checks, returned `success=true`, and reported saved-search alert notification readiness at `watch`. Alert queue counts stayed 197 pending / 0 failed / 0 processing; sampled recipients were unsubscribed=false.
- June 30, 2026 07:10 MDT `npm run run:alerts:dry -- --limit 50` completed in dry-run preview mode with 50 scanned rows, 0 sent, 0 skipped, 0 failed, and 50 masked rows marked `preview` / `Ready to send.` The execution plan stayed `caution`; no live alert send was run.
- June 30, 2026 07:31 MDT `npm run run:queue-dashboard -- --limit=5 --timeout-ms=3000` completed queue inspection cleanly. Diagnostics were empty; failed jobs, open dead-letter jobs, and stale active jobs were all clear; MLS sync, MLS page, listings, and dead-letter queues were healthy; `reie-alerts` remained `busy` with 273 waiting / 0 active / 0 failed jobs. No worker, retry, or live alert processing was run.
- June 30, 2026 07:35 MDT `npm run supabase:check:json` passed with `readiness.level="ready"` and no failed checks. Supabase URL/key checks, placeholder detection, project-ref consistency, Postgres URL shape, Supabase project DNS, Postgres DNS, Postgres TCP, Prisma `SELECT 1`, and Supabase REST all passed. The surfaced `npm run typesense:reindex` command was not run because reindexing remains out of scope unless explicitly requested.
- June 30, 2026 07:38 MDT `npm run typesense:collections:check` passed in collections-only mode. Canonical `properties` and `listings` schemas validated, and existing Typesense `properties` and `listings` collections were ready; no reset or reindex was run.
- June 30, 2026 07:47 MDT `npm run check:fast` passed. Worker output rebuilt; property-inquiry missing-recipient and dry-run suppression helpers passed; saved-search alert readiness stayed `watch`; consolidated notification readiness stayed `blocked`; strict-contract validation passed; bounded MLS sync dry-run completed with `executed=false` and no MLS Grid request; typecheck and lint passed.
- June 30, 2026 08:28 MDT `npm run build` passed. Next.js 15.1.6 compiled successfully, linted and checked types, collected page data, generated 130 static pages, finalized page optimization, and collected build traces; no live sync, worker, email, CRM, OpenAI, MLS Grid, reindex, reset, or queue retry operation was run.
- June 30, 2026 09:09 MDT `npm run check:property-inquiry-notification:readiness` rebuilt worker output, stayed non-sending and non-mutating, and failed closed as expected with `readiness.level="blocked"`. The only blocker remained `property_inquiry_recipient_missing` because `PROPERTY_INQUIRY_NOTIFY_TO` and fallback `REIE_INTERNAL_EMAIL` are unset; missing `RESEND_REPLY_TO_EMAIL` remained a warning.
- June 30, 2026 10:12 MDT `npm run check:notification-readiness` rebuilt worker output, stayed non-sending and non-mutating, returned `success=true`, and summarized overall notification readiness as `blocked`. Saved-search alert readiness stayed `watch` with sender fallback, missing reply-to, and 197 pending-row warnings; property-inquiry notification and aggregate launch notification readiness stayed blocked by missing property-inquiry recipient routing.
- June 30, 2026 11:12 MDT `npm run check:notification-readiness:strict` rebuilt worker output, stayed non-sending and non-mutating, returned `strictMode=true`, `commandSuccess=true`, and failed closed as expected with `success=false` because overall notification readiness remains blocked by missing property-inquiry recipient routing.
- June 30, 2026 11:15 MDT `npm run check:notification-readiness:strict-contract` rebuilt worker output, stayed non-sending and non-mutating, and passed. It confirmed the current environment fails closed, the dummy-recipient plus property-inquiry dry-run scenario fails closed with direct and aggregate dry-run blockers detected, and aggregate launch-readiness reply-to warning alignment remains true.
- June 30, 2026 11:17 MDT `npm run check:launch-readiness` rebuilt worker output, stayed non-sending and non-mutating, ran read-only Prisma connectivity and alert-queue count checks, and failed closed as expected with `readiness.level="blocked"` and one blocked launch gate. Supabase connectivity stayed `ready`; saved-search alert email stayed `watch` with 197 pending / 0 failed / 0 processing rows; property-inquiry notification email stayed blocked by missing recipient routing.
- June 30, 2026 11:42 MDT `npm run check:alert-notification-readiness` rebuilt worker output, stayed non-sending and non-mutating, ran read-only Prisma connectivity, alert-queue count, and sampled-recipient checks, returned `success=true`, and reported saved-search alert notification readiness at `watch`. Alert queue counts stayed 197 pending / 0 failed / 0 processing; sampled recipients were unsubscribed=false.
- June 30, 2026 11:58 MDT `npm run run:alerts:dry -- --limit 50` completed in dry-run preview mode with 50 scanned rows, 0 sent, 0 skipped, 0 failed, and 50 masked rows marked `preview` / `Ready to send.` The execution plan stayed `caution`; no live alert send was run.
- June 30, 2026 13:02 MDT `npm run run:queue-dashboard -- --limit=5 --timeout-ms=3000` completed queue inspection cleanly. Diagnostics were empty; failed jobs, open dead-letter jobs, and stale active jobs were all clear; MLS sync, MLS page, listings, and dead-letter queues were healthy; `reie-alerts` remained `busy` with 273 waiting / 0 active / 0 failed jobs. No worker, retry, or live alert processing was run.
- June 30, 2026 13:14 MDT `npm run supabase:check:json` passed with `readiness.level="ready"` and no failed checks. Supabase URL/key checks, placeholder detection, project-ref consistency, Postgres URL shape, Supabase project DNS, Postgres DNS, Postgres TCP, Prisma `SELECT 1`, and Supabase REST all passed. The surfaced `npm run typesense:reindex` command was not run because reindexing remains out of scope unless explicitly requested.
- June 30, 2026 13:55 MDT `npm run typesense:collections:check` passed in collections-only mode. Canonical `properties` and `listings` schemas validated, and existing Typesense `properties` and `listings` collections were ready; no reset or reindex was run.
- June 30, 2026 14:03 MDT `npm run check:fast` passed end to end. Worker output rebuilt; property-inquiry missing-recipient and dry-run suppression helpers passed; saved-search alert readiness stayed `watch`; consolidated notification readiness stayed `blocked`; strict-contract validation passed; bounded MLS sync dry-run completed with `executed=false` and no MLS Grid request; typecheck and lint passed.
- June 30, 2026 14:09 MDT `npm run build` passed. Next.js 15.1.6 compiled successfully, linted and checked types, collected page data, generated 130 static pages, finalized page optimization, and collected build traces.
- June 30, 2026 14:28 MDT `npm run check:property-inquiry-notification:readiness` rebuilt worker output, sent no email, mutated no rows, and failed closed as expected with `readiness.level="blocked"` because `PROPERTY_INQUIRY_NOTIFY_TO` is missing and fallback `REIE_INTERNAL_EMAIL` is unset. `PROPERTY_INQUIRY_NOTIFICATION_DRY_RUN` passed because it is disabled or unset; missing `RESEND_REPLY_TO_EMAIL` remained a warning.
- June 30, 2026 15:36 MDT `npm run check:notification-readiness` rebuilt worker output, sent no email, mutated no rows, returned `success=true`, and summarized overall notification readiness as `blocked`. Saved-search alert readiness stayed `watch`; property-inquiry notification and aggregate launch notification readiness stayed blocked by `property_inquiry_recipient_missing`.
- June 30, 2026 16:01 MDT `npm run check:notification-readiness:strict` rebuilt worker output, sent no email, mutated no rows, returned `strictMode=true`, `commandSuccess=true`, and failed closed as expected with `success=false` because overall notification readiness remains blocked by missing property-inquiry recipient routing.
- June 30, 2026 17:13 MDT `npm run check:notification-readiness:strict-contract` rebuilt worker output, sent no email, mutated no rows, and passed. It confirmed the current environment fails closed, the dummy-recipient plus property-inquiry dry-run scenario fails closed with direct and aggregate dry-run blockers detected, and aggregate launch-readiness reply-to warning alignment remains true.
- June 30, 2026 17:19 MDT `npm run check:launch-readiness` rebuilt worker output, sent no email, mutated no rows, ran read-only Prisma connectivity and alert-queue count checks, and failed closed as expected with `readiness.level="blocked"` and one blocked launch gate. Supabase connectivity stayed `ready`; saved-search alert email stayed `watch` with 197 pending / 0 failed / 0 processing rows; property-inquiry notification email stayed blocked by missing recipient routing.
- June 30, 2026 17:32 MDT `npm run check:alert-notification-readiness` rebuilt worker output, sent no email, mutated no rows, ran read-only Prisma connectivity, alert-queue count, and sampled-recipient checks, returned `success=true`, and reported saved-search alert notification readiness at `watch`. Alert queue counts stayed 197 pending / 0 failed / 0 processing; sampled recipients were unsubscribed=false.
- June 30, 2026 17:34 MDT `npm run run:alerts:dry -- --limit 50` completed in dry-run preview mode with `success=true`, 50 scanned rows, 0 sent, 0 skipped, 0 failed, and 50 masked rows marked `preview` / `Ready to send.` No live alert command was run.
- June 30, 2026 17:38 MDT `npm run run:queue-dashboard -- --limit=5 --timeout-ms=3000` completed with no diagnostics, no failed jobs, no open dead-letter jobs, and no stale active jobs. MLS sync, MLS page, listings, and dead-letter queues were healthy; `reie-alerts` remained `busy` with 273 waiting / 0 active / 0 failed jobs. No worker, retry, or live alert processing was run.
- June 30, 2026 17:41 MDT `npm run supabase:check:json` passed with `readiness.level="ready"` and no failed checks. Supabase URL/key checks, placeholder detection, project-ref consistency, Postgres URL shape, Supabase project DNS, Postgres DNS, Postgres TCP, Prisma `SELECT 1`, and Supabase REST all passed. The surfaced `npm run typesense:reindex` command was not run because reindexing remains out of scope unless explicitly requested.
- July 1, 2026 10:49 MDT `npm run typesense:collections:check` passed in collections-only mode. Canonical `properties` and `listings` schemas validated, and existing Typesense `properties` and `listings` collections were ready; no reset or reindex was run.
- July 1, 2026 10:57 MDT configured a gitignored local `.env.local` `PROPERTY_INQUIRY_NOTIFY_TO` value, then reran the non-sending readiness gates. Direct property-inquiry readiness, consolidated notification readiness, strict notification readiness, and aggregate launch readiness now return `watch` with no blockers; `RESEND_REPLY_TO_EMAIL`, sender fallback, and 197 pending alert rows remain operator-review warnings. `npm run check:notification-readiness:strict-contract` and `npm run check:fast` passed after hardening the strict-contract wrapper to test missing-recipient behavior through an explicit override environment.
- July 1, 2026 11:17 MDT the user confirmed Vercel has `PROPERTY_INQUIRY_NOTIFY_TO` scoped to Production and Preview, and confirmed production was redeployed after adding it. The hidden Vercel value was not printed; launch posture assumes it remains `davidquinngroup@gmail.com`.
- July 1, 2026 11:21 MDT reran `npm run check:property-inquiry-notification:readiness`, `npm run check:notification-readiness`, `npm run check:notification-readiness:strict`, and `npm run check:launch-readiness`. All four non-sending, non-mutating gates passed with readiness `watch` and no blockers. Property-inquiry recipient routing passed as masked `da***@gmail.com`; missing `RESEND_REPLY_TO_EMAIL`, sender fallback, and 197 pending alert rows remain operator-review warnings.
- July 1, 2026 11:25 MDT reran `npm run run:alerts:dry -- --limit 50`; it stayed in dry-run preview mode with 50 scanned rows, 50 ready-to-send preview rows, 0 sent, 0 skipped, and 0 failed. Follow-up `npm run run:queue-dashboard -- --limit=5 --timeout-ms=3000` completed with no diagnostics, no failed jobs, no open dead-letter jobs, and no stale active jobs; `reie-alerts` remained busy with 273 waiting / 0 active / 0 failed.
- July 1, 2026 11:28 MDT reran `npm run supabase:check:json` and `npm run typesense:collections:check`. Supabase readiness stayed `ready` with no failed checks. Typesense `properties` and `listings` collections stayed ready in collections-only mode; no reset or reindex was run.
- July 1, 2026 11:31 MDT `npm run check:fast` passed end to end. Worker output rebuilt; property-inquiry skip helpers passed; saved-search alert readiness stayed `watch`; consolidated notification readiness stayed `watch`; strict-contract validation passed; bounded MLS sync dry-run completed with `executed=false` and no MLS Grid request; typecheck and lint passed.
- July 1, 2026 11:33 MDT `npm run build` passed. Next.js 15.1.6 compiled successfully, completed lint/type validation, collected page data, generated 130 static pages, finalized page optimization, and collected build traces.
- July 1, 2026 11:47 MDT runtime smoke refresh passed after hardening `scripts/opsSmoke.ts` and regenerated `dist/scripts/opsSmoke.js` for configured property-inquiry readiness. `npm run smoke:mls-status` returned HTTP 200 with search-index health `healthy`; `npm run smoke:search` returned HTTP 200 with `meta.smoke.ready=true`; `npm run smoke:ops` passed with notification readiness `watch`, property-inquiry recipient status pass, dry-run guard coverage, and public-experience smoke assertions. The temporary local Next dev server was stopped afterward.
- July 2, 2026 11:29 MDT user-confirmed Resend readiness and Vercel reply-to setup: `davidquinngroup.com` is verified in Resend with DKIM and SPF verified, `RESEND_REPLY_TO_EMAIL=davidquinngroup@gmail.com` is configured in Vercel Production/Preview, and production was redeployed. The same reply-to was mirrored into gitignored local `.env.local` for parity. `npm run check:property-inquiry-notification:readiness`, `npm run check:notification-readiness`, `npm run check:notification-readiness:strict`, `npm run check:launch-readiness`, `npm run check:alert-notification-readiness`, `npm run check:notification-readiness:strict-contract`, and `npm run check:fast` passed without sending email or mutating rows. Property-inquiry notification readiness is now `ready`; consolidated and aggregate notification readiness remain `watch` because saved-search alerts still use sender fallback and 197 pending rows need dry-run review.
- July 2, 2026 11:39 MDT user-confirmed Vercel sender setup: `RESEND_FROM_EMAIL=alerts@davidquinngroup.com` is configured in Vercel Production/Preview and production was redeployed. The same sender was mirrored into gitignored local `.env.local` for parity. `npm run check:alert-notification-readiness`, `npm run check:property-inquiry-notification:readiness`, `npm run check:notification-readiness`, `npm run check:launch-readiness`, `npm run check:notification-readiness:strict`, `npm run check:notification-readiness:strict-contract`, and `npm run check:fast` passed without sending email or mutating rows. `npm run run:alerts:dry -- --limit 50` previewed 50 ready-to-send rows, sent 0, skipped 0, failed 0, and mutated no rows.
- July 2, 2026 11:45 MDT `npm run run:queue-dashboard -- --limit=5 --timeout-ms=3000` passed after sender readiness cleared. Queue inspection reported no diagnostics, no failed jobs, no open dead-letter jobs, and no stale active jobs; `reie-alerts` remained busy with 273 waiting / 0 active / 0 failed jobs. The surfaced alert worker command was not run.
- July 2, 2026 12:34 MDT `npm run check:launch-readiness` passed without sending email or mutating rows. Aggregate readiness stayed `watch` with no blockers: Supabase connectivity and property-inquiry notification email were `ready`, saved-search alert email stayed `watch` only because 197 pending rows still require final dry-run review before live processing, and failed/processing saved-search alert rows stayed 0.
- July 2, 2026 12:41 MDT `npm run run:alerts:dry -- --limit 50` passed in preview mode. It scanned 50 saved-search alert rows, previewed 50 ready-to-send rows, sent 0, skipped 0, failed 0, and mutated no rows; the surfaced live alert command was not run.
- July 2, 2026 12:47 MDT `npm run check:notification-readiness:strict` passed in strict mode without sending email or mutating rows. Notification readiness stayed `watch` with no blockers: property-inquiry notification was `ready`, saved-search alert notification stayed `watch`, and the only warning remained 197 pending saved-search alert rows requiring operator review.
- July 2, 2026 12:50 MDT `npm run check:notification-readiness:strict-contract` passed without sending email or mutating rows. Current env readiness stayed `watch` with exit code 0, while missing-recipient and property-inquiry dry-run override scenarios still failed closed with readiness `blocked` and exit code 1.
- July 2, 2026 13:28 MDT `npm run check:fast` passed end to end. Worker output rebuilt; notification readiness stayed `watch`; strict-contract validation passed; bounded MLS sync dry-run completed with `executed=false` and no MLS Grid request; typecheck passed; lint passed with no ESLint warnings or errors.
- July 2, 2026 13:53 MDT `npm run build` passed. Next.js 15.1.6 compiled successfully, completed lint/type validation, collected page data, generated 130 static pages, finalized page optimization, and collected build traces.
- July 2, 2026 14:02 MDT runtime smoke refresh passed against a temporary local Next dev server. `npm run smoke:mls-status` and `npm run smoke:search` returned HTTP 200; `npm run smoke:ops` passed with notification readiness `watch`, saved-search alert notification `watch`, property-inquiry notification `ready`, and public-experience smoke assertions. The temporary server was stopped afterward.
- The former missing property-inquiry recipient blocker, reply-to warning, and sender fallback warning are cleared for local and Vercel production posture, assuming the hidden Vercel values remain `davidquinngroup@gmail.com` and `alerts@davidquinngroup.com`. Continue to avoid live property-inquiry notification delivery, recurring alert sends, digest sends, or recurring email traffic until remaining watch items and final launch policy are reviewed.
