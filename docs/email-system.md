...
- June 29, 2026 12:00 MDT `npm run check:property-inquiry-notification:readiness` refreshed the direct non-sending, non-mutating property-inquiry notification gate and exited blocked as expected. `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `NEXT_PUBLIC_SITE_URL`, and disabled/unset `PROPERTY_INQUIRY_NOTIFICATION_DRY_RUN` checks passed; `RESEND_REPLY_TO_EMAIL` remained a warning.
