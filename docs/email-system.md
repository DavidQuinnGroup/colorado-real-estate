...
- Alert and digest dry-runs require `npm run supabase:check:json` readiness; current aggregate notification readiness is `watch` with property-inquiry notification readiness `ready`, saved-search alert sender fallback, and 197 pending saved-search alert rows requiring dry-run review before live processing.
...
- The former missing property-inquiry recipient blocker and reply-to warning are cleared for local and Vercel production posture, assuming the hidden Vercel values remain `davidquinngroup@gmail.com`. Continue to avoid live property-inquiry notification delivery, recurring alert sends, digest sends, or recurring email traffic until remaining watch items and final launch policy are reviewed.
