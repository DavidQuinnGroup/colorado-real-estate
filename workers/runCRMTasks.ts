...
type RunCRMTasksOptions = {
  limit?: number;
  status?: string;
  quiet?: boolean;
};

type CRMTaskWithUser = CRMTask & {
  user: {
    email: string;
    name: string | null;
    heatScore: number;
...
function getCRMTaskFailureReadiness(error: unknown): CRMTaskReadiness {
...
function parseArgs(argv: string[]): RunCRMTasksOptions | null {
  const options: RunCRMTasksOptions = {
    limit: DEFAULT_LIMIT,
...
function getMetadata(task: CRMTask): Prisma.JsonObject {
  return task.metadata && typeof task.metadata === 'object' && !Array.isArray(task.metadata) ? task.metadata : {};
}

function getRecord(value: unknown): Prisma.JsonObject {
...
function getIntentSummary(task: CRMTask) {
...
function getNextAction(task: CRMTask) {
  const metadata = getMetadata(task);
...
function getAlertReadiness(task: CRMTask): CRMTaskSummary['tasks'][number]['alertReadiness'] {
  const metadata = getMetadata(task);
...
function getLatestSavedSearchIntake(task: CRMTask): CRMTaskSummary['tasks'][number]['latestSavedSearchIntake'] {
  const metadata = getMetadata(task);
...
function toTaskSummary(task: CRMTaskWithUser): CRMTaskSummary['tasks'][number] {
  return {
    id: task.id,
    leadId: task.leadId,
...
async function fetchAuditSummary(): Promise<CRMTaskAuditSummary> {
...
function getReviewCommand(status: string) {
  if (status === 'active') return 'npm run run:crm:active';
  if (status === 'pending') return 'npm run run:crm:pending';
  if (status === 'reviewing') return 'npm run run:crm:reviewing';
  if (status === 'all') return 'npm run run:crm:all';
  return `npm run run:crm -- --limit ${DEFAULT_LIMIT} --status ${status}`;
}

function getReadiness(summary: Omit<CRMTaskSummary, 'readiness'>): CRMTaskReadiness {
...
async function fetchTasks(limit: number, status: string): Promise<CRMTaskWithUser[]> {
...
function printSummary(summary: CRMTaskSummary) {
...
export async function runCRMTasks(options: RunCRMTasksOptions = {}): Promise<CRMTaskSummary> {
  const limit = clampInteger(options.limit, DEFAULT_LIMIT, 1, MAX_LIMIT);
  const status = normalizeStatus(options.status);
  const quiet = Boolean(options.quiet);

  try {
    if (!quiet) console.log('REIE CRM task worker starting:', { limit, status });
    ...
    const summaryWithoutReadiness: Omit<CRMTaskSummary, 'readiness'> = {
      success: true,
    ...
    if (!quiet) console.error('REIE CRM task worker failed:', error instanceof Error ? error.message : error);
