...
function readOptionalBoolean(value: string | undefined) {
  ...
  if (['1', 'true', 'yes', 'y'].includes(value.trim().toLowerCase())) return true;
  if (['0', 'false', 'no', 'n'].includes(value.trim().toLowerCase())) return false;
  return undefined;
}
...
export async function runMLSWorker(options: MlsSyncJobData = {}) {
  return syncMLSGrid(normalizeJobData(options));
}

function summarizeResult(result: MlsSyncJobResult) {
  if (!result) {
    return {
      started: false,
...
function shouldDeadLetterFailedJob(job: Job<MlsSyncJobData> | undefined) {
  if (!job) return true;
...
function getFailureOperations(job: Job<MlsSyncJobData> | undefined, finalAttempt: boolean) {
  const jobId = job?.id;
...
function createMlsSyncWorker(config: MlsWorkerConfig) {
  const worker = new Worker<MlsSyncJobData, MlsSyncJobResult>(
    MLS_SYNC_QUEUE_NAME,
