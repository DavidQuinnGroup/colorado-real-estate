...
import { ALERT_QUEUE_NAME, type AlertJobData } from '../lib/queue/alertQueue.js';
import { enqueueDeadLetterFromJob } from '../lib/queue/deadLetterQueue.js';
import { getRedisConnection } from '../lib/queue/redis.js';

type AlertWorkerMode = 'queue' | 'batch' | 'hybrid';

type AlertWorkerConfig = {
  batchSize: number;
  intervalMs: number;
  once: boolean;
  mode: AlertWorkerMode;
  concurrency: number;
  dryRun: boolean;
};

type AlertBatchResult = Awaited<ReturnType<typeof processAlertQueue>>;
...
function readMode(value: string | undefined): AlertWorkerMode {
  if (value === 'queue' || value === 'batch' || value === 'hybrid') return value;
  return 'hybrid';
}

function getConfig(): AlertWorkerConfig {
  return {
    batchSize: readNumber(process.env.ALERT_WORKER_BATCH_SIZE, DEFAULT_BATCH_SIZE, 1, 200),
    intervalMs: readNumber(process.env.ALERT_WORKER_INTERVAL_MS, DEFAULT_INTERVAL_MS, 5_000, 30 * 60_000),
...
function validateConfig(config: AlertWorkerConfig) {
...
function shouldDeadLetterFailedJob(job: Job<AlertJobData> | undefined) {
  if (!job) return true;
...
function getFailureOperations(job: Job<AlertJobData> | undefined, finalAttempt: boolean) {
  const jobId = job?.id;
...
function getBatchRecommendation(result: AlertBatchResult) {
  if (result.failed > 0) return 'Review failed alert rows before running a larger or live batch.';
  if (result.dryRun && result.scanned > 0) return 'Dry-run preview completed. Review rows before enabling live worker processing.';
  if (result.dryRun) return 'Dry-run found no pending alert work.';
  if (result.sent > 0) return 'Live alert batch sent email. Confirm EmailLog and AlertQueue status.';
  return 'No live alert sends were required for this batch.';
}

async function runAlertBatch(config: AlertWorkerConfig) {
...
function getAlertId(job: Job<AlertJobData>) {
...
async function startQueueWorker(config: AlertWorkerConfig) {
  const connection = getRedisConnection();
  const worker = new Worker<AlertJobData>(ALERT_QUEUE_NAME, processAlertJob, {
    connection,
...
function startShutdownHandlers(workers: Worker[]) {
...
async function startPollingLoop(config: AlertWorkerConfig) {
  while (true) {
    try {
