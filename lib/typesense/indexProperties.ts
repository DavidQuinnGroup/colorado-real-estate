...
function compactNetworkError(value: string) {
...
function errorMessage(error: unknown) {
  if (error instanceof Error) return compactNetworkError(toSingleLine(error.message));
...
function getSupabaseHost() {
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
  if (!supabaseUrl) return 'unknown Supabase host';
...
function getSupabaseFetchFailureMessage(error: unknown) {
  return [
...
async function fetchPropertyBatch(from: number, to: number) {
  let response: Awaited<ReturnType<ReturnType<SupabaseClient['from']>['select']>>;
