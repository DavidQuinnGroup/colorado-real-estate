...
type TypesenseField = NonNullable<CollectionCreateSchema['fields']>[number];

export type SchemaFieldRule = Readonly<{
  name: string;
  type: TypesenseField['type'];
  facet?: boolean;
  sort?: boolean;
  optional?: boolean;
}>;

type InvalidSchemaField = {
  name: string;
  issue: string;
};
...
type SchemaValidationResult = {
  ...
  missingQueryFields: string[];
  missingFilterFields: string[];
  missingSortFields: string[];
  invalidFields: InvalidSchemaField[];
  invalidDefaultSortingField: string | null;
};
...
export const SEARCH_SCHEMA_FIELD_RULES: readonly SchemaFieldRule[] = [
  { name: 'id', type: 'string' },
...
export const SEARCH_SCHEMA_REQUIRED_FIELD_NAMES = SEARCH_SCHEMA_FIELD_RULES.filter((field) => !field.optional).map(
  (field) => field.name,
);

export const SEARCH_SCHEMA_REQUIRED_FACET_FIELD_NAMES = SEARCH_SCHEMA_FIELD_RULES.filter((field) => field.facet).map(
  (field) => field.name,
);
...
export const SEARCH_SCHEMA_SORT_FIELD_NAMES = ['price', 'updatedAt'] as const;
...
function toTypesenseField(rule: SchemaFieldRule): TypesenseField {
  return {
    name: rule.name,
    type: rule.type,
    facet: rule.facet,
...
function createSearchCollectionSchema(name: string): CollectionCreateSchema {
  return {
    name,
...
function getFieldMap(schema: CollectionCreateSchema) {
  return new Map((schema.fields || []).map((field) => [field.name, field]));
}

function getDuplicateFieldNames(schema: CollectionCreateSchema) {
...
function getInvalidFields(schema: CollectionCreateSchema) {
  const fieldMap = getFieldMap(schema);
  const invalidFields: InvalidSchemaField[] = [];

  for (const rule of SEARCH_SCHEMA_FIELD_RULES) {
    if (TYPESENSE_RESERVED_FIELD_NAMES.has(rule.name)) continue;
...
function getMissingNamedFields(schema: CollectionCreateSchema, fieldNames: readonly string[]) {
  const fieldMap = getFieldMap(schema);
  return fieldNames.filter((fieldName) => !fieldMap.has(fieldName));
}

export function getSearchSchemaValidation(schema: CollectionCreateSchema): SchemaValidationResult {
  const fieldMap = getFieldMap(schema);

  return {
    duplicateFields: getDuplicateFieldNames(schema),
    missingFields: SEARCH_SCHEMA_REQUIRED_FIELD_NAMES.filter(
      (fieldName) => !TYPESENSE_RESERVED_FIELD_NAMES.has(fieldName) && !fieldMap.has(fieldName),
    ),
    ...
    invalidDefaultSortingField:
      schema.default_sorting_field === SEARCH_SCHEMA_DEFAULT_SORTING_FIELD
        ? null
        : `expected ${SEARCH_SCHEMA_DEFAULT_SORTING_FIELD}, received ${schema.default_sorting_field || 'none'}`,
  };
}

export function formatSearchSchemaValidationError(schema: CollectionCreateSchema) {
...
export function validateSearchSchema(schema: CollectionCreateSchema) {
