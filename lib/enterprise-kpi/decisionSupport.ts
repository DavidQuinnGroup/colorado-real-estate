...
import {
  ...
  type ExecutiveCommandCenterPayload,
} from "./executiveWorkspace.js";
...
export const DECISION_SUPPORT_PROVENANCE = "NON_PRODUCTION_FIXTURE" as const satisfies ObservationProvenance;

export type DecisionUrgency = "IMMEDIATE" | "NEAR_TERM" | "SCHEDULED" | "LOW";
export type DecisionHorizon = "NOW" | "SEVEN_DAYS" | "THIRTY_DAYS" | "NINETY_DAYS";
export type DecisionOptionType =
  ...
  | "INVESTIGATE"
  | "VALIDATE_DATA";
export type CriterionDirection = "BENEFIT" | "BURDEN";
export type CriterionAssessmentState = "SCORED" | "UNKNOWN" | "NOT_APPLICABLE" | "INSUFFICIENT_EVIDENCE";
export type DecisionRecommendationKind =
  | "RECOMMEND_OPTION"
  | "NO_RECOMMENDATION_INSUFFICIENT_EVIDENCE"
  | "EXECUTIVE_JUDGMENT_REQUIRED"
  | "VALIDATE_DATA_BEFORE_DECISION"
  | "DEFER_PENDING_EVIDENCE";
export type DecisionDispositionStatus =
  | "APPROVED"
  | "REJECTED"
  | "DEFERRED"
  | "REVISED"
  ...
  | "NO_DECISION";
export type DecisionConfidenceLevel = ConfidenceAssessment["level"];

export type DecisionEvidenceReference = EvidenceReference & {
  decisionUse: "SITUATION" | "OPTION" | "CRITERION" | "OUTCOME" | "RECOMMENDATION";
};
...
export type DecisionSituation = {
  ...
  triggeringIntelligenceEventIds: string[];
  ...
  knownLimitations: DecisionLimitation[];
};
...
export type DecisionCriterionScore = {
  criterionId: string;
  rawAssessment: number | "UNKNOWN" | "NOT_APPLICABLE" | "INSUFFICIENT_EVIDENCE";
  normalizedScore: number | null;
  weight: number;
  weightedContribution: number | null;
  supportingEvidence: DecisionEvidenceReference[];
  confidence: ConfidenceAssessment;
  assumptions: string[];
  missingDataStatus: CriterionAssessmentState;
  explanation: string;
};

export type EnterpriseDecisionScore = {
  totalScore: number | null;
  maximumPossibleScore: 100;
  coveragePercentage: number;
  confidence: ConfidenceAssessment;
  calculationApplicabilityConfidence: DecisionConfidenceLevel;
  realWorldApplicabilityConfidence: DecisionConfidenceLevel;
  criterionContributions: DecisionCriterionScore[];
...
export type ExpectedOutcome = {
  ...
  baseline: number | "UNAVAILABLE";
  target: number | "UNAVAILABLE";
...
export type DecisionOption = {
  ...
  financialImplications: string | "UNKNOWN";
  risks: DecisionRiskComponent[];
  dependencies: string[];
  ...
  assumptions: string[];
  limitations: DecisionLimitation[];
  score: EnterpriseDecisionScore;
};

export type DecisionRiskComponent = {
  riskId: string;
  category: "TECHNICAL" | "OPERATIONAL" | "CUSTOMER" | "GOVERNANCE" | "FINANCIAL_UNCERTAINTY" | "OPPORTUNITY_COST";
...
export type DecisionRecommendation = {
  recommendationKind: DecisionRecommendationKind;
  recommendedOptionId: string | null;
  reason: string;
  supportingCriteria: string[];
  materialRisks: DecisionRiskComponent[];
  materialUnknowns: string[];
  confidence: ConfidenceAssessment;
  alternativeOptionId: string | null;
  conditionsThatCouldChangeRecommendation: string[];
  humanReviewRequirement: typeof HUMAN_DECISION_REQUIRED;
  evidence: DecisionEvidenceReference[];
  calculationVersion: string;
  provenance: ObservationProvenance;
};
...
export type DecisionOverride = {
  originalRecommendationOptionId: string | null;
  selectedOptionId: string | null;
  overrideRationale: string;
  decisionAuthority: "EXECUTIVE_ROLE_PLACEHOLDER";
  knownRisksAccepted: string[];
  expectedOutcomes: ExpectedOutcome[];
  reviewDate: string;
  evidenceAcknowledged: DecisionEvidenceReference[];
  timestamp: string;
  provenance: ObservationProvenance;
  demonstrationOnly: true;
};
...
export type DecisionReviewSchedule = {
  ...
  relevantExpectedOutcomeIds: string[];
  evidenceRequired: string[];
  responsibleLeadershipRole: string;
  schedulingStatus: "SEMANTIC_ONLY_NOT_SCHEDULED";
};

export type DecisionLimitation = {
  limitationId: string;
  description: string;
  severity: "LOW" | "MEDIUM" | "HIGH";
};

export type EnterpriseDecisionPackage = {
  packageId: string;
  labels: [typeof HUMAN_DECISION_REQUIRED, typeof DECISION_SUPPORT_PROVENANCE];
  ...
  optionComparison: {
    ...
    scores: { optionId: string; score: EnterpriseDecisionScore }[];
    ranking: { optionId: string; rank: number; score: number | null }[];
  };
  recommendation: DecisionRecommendation;
  risksAndTradeoffs: DecisionRiskComponent[];
  expectedOutcomes: ExpectedOutcome[];
  confidence: ConfidenceAssessment;
  dataFreshness: FreshnessAssessment;
  knownLimitations: DecisionLimitation[];
  humanDecisionRequired: typeof HUMAN_DECISION_REQUIRED;
  reviewSchedule: DecisionReviewSchedule;
  dispositionDemo: DecisionDisposition;
  overrideDemo: DecisionOverride;
  calculationVersion: string;
  provenance: ObservationProvenance;
};

type OptionDraft = Omit<DecisionOption, "score"> & {
  criterionRawScores: Record<string, number | "UNKNOWN" | "NOT_APPLICABLE" | "INSUFFICIENT_EVIDENCE">;
};
...
export function validateDecisionCriteriaWeights(criteria: DecisionCriterion[] = DECISION_CRITERIA) {
...
export function buildDecisionSupportSnapshot() {
  ...
  return {
    metadata: {
      ...
      labels: [HUMAN_DECISION_REQUIRED, DECISION_SUPPORT_PROVENANCE] as const,
    },
    ...
    summary: {
      ...
      recommendationsAwaitingHumanReview: packages.filter(
        (item) => item.recommendation.humanReviewRequirement === HUMAN_DECISION_REQUIRED,
      ).length,
      moreEvidenceRequired: packages.filter(
        (item) =>
          item.recommendation.recommendationKind === "VALIDATE_DATA_BEFORE_DECISION" ||
          item.recommendation.recommendationKind === "DEFER_PENDING_EVIDENCE" ||
          item.recommendation.recommendationKind === "NO_RECOMMENDATION_INSUFFICIENT_EVIDENCE",
      ).length,
...
export function getDecisionSituations(options: { limit?: number; offset?: number; domain?: KpiDomain } = {}) {
  const snapshot = buildDecisionSupportSnapshot();
  const filtered = options.domain
    ? snapshot.situations.filter((item) => item.relevantDomains.includes(options.domain as KpiDomain))
    : snapshot.situations;
...
export function getDecisionPackages(options: { limit?: number; offset?: number; domain?: KpiDomain } = {}) {
  const snapshot = buildDecisionSupportSnapshot();
  const filtered = options.domain
    ? snapshot.packages.filter((item) => item.situation.relevantDomains.includes(options.domain as KpiDomain))
    : snapshot.packages;
  const limit = Math.min(Math.max(options.limit ?? 100, 1), 250);
  const offset = Math.max(options.offset ?? 0, 0);
  return {
...
export function getDecisionPackage(packageId: string) {
  return buildDecisionSupportSnapshot().packages.find((item) => item.packageId === packageId) ?? null;
}

export function getDecisionPackageComparison(packageId: string) {
  const decisionPackage = getDecisionPackage(packageId);
  return decisionPackage?.optionComparison ?? null;
}

export function getDecisionPackageRecommendation(packageId: string) {
  const decisionPackage = getDecisionPackage(packageId);
  return decisionPackage?.recommendation ?? null;
}

export function getDecisionPackageExpectedOutcomes(packageId: string) {
  const decisionPackage = getDecisionPackage(packageId);
  return decisionPackage?.expectedOutcomes ?? null;
}

function buildDecisionPackages(commandCenter: ExecutiveCommandCenterPayload): EnterpriseDecisionPackage[] {
  ...
  return packages.sort(
    (left, right) =>
      urgencyRank(right.situation.urgency) - urgencyRank(left.situation.urgency) ||
      left.packageId.localeCompare(right.packageId),
  );
}

function buildPlatformRiskPackage(
  commandCenter: ExecutiveCommandCenterPayload,
...
function buildCustomerOpportunityPackage(
  commandCenter: ExecutiveCommandCenterPayload,
...
function buildDataIntegrityPackage(
  commandCenter: ExecutiveCommandCenterPayload,
  attention: ExecutiveAttentionItem | null,
) {
...
function buildGovernanceRecoveryPackage(
  commandCenter: ExecutiveCommandCenterPayload,
  opportunity: OpportunitySignal | null,
) {
...
function decisionPackage(
  ...
  const knownLimitations = uniqueBy(
    [...situation.knownLimitations, ...options.flatMap((item) => item.limitations)],
...
function scoreOptions(options: OptionDraft[]): DecisionOption[] {
...
function scoreOption(option: OptionDraft): EnterpriseDecisionScore {
  ...
  const level: DecisionConfidenceLevel =
    coveragePercentage >= 85 && unknownCriteria.length === 0 ? "MEDIUM" : coveragePercentage >= 55 ? "LOW" : "INSUFFICIENT";
...
function scoreCriterion(criterion: DecisionCriterion, option: OptionDraft): DecisionCriterionScore {
  const raw = option.criterionRawScores[criterion.criterionId] ?? "UNKNOWN";
  const state: CriterionAssessmentState =
    raw === "UNKNOWN" ? "UNKNOWN" :
...
function recommendOption(options: DecisionOption[]): DecisionRecommendation {
...
function buildOptionComparison(options: DecisionOption[]) {
...
function optionDraft(
  ...
  proposedAction: string,
  evidence: DecisionEvidenceReference[],
  criterionRawScores: OptionDraft["criterionRawScores"],
): OptionDraft {
  return {
    optionId,
    type,
...
function risksForOption(optionId: string, evidence: DecisionEvidenceReference[], rawRisk: OptionDraft["criterionRawScores"][string]): DecisionRiskComponent[] {
...
function outcomesForOption(optionId: string, evidence: DecisionEvidenceReference[]): ExpectedOutcome[] {
...
function buildSituation(input: Omit<DecisionSituation, "knownLimitations" | "provenance">): DecisionSituation {
...
function decisionEvidence(evidence: EvidenceReference[]): DecisionEvidenceReference[] {
...
function kpis(evidence: DecisionEvidenceReference[], fallback: string[]) {
  const ids = evidence.map((item) => item.kpiId).filter((item): item is string => Boolean(item));
  return [...new Set(ids.length > 0 ? ids : fallback)];
}

function eventIdsFromEvidence(
  commandCenter: ExecutiveCommandCenterPayload,
  evidence: DecisionEvidenceReference[],
) {
...
function packageConfidence(options: DecisionOption[], situationValue: DecisionSituation): ConfidenceAssessment {
  const avgCoverage = options.reduce((sum, option) => sum + option.score.coveragePercentage, 0) / Math.max(options.length, 1);
  const level: DecisionConfidenceLevel = avgCoverage >= 85 && situationValue.confidence.level !== "INSUFFICIENT" ? "MEDIUM" : avgCoverage >= 55 ? "LOW" : "INSUFFICIENT";
...
function confidence(level: DecisionConfidenceLevel, evidenceCount: number, factors?: string[]): ConfidenceAssessment {
...
function freshnessFromDecisionEvidence(evidence: DecisionEvidenceReference[]): FreshnessAssessment {
  const latest = [...evidence].sort((left, right) => right.timestamp.localeCompare(left.timestamp))[0] ?? null;
  if (!latest) return { state: "UNKNOWN", ageHours: null, expectationHours: null, assessedAt: FIXTURE_OBSERVATION_TIMESTAMP };
  const ageHours = (new Date(FIXTURE_OBSERVATION_TIMESTAMP).getTime() - new Date(latest.timestamp).getTime()) / 3_600_000;
...
function reviewSchedule(packageId: string, outcomes: ExpectedOutcome[]): DecisionReviewSchedule {
  return {
    reviewDate: "2026-08-17",
...
function limitation(limitationId: string, description: string, severity: DecisionLimitation["severity"]): DecisionLimitation {
  return { limitationId, description, severity };
}

function urgencyRank(urgency: DecisionUrgency) {
