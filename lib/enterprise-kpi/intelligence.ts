...
import {
  EIF_FIXTURE_OBSERVATIONS,
  FIXTURE_OBSERVATION_TIMESTAMP,
...
export const PROVISIONAL_DOMAIN_WEIGHTS: Record<KpiDomain, number> = {
  PLATFORM: 0.22,
  CUSTOMER: 0.16,
  OPERATIONS: 0.2,
  BUSINESS: 0.14,
...
export const EIF_DEMO_OBSERVATION_SERIES: Record<string, KpiObservation[]> = {
  "KPI-PLAT-001": [
...
function fixture(kpiId: string, value: number, observedAt: string, note: string): KpiObservation {
  return {
    kpiId,
    value,
    observedAt,
...
export function assessFreshness(
  ...
  assessedAt = new Date(FIXTURE_OBSERVATION_TIMESTAMP),
): FreshnessAssessment {
  ...
  const ageHours = (assessedAt.getTime() - observedAt.getTime()) / 3_600_000;
  const state: IntelligenceFreshnessState =
    ageHours <= expectationHours * 0.5 ? "FRESH" : ageHours <= expectationHours ? "AGING" : "STALE";
...
export function assessConfidence(input: {
  ...
  if (input.observationCount < 2) score -= 0.35;
  else if (input.observationCount < 3) score -= 0.15;
  if (input.provenance === "NON_PRODUCTION_FIXTURE") score -= 0.2;
  if (input.freshness === "STALE") score -= 0.25;
  if (input.freshness === "UNKNOWN") score -= 0.2;
  if (input.coveragePercentage < 50) score -= 0.25;
  score = Math.max(0, Math.min(1, score));
  const level =
    score >= 0.8 ? "HIGH" : score >= 0.55 ? "MEDIUM" : score >= 0.25 ? "LOW" : "INSUFFICIENT";
  return {
    level,
    score,
    factors: [
...
function normalizeEvaluation(evaluation: KpiEvaluation): number | null {
  return scoreForStatus(evaluation.status);
}

function weightedAverage(items: { score: number; weight: number }[]) {
...
export function calculateDomainHealth(
  ...
  const latestObservation = eligible
    ...
    .filter((observation): observation is KpiObservation => observation !== null)
...
export function buildIntelligenceHealthSnapshot(
  observations: KpiObservation[] = EIF_FIXTURE_OBSERVATIONS,
  now = new Date(FIXTURE_OBSERVATION_TIMESTAMP),
  domainWeights: Record<KpiDomain, number> = PROVISIONAL_DOMAIN_WEIGHTS,
): EnterpriseHealthSnapshot {
...
export function analyzeKpiTrend(
  kpiId: string,
  observations: KpiObservation[] = EIF_DEMO_OBSERVATION_SERIES[kpiId] ?? [],
  now = new Date(FIXTURE_OBSERVATION_TIMESTAMP),
): KpiTrend {
  ...
  const values = ordered.map((item) => item.value).filter((value): value is number => value !== null);
  if (values.length < 2) return trendResult(kpiId, "INSUFFICIENT_DATA", "UNKNOWN", ordered, freshness, evidence);
  ...
  const improves =
    definition.desiredTrend === "HIGHER_IS_BETTER" ? absoluteChange > 0 : absoluteChange < 0;
  const declines =
    definition.desiredTrend === "HIGHER_IS_BETTER" ? absoluteChange < 0 : absoluteChange > 0;
  const trend: KpiTrendDirection =
    rawDirection === "MIXED" ? "VOLATILE" : absoluteChange === 0 ? "STABLE" : improves ? "IMPROVING" : declines ? "DECLINING" : "UNKNOWN";
...
function trendResult(
  ...
  rawDirection: KpiTrend["rawDirection"],
  ordered: KpiObservation[],
...
export function getKpiTrends(): KpiTrend[] {
  return ENTERPRISE_KPI_REGISTRY.map((definition) => analyzeKpiTrend(definition.id));
}

export function detectKpiTransitions(): KpiTransition[] {
  const transitions: KpiTransition[] = [];
  ...
  for (const [kpiId, observations] of Object.entries(EIF_DEMO_OBSERVATION_SERIES)) {
    ...
    for (let index = 1; index < ordered.length; index += 1) {
      ...
      if (previous.status === current.status && previous.freshness === current.freshness) continue;
...
export function detectRiskSignals(): RiskSignal[] {
  ...
  for (const trend of trends.filter((item) => item.trend === "DECLINING")) {
    ...
    if (kpi) signals.push(risk(`RISK-${trend.kpiId}-DECLINING`, "KPI declines across governed trend window", "MEDIUM", [kpi.domain], trend.evidence));
  }
  for (const transition of transitions.filter((item) => item.currentStatus === "CRITICAL")) {
    const kpi = getEnterpriseKpi(transition.kpiId);
    if (kpi) signals.push(risk(`RISK-${transition.kpiId}-ENTERED-CRITICAL`, "KPI enters CRITICAL", "HIGH", [kpi.domain], transition.evidence));
  }
  ...
  for (const trend of trends.filter((item) => item.freshness.state === "STALE")) {
    const kpi = getEnterpriseKpi(trend.kpiId);
    if (kpi) signals.push(risk(`RISK-${trend.kpiId}-STALE`, "Required data becomes stale", "MEDIUM", [kpi.domain], trend.evidence));
  }
...
function risk(signalId: string, condition: string, severity: RiskSignal["severity"], affectedDomains: KpiDomain[], evidence: EvidenceReference[]): RiskSignal {
  return {
    signalId,
    condition,
    severity,
...
export function detectOpportunitySignals(): OpportunitySignal[] {
  const trends = getKpiTrends();
  const transitions = detectKpiTransitions();
  const signals: OpportunitySignal[] = [];
  for (const trend of trends.filter((item) => item.trend === "IMPROVING")) {
    const kpi = getEnterpriseKpi(trend.kpiId);
    if (kpi) signals.push(opportunity(`OPP-${trend.kpiId}-IMPROVING`, "KPI improves consistently", kpi.domain, "Improving KPI behavior may support carefully scoped preview confidence.", trend.evidence));
  }
  for (const transition of transitions.filter((item) => item.previousStatus === "WARNING" && item.currentStatus === "HEALTHY")) {
    const kpi = getEnterpriseKpi(transition.kpiId);
    if (kpi) signals.push(opportunity(`OPP-${transition.kpiId}-RETURNED-HEALTHY`, "A formerly warning KPI returns to healthy", kpi.domain, "Recovery may indicate a stabilizing preview capability for human review.", transition.evidence));
  }
...
function opportunity(signalId: string, condition: string, relevantDomain: KpiDomain, value: string, evidence: EvidenceReference[]): OpportunitySignal {
  return {
    signalId,
    condition,
    confidence: assessConfidence({
...
export function buildIntelligenceEvents(): IntelligenceEvent[] {
  ...
  for (const signal of risks) {
    events.push(event(`EVT-${signal.signalId}`, "ENTERPRISE_RISK", signal.condition, signal.potentialConsequence, signal.severity, signal.affectedDomains[0] ?? "OPERATIONS", signal.evidence.map((item) => item.kpiId).filter((item): item is string => Boolean(item)), signal.evidence, "RULE-RISK-SIGNAL", detectedAt));
  }
  for (const signal of opportunities) {
    events.push(event(`EVT-${signal.signalId}`, "ENTERPRISE_OPPORTUNITY", signal.condition, signal.potentialEnterpriseValue, "LOW", signal.relevantDomain, signal.evidence.map((item) => item.kpiId).filter((item): item is string => Boolean(item)), signal.evidence, "RULE-OPPORTUNITY-SIGNAL", detectedAt));
  }
  return events;
}
...
function event(
  eventId: string,
  eventClass: IntelligenceEvent["eventClass"],
  title: string,
  summary: string,
