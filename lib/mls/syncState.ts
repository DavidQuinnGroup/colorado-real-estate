...
export type MlsSyncStateUpdate = {
  isSyncing?: boolean;
...
function normalizeUpdate(values: MlsSyncStateUpdate) {
...
export async function updateMlsSyncState(values: MlsSyncStateUpdate) {
...
export async function updateLastSync(date: string | Date) {
  return updateMlsSyncState({
    lastSync: date,
  });
}

export async function markMlsSyncStarted(values: MlsSyncStateUpdate = {}) {
  return updateMlsSyncState({
    ...values,
    isSyncing: true,
...
export async function markMlsSyncFinished(values: MlsSyncStateUpdate = {}) {
  return updateMlsSyncState({
    ...values,
    isSyncing: false,
    lastSync: values.lastSync ?? new Date(),
