...
export type MlsPageJobSource = 'api' | 'coordinator' | 'script' | 'system';

export type MlsPageJobData = {
  requestedAt?: string;
  requestedBy?: string;
  source?: MlsPageJobSource;
  skip: number;
  top: number;
  lastSync: string;
...
export const MLS_PAGE_DEFAULT_TIMEOUT_MS = 30_000;
...
export const mlsPageQueue = new Queue<MlsPageJobData>(MLS_PAGE_QUEUE_NAME, {
  connection,
  defaultJobOptions,
});
...
function getSafeLastSync(value: string | undefined) {
  if (!value) return MLS_PAGE_DEFAULT_LAST_SYNC;

  const parsed = new Date(value);
  if (Number.isNaN(parsed.getTime())) return MLS_PAGE_DEFAULT_LAST_SYNC;
...
export function normalizeMlsPageJobData(data: Partial<MlsPageJobData>): MlsPageJobData {
  return {
...
function getJobId(data: MlsPageJobData) {
