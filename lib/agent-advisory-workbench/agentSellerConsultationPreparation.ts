...
export function prepareAgentSellerConsultation(request: AgentSellerPreparationRequest): AgentSellerConsultationExperience {
  ...
  if (packet.admission !== "ADMITTED") {
    ...
    return Object.freeze({ status: AGENT_SELLER_CONSULTATION_PREPARATION_STATUS, packet, composition: null, playbook: null, humanState: { label: (needsChoices ? "Complete the consultation choices" : "Seller preparation unavailable") as "Complete the consultation choices" | "Seller preparation unavailable", message: needsChoices ? "Choose a consultation position and two to five Seller topics before preparing the briefing." : "This preparation is limited to explicit Agent inputs and governed Seller consultation context." } });
  }
