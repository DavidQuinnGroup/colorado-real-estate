...
export const BOULDER_COUNTY_RECORDER_INDEX_SOURCE_QUALITY_EVIDENCE_REVIEWED_AT = '2026-08-16' as const;
export const BOULDER_COUNTY_RECORDER_INDEX_MANIFEST_ELIGIBILITY = 'READY_WITH_KNOWN_GAPS' as const;

export const BOULDER_COUNTY_RECORDER_INDEX_SOURCE_QUALITY_CERTIFICATION: SourceEvidenceCertificationReference = Object.freeze({
  certificationId: 'CERT-BOULDER-COUNTY-RECORDER-INDEX-SOURCE-QUALITY-EVIDENCE-001',
...
export const BOULDER_COUNTY_RECORDER_INDEX_SOURCE_QUALITY_CONVERSION_REQUEST = Object.freeze<CountySourceQualityEvidenceConversionRequest>({
  ...
  evidenceReferences: [{
    ...
    evidenceReferenceId: 'SQE-BOULDER-COUNTY-RECORDER-INDEX-CERT-001',
    posture: 'REFERENCED',
...
export const BOULDER_COUNTY_RECORDER_INDEX_SOURCE_QUALITY_FIREWALL = Object.freeze({
  ...
  indexBoundary: 'INDEX_OR_SEARCH_METADATA_NOT_DOCUMENT_CONTENT',
  rawData: 'RAW_RECORDER_PROPERTY_PERSON_DATA_NOT_ACCEPTED_BY_EVIDENCE_PACKAGE',
});

export function convertBoulderCountyRecorderIndexSourceQualityEvidence() {
  return convertCountyStructuredEvidence(BOULDER_COUNTY_RECORDER_INDEX_SOURCE_QUALITY_CONVERSION_REQUEST);
}

export function normalizeBoulderCountyRecorderIndexSourceQualityEvidence() {
  const conversion = convertBoulderCountyRecorderIndexSourceQualityEvidence();
  return normalizeSourceEvidence({ sourceId: BOULDER_COUNTY_RECORDER_INDEX_SOURCE_ID, linkages: conversion.linkages });
}

export function summarizeBoulderCountyRecorderIndexSourceQualityEvidence() {
  return summarizeSourceQuality(normalizeBoulderCountyRecorderIndexSourceQualityEvidence());
}

export function createBoulderCountyRecorderIndexSourceQualityAssemblyRequest(): SourceQualitySummaryAssemblyRequest {
  const request = createCountySourceQualityAssemblyRequest(convertBoulderCountyRecorderIndexSourceQualityEvidence());
  if (!request) throw new Error('Exact Recorder index conversion must produce a sparse canonical assembly request.');
  return request;
}
