...
export const INTERNAL_PREVIEW_CALCULATION_VERSION =
  "EIA-1.0-internal-preview-adapter-v1";
export const INTERNAL_PREVIEW_SOURCE_VERSION =
  "PROJECT-ATLAS-RC1-internal-preview-certification-v1";
...
type InternalPreviewObservationPlan = EnterpriseAdapterObservationPlan & {
  kpiId: InternalPreviewKpiId;
};
...
type CertificationRecord = {
  ...
  certification?: {
    decision?: string;
    certifiedAt?: string;
    certifiedRuntimeCommit?: string;
    scope?: string;
    closedIssues?: string[];
    explicitExclusions?: string[];
    operatingConditions?: string[];
    knownWatchItems?: string[];
  };
};

type InternalPreviewSourceState = {
  adapterId: typeof INTERNAL_PREVIEW_ADAPTER_ID;
  adapterVersion: typeof INTERNAL_PREVIEW_ADAPTER_VERSION;
  sourceVersion: typeof INTERNAL_PREVIEW_SOURCE_VERSION;
  certificationId: typeof CERTIFICATION_ID;
  ...
  operationalIssueSummary: {
    openCriticalOrHighIssues: number;
    closedIssueCount: number;
    knownWatchItemCount: number;
    watchItemsNonBlocking: boolean;
  };
  unavailableSources: Array<{
    kpiId: Exclude<InternalPreviewKpiId, "KPI-OPS-001">;
    source: string;
    reason: string;
  }>;
  sourceRecords: string[];
  certificationRecordHash: string;
};

type InternalPreviewSourceSummary = {
  ...
  knownWatchItemCount: number;
  openCriticalOrHighIssues: number;
  unavailableKpis: string[];
};

export type InternalPreviewAdapterResult = EnterpriseAdapterResult & {
  adapter: {
    id: typeof INTERNAL_PREVIEW_ADAPTER_ID;
    name: typeof INTERNAL_PREVIEW_ADAPTER_NAME;
    version: typeof INTERNAL_PREVIEW_ADAPTER_VERSION;
  ...
  sourceSummary?: InternalPreviewSourceSummary;
  observations: InternalPreviewObservationPlan[];
};

const unavailablePreviewSources: InternalPreviewSourceState["unavailableSources"] = [
  {
    kpiId: "KPI-CUST-001",
...
const unsupportedInternalPreviewMetrics: EnterpriseAdapterUnsupportedMetric[] = [
  {
...
async function readCertificationRecord(): Promise<CertificationRecord> {
  const raw = await readFile(repoPath(RELEASE_CANDIDATE_BOARD_PATH), "utf8");
  const parsed = JSON.parse(raw) as { issues?: CertificationRecord[] };
...
async function readInternalPreviewSourceState(): Promise<InternalPreviewSourceState> {
...
function sourceEffectiveAt(source: InternalPreviewSourceState): Date {
  const timestamp = new Date(source.certificationTimestamp);
  return Number.isFinite(timestamp.getTime()) ? timestamp : new Date(0);
}

function sourceQueryRefFor(source: InternalPreviewSourceState): string {
  return [
...
export function fingerprintInternalPreviewSourceState(
  state: InternalPreviewSourceState,
): string {
...
function confidenceForOperationalIssues(source: InternalPreviewSourceState): EIAConfidence {
  if (source.certificationStatus !== "CLOSED") return "LOW";
  if (source.certificationDecision !== "CERTIFIED_FOR_INTERNAL_PREVIEW") return "LOW";
  if (source.closedIssues.length < 8) return "MEDIUM";
  if (!source.operationalIssueSummary.watchItemsNonBlocking) return "MEDIUM";
  return "HIGH";
}

function unavailablePlan(
  source: InternalPreviewSourceState,
  sourceInfo: InternalPreviewSourceState["unavailableSources"][number],
  freshness: EIAFreshness,
): InternalPreviewObservationPlan {
  const definitions: Record<Exclude<InternalPreviewKpiId, "KPI-OPS-001">, {
    displayName: string;
    unit: InternalPreviewObservationPlan["unit"];
    formula: string;
  }> = {
    "KPI-CUST-001": {
...
function mapInternalPreviewObservations(
  source: InternalPreviewSourceState,
  freshness: EIAFreshness,
): InternalPreviewObservationPlan[] {
  return [
...
function summarizeInternalPreviewSourceState(source: InternalPreviewSourceState): InternalPreviewSourceSummary {
  return {
    certificationId: source.certificationId,
...
} satisfies EnterpriseAdapterLifecycleConfig<InternalPreviewSourceState>;

export async function invokeInternalPreviewAdapter(options: {
  execute: boolean;
  invocationId?: string;
  now?: Date;
}): Promise<InternalPreviewAdapterResult> {
  return invokeEnterpriseAdapter(internalPreviewAdapterConfig, options) as Promise<InternalPreviewAdapterResult>;
}
