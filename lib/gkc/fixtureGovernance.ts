...
export type GkcKnowledgeClassification = (typeof GKC_KNOWLEDGE_CLASSIFICATIONS)[number];
...
export type GkcFixtureObjectType = (typeof GKC_FIXTURE_OBJECT_TYPES)[number];

export const GKC_CONFIDENCE_LEVELS = ["INSUFFICIENT", "LOW", "MEDIUM", "HIGH"] as const;
export type GkcConfidence = (typeof GKC_CONFIDENCE_LEVELS)[number];

export const GKC_VALUE_KINDS = ["TEXT", "NUMBER", "BOOLEAN", "JSON", "DATE"] as const;
export type GkcValueKind = (typeof GKC_VALUE_KINDS)[number];

export const GKC_FRESHNESS_STATES = ["FRESH", "AGING", "STALE", "UNKNOWN", "NOT_APPLICABLE"] as const;
export type GkcFreshness = (typeof GKC_FRESHNESS_STATES)[number];

export const GKC_REVIEW_STATES = ["PENDING_REVIEW", "REVIEWED", "CONFLICTED", "REJECTED"] as const;
export type GkcReviewState = (typeof GKC_REVIEW_STATES)[number];

export type GkcEligibility = {
  internalUse: boolean;
...
export const GKC_SAFE_ELIGIBILITY_DEFAULTS: GkcEligibility = {
  internalUse: false,
...
export type GkcSourceFixture = {
  id: string;
  synthetic: true;
  sourceClass: GkcSourceClass;
  authorityLevel: "PRIMARY" | "SUPPLEMENTAL" | "EDITORIAL" | "UNVERIFIED";
  licensingRestricted: boolean;
  publicDisplayRestricted: boolean;
  health: "PROPOSED" | "ACTIVE" | "DEGRADED" | "RESTRICTED" | "RETIRED";
  cadence: "REAL_TIME" | "DAILY" | "MONTHLY" | "ANNUAL" | "EVENT_DRIVEN" | "UNKNOWN";
};

export type GkcFixtureObject = {
  id: string;
  synthetic: true;
  objectType: GkcFixtureObjectType;
  canonicalName: string;
  canonicalSlug: string;
};

export type GkcSchemaKeyDefinition = {
  key: string;
  domain: string;
  valueKind: GkcValueKind;
  applicableObjectTypes: readonly GkcFixtureObjectType[];
  unit?: "USD" | "COUNT" | "DAYS" | "NONE";
  requiresSource: boolean;
  requiresEffectiveDate: boolean;
  freshnessPolicy: GkcFreshness;
  confidenceFloor: GkcConfidence;
...
export type GkcObservationFixture = {
  id: string;
  synthetic: true;
  objectId: string;
  objectType: GkcFixtureObjectType;
  ...
  freshness: GkcFreshness;
  confidence: GkcConfidence;
  reviewStatus: GkcReviewState;
  conflictGroupKey?: string;
  eligibility: GkcEligibility;
};
...
export type GkcValidationResult = {
  ok: boolean;
  checks: string[];
};

const classificationSet = new Set<string>(GKC_KNOWLEDGE_CLASSIFICATIONS);
const sourceClassSet = new Set<string>(GKC_SOURCE_CLASSES);
const objectTypeSet = new Set<string>(GKC_FIXTURE_OBJECT_TYPES);

const confidenceRank: Record<GkcConfidence, number> = {
  INSUFFICIENT: 0,
  LOW: 1,
  MEDIUM: 2,
  HIGH: 3,
...
export const GKC_SOURCE_FIXTURES: readonly GkcSourceFixture[] = [
  {
...
const permittedSourceClassifications: Record<GkcSourceClass, readonly GkcKnowledgeClassification[]> = {
...
export function assertGkcKnowledgeClassification(value: string): asserts value is GkcKnowledgeClassification {
  if (!classificationSet.has(value)) {
    throw new Error(`Unknown GKC knowledge classification: ${value}`);
  }
}

export function assertGkcSourceClass(value: string): asserts value is GkcSourceClass {
  if (!sourceClassSet.has(value)) {
    throw new Error(`Unknown GKC source class: ${value}`);
  }
}

export function assertGkcFixtureObjectType(value: string): asserts value is GkcFixtureObjectType {
  if (!objectTypeSet.has(value)) {
    throw new Error(`Unauthorized synthetic GKC fixture object type: ${value}`);
  }
}

export function normalizeGkcAliasValue(value: string): string {
  return value
    .normalize("NFKD")
    .replace(/[‘’]/g, "'")
    .replace(/[“”]/g, '"')
...
export function normalizeGkcZipAlias(value: string): string {
...
export function buildGkcObjectDedupeKey(object: Pick<GkcFixtureObject, "objectType" | "canonicalSlug">): string {
...
export function buildGkcAliasDedupeKey(alias: GkcAliasFixture): string {
...
export function buildGkcObservationDedupeKey(observation: GkcObservationFixture): string {
  return [
    "GKC_OBSERVATION",
...
export function detectGkcAliasCollisions(aliases: readonly GkcAliasFixture[]): Map<string, string[]> {
...
export function validateGkcSourceClassification(source: GkcSourceFixture, classification: GkcKnowledgeClassification): void {
...
export function validateGkcSchemaKeyObservation(observation: GkcObservationFixture): GkcSchemaKeyDefinition {
...
export function validateGkcSourceRequirement(observation: GkcObservationFixture): void {
...
export function canActivateGkcEligibility(observation: GkcObservationFixture, requested: Partial<GkcEligibility>): boolean {
...
export function resolveGkcConflictGroup(observations: readonly GkcObservationFixture[], preferredObservationId: string): readonly GkcObservationFixture[] {
...
export function validateNoPublicEligibilityDefaults(observations: readonly GkcObservationFixture[]): void {
...
export function assertSyntheticFixtureId(id: string): void {
  if (!id.startsWith("synthetic-")) {
    throw new Error(`Fixture id must be synthetic-prefixed: ${id}`);
  }
}

export function createGkcRepresentativeObservations(): readonly GkcObservationFixture[] {
  return [
    {
...
export function runGkcFixtureGovernanceValidation(): GkcValidationResult {
  const checks: string[] = [];
