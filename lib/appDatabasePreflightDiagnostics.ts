...
export type AppDatabasePreflightDiagnostics = {
...
export function getAppDatabasePreflightErrorMessage(error: unknown) {
  if (error instanceof Error) return error.message.replace(/\s+/g, ' ').trim();
  return String(error || 'Unknown database preflight failure.');
}

export function shouldSkipAppDatabasePreflight(env: Record<string, string | undefined> = process.env) {
...
export function getAppDatabasePreflightFailureMessage(context: AppDatabasePreflightContext, error: unknown) {
  return [
...
export function getAppDatabasePreflightDiagnostics(): AppDatabasePreflightDiagnostics {
  const context = {
