...
export const EIF_FIXTURE_OBSERVATIONS = [
    {
        kpiId: "KPI-PLAT-001",
        value: 100,
...
export function buildEnterpriseHealthSnapshot(observations = EIF_FIXTURE_OBSERVATIONS, now = new Date(FIXTURE_OBSERVATION_TIMESTAMP)) {
    const byKpi = new Map(observations.map((observation) => [observation.kpiId, observation]));
    const evaluations = ENTERPRISE_KPI_REGISTRY.map((definition) => evaluateKpi(definition, byKpi.get(definition.id) ?? null, now));
...
export function getLatestKpiEvaluations() {
