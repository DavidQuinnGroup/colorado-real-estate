...
import { INTELLIGENCE_CALCULATION_VERSION, PROVISIONAL_DOMAIN_WEIGHTS, buildIntelligenceEvents, buildIntelligenceHealthSnapshot, detectKpiTransitions, detectOpportunitySignals, detectRiskSignals, getKpiTrends, } from "./intelligence.js";
...
export function buildExecutiveCommandCenterPayload() {
...
export function buildDailyExecutiveBrief() {
    ...
    const sections = [
        section("Enterprise Status", `Overall enterprise health is ${formatScore(commandCenter.enterpriseStatus.overallScore)} with ${commandCenter.enterpriseStatus.overallStatus} status. Current output is ${commandCenter.enterpriseStatus.provenance}, confidence is ${commandCenter.enterpriseStatus.confidence}, freshness is ${commandCenter.enterpriseStatus.freshness}, and coverage is ${commandCenter.enterpriseStatus.coveragePercentage}%.`, [], commandCenter.evidenceReferences.slice(0, 3)),
        section("Material Changes", summarizeChanges(commandCenter.materialChanges), commandCenter.materialChanges.map((item) => item.intelligenceEventId).filter((item) => Boolean(item)), commandCenter.materialChanges.flatMap((item) => item.evidence).slice(0, 6)),
        ...
        section("Top Risk", topRisk
            ...
            : "No active risk signal is available from the governed fixture set.", topRisk ? eventIdsForEvidence(commandCenter.materialChanges, topRisk.evidence) : [], topRisk?.evidence ?? []),
        section("Top Opportunity", topOpportunity
            ...
            : "No active opportunity signal is available from the governed fixture set.", topOpportunity ? eventIdsForEvidence(commandCenter.materialChanges, topOpportunity.evidence) : [], topOpportunity?.evidence ?? []),
        section("Executive Attention Required", firstAttention
            ? `${firstAttention.title} is priority ${firstAttention.priority}. Suggested review action: ${firstAttention.suggestedReviewAction}.`
            : "No executive attention item is currently generated by the deterministic fixture rules.", firstAttention?.supportingIntelligenceEventId ? [firstAttention.supportingIntelligenceEventId] : [], firstAttention?.evidence ?? []),
        section("Data Confidence and Limitations", `The workspace is ${commandCenter.dataIntegrity.overallProvenance}. Unknown KPIs: ${commandCenter.dataIntegrity.unknownKpiCount}. Stale KPIs: ${commandCenter.dataIntegrity.staleKpiCount}. Defined-but-unavailable KPIs: ${commandCenter.dataIntegrity.definedButUnavailableKpiCount}. Persistence is intentionally unavailable in Sprint 3.`, [], []),
    ];
    return {
        metadata: {
            title: "PROJECT ATLAS",
...
function buildDomainSummaries(health, events) {
    return health.domainResults.map((domain) => {
        ...
        const primaryPositiveContributor = [...included].sort((left, right) => (right.weightedScore ?? -1) - (left.weightedScore ?? -1))[0] ?? null;
        const primaryNegativeContributor = [...included].sort((left, right) => (left.weightedScore ?? 101) - (right.weightedScore ?? 101))[0] ?? null;
...
function materialChange(input) {
    const confidence = input.event?.confidence ??
    ...
    return {
        ...
        drillDown: buildDrillDown({
            ...
            event: input.event,
            detectionRuleId: input.event?.detectionRuleId ?? input.detectionRuleId,
            kpiIds: input.evidence.map((item) => item.kpiId).filter((item) => Boolean(item)),
            evidence: input.evidence,
            health: input.health,
        }),
    };
...
function buildExecutiveAttentionItems(input) {
    ...
    for (const risk of input.risks) {
        ...
        const priority = risk.severity === "CRITICAL" ? "CRITICAL" : risk.severity === "HIGH" ? "HIGH" : "MEDIUM";
        items.push({
    ...
    for (const transition of input.transitions.filter((item) => STATUS_RANK[item.currentStatus] > STATUS_RANK[item.previousStatus])) {
        ...
        if (!kpi)
            continue;
        const event = input.eventBySource.get(transition.transitionId) ?? null;
        const priority = transition.currentStatus === "CRITICAL" ? "HIGH" : "MEDIUM";
        items.push({
...
function buildDataIntegritySummary(health, events, risks, opportunities) {
...
function buildEventSourceMap(events) {
    const map = new Map();
...
function materialRankScore(input) {
    return (SEVERITY_RANK[input.severity] * 100 +
...
function sortOpportunities(opportunities) {
    return [...opportunities].sort((left, right) => CONFIDENCE_RANK[right.confidence.level] - CONFIDENCE_RANK[left.confidence.level] ||
        right.evidence.length - left.evidence.length ||
        left.signalId.localeCompare(right.signalId));
}
function freshnessFromEvidence(evidence) {
    ...
    const ageHours = Number.isNaN(observedAt.getTime())
        ? null
        : (assessedAt.getTime() - observedAt.getTime()) / 3600000;
    const state = ageHours === null ? "UNKNOWN" : ageHours <= 24 ? "FRESH" : ageHours <= 48 ? "AGING" : "STALE";
    return { state, ageHours, expectationHours: 24, assessedAt: FIXTURE_OBSERVATION_TIMESTAMP };
}
function emptyFreshness(state = "UNKNOWN") {
    return { state, ageHours: null, expectationHours: null, assessedAt: FIXTURE_OBSERVATION_TIMESTAMP };
}
function confidenceFromEvidence(evidence, fallback) {
...
function section(heading, body, supportingIntelligenceEventIds, evidence) {
    return {
        heading,
        body,
...
function summarizeChanges(changes) {
    if (changes.length === 0)
        return "No material change is available from the governed fixture set.";
    return changes
        .slice(0, 3)
        .map((item) => `${item.title} (${item.severity}, ${item.domain})`)
        .join("; ");
}
function summarizeDomain(changes, label) {
    if (changes.length === 0)
        return `${label} has no material change in the governed fixture set. Unknown or unavailable data remains visible elsewhere in the workspace.`;
    return changes
        .slice(0, 2)
        .map((item) => `${item.title} requires ${item.severity === "LOW" ? "monitoring" : "review"}.`)
        .join(" ");
}
function ids(changes) {
    return changes.map((item) => item.intelligenceEventId).filter((item) => Boolean(item));
}
function evidence(changes) {
    return changes.flatMap((item) => item.evidence).slice(0, 5);
}
function eventIdsForEvidence(changes, evidenceReferences) {
