...
export function normalizeUnsubscribeToken(value) {
...
export function classifyUnsubscribeToken(token, record) {
    if (!token) {
        return {
            ...
            statusCode: 400,
            title: 'Invalid Link',
            message: 'This unsubscribe link is missing or incomplete. Please contact David Quinn Group if you need help adjusting email preferences.',
        };
    }
    if (!record) {
        return {
            ...
            message: 'This unsubscribe link is no longer available. Please contact David Quinn Group if you need help adjusting email preferences.',
        };
    }
    if (record.usedAt) {
        return {
