...
function toBoundedScore(value, fallback = 0) {
...
function getCreatedAt(listing) {
    return toEpochMilliseconds(firstValue(listing.createdAt, listing.CreatedAt, listing.ListingContractDate, listing.OriginalEntryTimestamp, listing.OnMarketDate, listing.inputDate, listing.InputDate));
}
function getUpdatedAt(listing, createdAt) {
    return (toEpochMilliseconds(firstValue(listing.updatedAt, listing.UpdatedAt, listing.ModificationTimestamp, listing.ListingModificationTimestamp, listing.MajorChangeTimestamp, listing.StatusChangeTimestamp, listing.PriceChangeTimestamp, listing.CloseDate)) ??
        createdAt ??
        Date.now());
}
function getGcForensics(listing) {
    const value = firstValue(listing.gcForensics, listing.GCForensics, listing.forensics);
    return isRecord(value) ? value : {};
}
function getBathCount(listing) {
    const explicitBaths = toNumber(firstValue(listing.baths, listing.BathroomsTotalInteger, listing.BathroomsTotalDecimal, listing.BathroomsTotal, listing.BathsTotal));
    if (explicitBaths !== undefined)
...
function getAddress(listing) {
    const streetParts = [
        ...
        .filter(Boolean);
    return toCleanString(firstValue(listing.address, listing.UnparsedAddress, listing.FullAddress, listing.StreetAddress, streetParts.length ? streetParts.join(' ') : undefined), 'Address unavailable');
}
function getMlsId(listing) {
    return toOptionalString(firstValue(listing.mlsId, listing.mls_id, listing.ListingKey, listing.ListingId, listing.MlsId, listing.MLSNumber, listing.ListingNumber, listing.Id, listing.mlsid));
}
function getLatLng(listing) {
    const lat = toNumber(firstValue(listing.lat, listing.latitude, listing.Latitude, getNestedValue(listing.PropertyLocation, 'Latitude'), getNestedValue(listing.Coordinates, 'Latitude')));
    const lng = toNumber(firstValue(listing.lng, listing.longitude, listing.Longitude, getNestedValue(listing.PropertyLocation, 'Longitude'), getNestedValue(listing.Coordinates, 'Longitude')));
    const isValidLocation = lat !== undefined &&
        lng !== undefined &&
        Math.abs(lat) <= 90 &&
        Math.abs(lng) <= 180 &&
        !(lat === 0 && lng === 0);
    return {
        ...
        location: isValidLocation ? [lat, lng] : undefined,
    };
}
function getNeighborhood(listing) {
    return toCleanString(firstValue(listing.neighborhood, listing.Neighborhood, listing.MLSAreaMajor, listing.Area, listing.SubdivisionName), 'Unassigned');
}
function getListingAgent(listing) {
    ...
    if (fullName)
        return toOptionalString(fullName);
...
function withoutUndefined(document) {
    return Object.fromEntries(Object.entries(document).filter(([, value]) => value !== undefined));
}
function assertRequiredSearchFields(document) {
...
function getCollectionResult(results, collectionName) {
    const index = TARGET_COLLECTIONS.indexOf(collectionName);
    return results[index]?.status === 'fulfilled';
}
function getFailedCollectionMessages(results, documentId) {
...
export function toListingDocument(listing) {
    ...
    const document = withoutUndefined({
        ...
        propertyType: toOptionalString(firstValue(listing.propertyType, listing.property_type, listing.PropertyType, listing.PropertySubType)),
        ...
        schoolDistrict: toOptionalString(firstValue(listing.schoolDistrict, listing.HighSchoolDistrict, listing.SchoolDistrict, listing.ElementarySchoolDistrict)),
...
export async function indexListing(listing) {
    ...
    const results = await Promise.allSettled(TARGET_COLLECTIONS.map((collectionName) => typesense.collections(collectionName).documents().upsert(document)));
