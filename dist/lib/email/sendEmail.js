...
function getReviewSignal(listing) {
    if (listing.hasPolybutyleneRisk)
        return 'Plumbing Review';
...
function getListingPathIdentity(listing) {
...
function getTrackedListingUrl(listing, options) {
    if (!options.userId || !listing.id)
        return listing.url;
...
function renderListingImage(listing, url) {
    if (!listing.image)
        return '';
    return `
...
function renderDigestHtml(listings, options) {
...
function renderDigestText(listings, options) {
    const lines = [
        'David Quinn Group',
        'Real Estate Intelligence Digest',
        '',
...
function getSubject(listingCount) {
...
export async function sendEmail(to, listings, options = {}) {
