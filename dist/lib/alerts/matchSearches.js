...
function hasCompleteBounds(search) {
    return search.north !== null && search.south !== null && search.east !== null && search.west !== null;
}
function matchesBounds(search, property) {
    if (!hasCompleteBounds(search))
        return true;
...
function matchesSearch(search, property) {
    ...
    if (!sameValue(search.city, property.city))
        return false;
    if (search.minPrice !== null && propertyPrice < search.minPrice)
        return false;
    if (search.beds !== null && propertyBeds < search.beds)
        return false;
    if (search.type && !sameValue(search.type, property.propertyType))
        return false;
...
function buildAlertPayload(property) {
...
async function fetchCandidateSearches(property) {
...
async function queueAlert(search, property, payload) {
    try {
...
export async function matchAndNotify(property) {
    ...
    for (const search of candidateSearches) {
        if (!matchesSearch(search, property))
            continue;
