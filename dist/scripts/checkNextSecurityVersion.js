...
const declaredVersion = packageJson.dependencies?.next ?? packageJson.devDependencies?.next ?? '';
if (!declaredVersion) {
...
for (const [label, version] of [
    ...
    if (version.major === 15 && version.minor === 1 && compareVersions(version, floor) < 0) {
        fail(`${label} Next.js version is below the approved 15.1 security floor: ${minimumPatchedVersion}.`);
    }
}
console.log(`[next-security-version] ok: declared ${declaredVersion}, installed ${installedPackage.version}, floor ${minimumPatchedVersion}.`);
