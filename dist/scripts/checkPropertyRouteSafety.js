...
async function fetchProperty(identity) {
    ...
    const { data, error } = await client
        ...
        .maybeSingle();
    if (error)
        throw new Error(`Supabase property fallback query failed: ${error.message}`);
    return data;
}
async function fetchPhotoCount(propertyId) {
    const client = getSupabaseClient();
    ...
    if (error)
        throw new Error(`Supabase property photo fallback query failed: ${error.message}`);
    return count ?? 0;
}
...
async function main() {
    ...
    console.log(`[property-route-safety] ok: selected property fallback resolved ${selected.address}, id fallback resolved, missing identity returned null, photos=${photoCount}.`);
}
main().catch((error) => {
