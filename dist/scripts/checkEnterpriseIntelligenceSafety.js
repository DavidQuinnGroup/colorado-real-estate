...
import { EIF_DEMO_OBSERVATION_SERIES, analyzeKpiTrend, assessConfidence, assessFreshness, buildIntelligenceEvents, buildIntelligenceHealthSnapshot, detectKpiTransitions, detectOpportunitySignals, detectRiskSignals, evaluateKpi, getEnterpriseKpi, } from "../lib/enterprise-kpi/index.js";
function obs(kpiId, value, observedAt) {
    return {
        kpiId,
        value,
        observedAt,
...
function assertConfidenceFreshness() {
    assert.equal(assessConfidence({
        observationCount: 3,
        provenance: "NON_PRODUCTION_FIXTURE",
        freshness: "FRESH",
        coveragePercentage: 100,
    }).level, "MEDIUM");
    assert.equal(assessFreshness(EIF_DEMO_OBSERVATION_SERIES["KPI-GOV-001"][0], 24, new Date("2026-07-18T18:43:18Z")).state, "STALE");
}
...
assertApiContracts();
console.log("[enterprise-intelligence-safety] ok: health, trends, transitions, events, risk, opportunity, evidence, confidence, freshness, and API contracts passed.");
