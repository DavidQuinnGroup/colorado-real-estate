...
import { SEARCH_SCHEMA_FIELD_RULES, SEARCH_SCHEMA_FILTER_FIELD_NAMES, SEARCH_SCHEMA_QUERY_FIELD_NAMES, SEARCH_SCHEMA_SORT_FIELD_NAMES, client, formatSearchSchemaValidationError, searchSchemas, } from '../lib/typesense/schema.js';
...
function assertValidSearchSchema(schema) {
    const error = formatSearchSchemaValidationError(schema);
    if (error) {
        throw new Error(error);
    }
}
function assertValidRetrievedCollection(collection) {
...
async function runCheck() {
    ...
    if (failures.length) {
        throw new Error(`Typesense collection check failed for ${failures.map((inspection) => inspection.schemaName).join(', ')}. ${getRepairInstruction()}`);
    }
...
async function run(options) {
    console.log(`Creating canonical Typesense search collections through compatibility helper. check=${options.check}, dryRun=${options.dryRun}, reset=${options.reset}.`);
