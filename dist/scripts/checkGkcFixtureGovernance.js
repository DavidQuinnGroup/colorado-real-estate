...
import { GKC_FIXTURE_OBJECTS, GKC_KNOWLEDGE_CLASSIFICATIONS, GKC_KNOWLEDGE_LIFECYCLE_TRANSITIONS, GKC_OBJECT_LIFECYCLE_TRANSITIONS, GKC_SAFE_ELIGIBILITY_DEFAULTS, GKC_SCHEMA_KEY_REGISTRY, GKC_SOURCE_CLASSES, GKC_SOURCE_FIXTURES, GKC_SOURCE_LIFECYCLE_TRANSITIONS, buildGkcAliasDedupeKey, buildGkcObjectDedupeKey, buildGkcObservationDedupeKey, canActivateGkcEligibility, createGkcRepresentativeObservations, detectGkcAliasCollisions, normalizeGkcAliasValue, normalizeGkcZipAlias, resolveGkcConflictGroup, runGkcFixtureGovernanceValidation, validateGkcSchemaKeyObservation, validateGkcSourceClassification, validateGkcSourceRequirement, validateLifecycleTransition, } from "../lib/gkc/fixtureGovernance.js";
...
for (const classification of GKC_KNOWLEDGE_CLASSIFICATIONS) {
    ...
    const validObservation = {
        ...
        valueKind: classification === "EDITORIAL_KNOWLEDGE" ? "TEXT" : firstObservation.valueKind,
        ...
        confidence: classification === "EDITORIAL_KNOWLEDGE" ? "LOW" : firstObservation.confidence,
    };
    validateGkcSourceRequirement(validObservation);
}
assert.throws(() => validateGkcSourceRequirement({ ...firstObservation, classification: "UNKNOWN" }));
...
assert.equal(canActivateGkcEligibility({ ...firstObservation, sourceId: "synthetic-source-authoritative-government", classification: "AUTHORITATIVE_FACT" }, { publicPageEligible: true }), false);
...
for (const registryEntry of GKC_SCHEMA_KEY_REGISTRY) {
    ...
    validateGkcSchemaKeyObservation({
        ...
        value: registryEntry.valueKind === "NUMBER"
            ? 1
...
const aliases = [
    { objectId: "synthetic-gio-municipality-north-table", aliasText: "North Table", aliasType: "COMMON" },
    { objectId: "synthetic-gio-neighborhood-pine-bench", aliasText: " north   table ", aliasType: "COLLOQUIAL" },
    { objectId: "synthetic-gio-subdivision-copper-meadow", aliasText: "Copper Meadows", aliasType: "SUBDIVISION_VARIANT" },
    { objectId: "synthetic-gio-subdivision-copper-meadow", aliasText: "Copper Meadows", aliasType: "LEGACY", deprecated: true },
];
...
assert.equal(canActivateGkcEligibility({
    ...firstObservation,
...
assert.deepEqual(observations.map(buildGkcObservationDedupeKey), createGkcRepresentativeObservations().map(buildGkcObservationDedupeKey));
...
console.log("[gkc-fixture-governance] ok: synthetic fixtures, classification/source/schema-key/alias/lifecycle/source-requirement/conflict/eligibility/idempotency/runtime-isolation checks passed without database access or runtime activation.");
