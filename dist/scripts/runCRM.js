...
function getReadinessVerdict(summary) {
    if (!summary.success)
        return 'CRM task scan failed.';
    if (!summary.scanned)
        return summary.status === 'all' ? 'No CRM tasks are currently available.' : 'No CRM tasks matched this status.';
    if (summary.alertIncomplete > 0)
        return 'Some CRM tasks need stronger saved-search criteria before automated alert matching.';
    if (summary.alertReady > 0)
        return 'CRM tasks include alert-ready REIE intake context.';
    if (summary.alertWatch > 0)
        return 'CRM tasks include watch-level saved-search context.';
