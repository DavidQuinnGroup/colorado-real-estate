...
import { buildEnterpriseHealthSnapshot, determineFreshness, ENTERPRISE_KPI_REGISTRY, evaluateKpi, getEnterpriseKpi, listEnterpriseKpis, validateEnterpriseKpiRegistry, } from "../lib/enterprise-kpi/index.js";
function cloneDefinition(partial = {}) {
...
function observation(value, observedAt = "2026-07-18T18:43:18Z") {
    return {
        kpiId: "KPI-TEST-001",
        value,
        observedAt,
...
function assertUnknownAndFreshness() {
    ...
    assert.equal(determineFreshness(definition, observation(100, "2026-07-16T18:43:18Z"), new Date("2026-07-18T18:43:18Z")), "STALE");
    assert.equal(evaluateKpi(definition, observation(100, "2026-07-16T18:43:18Z"), new Date("2026-07-18T18:43:18Z")).status, "UNKNOWN");
    assert.equal(evaluateKpi(definition, {
        ...observation(null),
...
function assertHealth() {
    ...
    assert.equal(snapshot.domainResults.find((domain) => domain.domain === "CUSTOMER")?.status, "UNKNOWN");
...
function assertApiContracts() {
    ...
    const detailRoute = fs.readFileSync("app/api/admin/enterprise/kpis/[id]/route.ts", "utf8");
...
console.log("[enterprise-kpi-safety] ok: registry, thresholds, freshness, health, API auth, idempotency, and migration baseline passed.");
