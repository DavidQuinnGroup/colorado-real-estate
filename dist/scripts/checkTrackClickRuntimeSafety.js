...
function createSyntheticStores() {
    ...
    class SyntheticQuery {
        ...
        range(from, to) {
            ...
            const statuses = this.query.filters.status;
        ...
        executeUpdate() {
            if (this.query.table === 'AlertQueue') {
                const ids = this.query.filters.id;
    ...
    const supabaseClient = {
        from: (table) => new SyntheticQuery(table),
    };
    return {
        state,
        prismaClient,
...
async function assertSourceGuards() {
    ...
    assert(store.includes('No unclicked alert matched the tracked listing.'), 'Expected repeated synthetic requests to avoid duplicate enrichment when no unclicked alert remains.');
}
async function main() {
    ...
    const first = await trackClick(USER_ID, LISTING_ID, 'email_alert', DESTINATION, {
        ...
        supabaseClient: stores.supabaseClient,
