...
const generatedClientPath = join(process.cwd(), 'node_modules', '.prisma', 'client', 'index.d.ts');
...
if (!existsSync(generatedClientPath)) {
    fail(`Generated Prisma Client type file is missing at ${generatedClientPath}. Run "npx prisma generate".`);
}
const generatedTypes = readFileSync(generatedClientPath, 'utf8');
const missingModels = [];
const missingAccessors = [];
...
console.log(`[prisma-client-parity] ok: verified ${requiredModels.length} generated Prisma model accessors.`);
