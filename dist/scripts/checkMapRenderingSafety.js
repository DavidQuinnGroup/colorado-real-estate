...
async function main() {
    const [homePage, homeSearch, searchInterface, globalsCss, searchMap, selectedDrawer, packageJson, searchControls, mapSidebar, propertyCard, saveSearch, propertyPage, propertyInquiryForm, relatedPropertyLinks, equityVision,] = await Promise.all([
    ...
    for (const [label, source] of [
        ...
        assert(!source.match(/Grand Plan-powered|personalized results|best match|perfect home|dream home|exclusive listings|off-market|match score|fit score|AI-powered|automated recommendation|guaranteed outcome|permanent storage|cross-device synchronization/i), `${label} must not expose unsupported Guided Search Wave 1 continuity or claim language.`);
    }
    ...
    for (const [label, source] of [
        ...
        assert(!source.match(/Listing Prep ROI|ROI Engine|Equity Lift|Guaranteed return|Predictive value|Verified financial outcome|Optimized Value|Valuation Suite|Standard Portal Est\.|Investment Cost|equity capture/i), `${label} must not expose unsupported ROI, equity, or valuation outcome language.`);
    }
    assert(!propertyInquiryForm.match(/Selected Signal|Property Scorecard|Asset Snapshot|Authority Paths|Decision Signal|Triage|\bEFF\b|\bRES\b|\bEff\b|\bRes\b|\bDiagnostic\b|Scorecard|Operational status|Smoke ready|Smoke review|AI analysis|AI inspection|predictive|heatmap|guaranteed fit|guaranteed-fit|guaranteed condition|guaranteed equity|ROI|live traffic|commute savings|Discuss This Asset|Altitude Forensics|Internal intelligence/i), 'property inquiry form must not expose unsupported or operational Wave 2 property language.');
    console.log('[map-rendering-safety] ok: deterministic map pane visibility, dedicated search discovery framing, Wave 4 map guidance, popup/drawer language, native Leaflet tile reset, resize invalidation, and no decorative basemap overlays verified.');
}
main().catch((error) => {
