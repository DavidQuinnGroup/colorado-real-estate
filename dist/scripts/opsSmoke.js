...
async function assertMlsStatus() {
    ...
    assert.ok(typeof payload.mediaDiagnostics.health === 'string', 'Expected MLS status mediaDiagnostics health metadata.');
    assert.ok(typeof payload.mediaDiagnostics.extractedMediaCount === 'number', 'Expected MLS status mediaDiagnostics extracted media count.');
    assert.ok(typeof payload.mediaDiagnostics.ignoredMediaItemCount === 'number', 'Expected MLS status mediaDiagnostics ignored media count.');
    ...
    assert.ok(typeof (isRecord(payload.commands) ? payload.commands.rawStatus : null) === 'string' &&
        String(payload.commands.rawStatus).includes('/api/mls/status'), 'Expected MLS status rawStatus command.');
    ...
    if (ADMIN_KEY) {
        ...
        const unauthorizedPayload = (await unauthorizedResponse.json().catch(() => null));
        ...
        assert.ok(typeof unauthorizedPayload.command === 'string' &&
            unauthorizedPayload.command.includes('/api/mls/status') &&
            unauthorizedPayload.command.includes('inspect=overview'), 'Expected MLS status unauthorized command metadata to preserve query.');
...
async function assertMlsSyncRouteDryRun() {
    const path = '/api/mls/sync?dryRun=true&force=true&maxPages=999&pageSize=999&startPage=-5&rateDelayMs=999999&pageTimeoutMs=999999&includeMedia=true';
    ...
    assert.ok(isRecord(payload.job.data), 'Expected MLS sync route dry-run job data.');
    ...
    assert.equal(payload.job.name, 'sync', 'Expected MLS sync route dry-run job name.');
    const expectedJobResultMetrics = payload.expectedJobResultMetrics;
    assert.ok(Array.isArray(expectedJobResultMetrics.media), 'Expected MLS sync route dry-run media job result metric hints.');
    assert.ok(expectedJobResultMetrics.media.includes('mediaDiagnostics.listingsWithMedia'), 'Expected MLS sync route dry-run listings-with-media metric hint.');
    assert.ok(expectedJobResultMetrics.media.includes('mediaDiagnostics.extractedMediaCount'), 'Expected MLS sync route dry-run extracted-media metric hint.');
    assert.ok(expectedJobResultMetrics.media.includes('mediaDiagnostics.ignoredMediaItemCount'), 'Expected MLS sync route dry-run ignored-media metric hint.');
    assert.ok(typeof expectedJobResultMetrics.interpretation === 'string' &&
        expectedJobResultMetrics.interpretation.includes('mediaDiagnostics'), 'Expected MLS sync route dry-run media diagnostics interpretation.');
    const mediaDiagnosticsInspection = payload.mediaDiagnosticsInspection;
    assert.equal(mediaDiagnosticsInspection.statusRoute, '/api/mls/status', 'Expected MLS sync media diagnostics inspection status route.');
    assert.equal(mediaDiagnosticsInspection.adminPanelSection, 'MLS Media Diagnostics', 'Expected MLS sync media diagnostics inspection admin panel section.');
    assert.equal(mediaDiagnosticsInspection.includeMediaRequested, true, 'Expected MLS sync media diagnostics inspection to preserve includeMedia request state.');
    assert.ok(Array.isArray(mediaDiagnosticsInspection.requiredStatusFields) &&
        mediaDiagnosticsInspection.requiredStatusFields.includes('mediaDiagnostics.extractedMediaCount'), 'Expected MLS sync media diagnostics inspection required status fields.');
    assert.ok(typeof mediaDiagnosticsInspection.liveVolumeGuidance === 'string' &&
        mediaDiagnosticsInspection.liveVolumeGuidance.includes('includes media'), 'Expected MLS sync media diagnostics inspection live volume guidance.');
    ...
    assert.ok(typeof payload.liveEnqueueHint === 'string' && payload.liveEnqueueHint.includes('/api/mls/sync?execute=true'), 'Expected MLS sync route live enqueue hint.');
    assert.ok(typeof (payload.commands.dryRunSync) === 'string' &&
        String(payload.commands.dryRunSync).includes('dryRun=true'), 'Expected MLS sync route dry-run command.');
    assert.ok(typeof (payload.commands.liveSync) === 'string' &&
        String(payload.commands.liveSync).includes('execute=true'), 'Expected MLS sync route live command.');
    return {
        path,
...
async function assertSearch() {
    ...
    assert.ok(isRecord(payload.meta.smoke), 'Expected search smoke metadata.');
    assert.equal(payload.meta.smoke.terminal, 'Terminal 5', 'Expected search smoke terminal metadata.');
    assert.equal(payload.meta.smoke.command, 'npm run smoke:search', 'Expected search smoke command metadata.');
    assert.ok(isRecord(payload.meta.smoke.checks), 'Expected search smoke checks metadata.');
    return {
        path,
        ...
        terminal: payload.terminal,
        smokeReady: isRecord(payload.meta.smoke) ? payload.meta.smoke.ready : null,
    };
}
...
function assertMlsListingMediaDiagnostics() {
    ...
    assert.deepEqual(diagnostics.topLevelPhotoFields, ['PrimaryPhotoURL', 'LargePhotoURL'], 'Expected MLS listing media diagnostics top-level photo fields.');
    ...
    assert.equal(normalized.records.some((record) => record.url.endsWith('floorplan.pdf')), false, 'Expected top-level floorplan PDF URL to be rejected instead of forced to image media.');
    return {
...
function assertMlsPageFetchDiagnostics() {
    ...
    assert.ok(diagnostics.top < 10000, 'Expected MLS page diagnostics to bound oversized page size.');
    assert.ok(diagnostics.timeoutMs < 999999, 'Expected MLS page diagnostics to bound oversized timeout.');
...
function assertMlsUpsertDiagnostics() {
    const listingDiagnostics = getUpsertListingDiagnostics({
        ...
        PublicRemarks: 'Updated custom home with solar, heat pump, standing seam metal roof, 200 amp service, expansive soil review, and polybutylene piping disclosure.',
    });
...
function assertSearchIndexDiagnostics() {
    ...
    assert.ok(typeof missingDiagnostics.error === 'string' && missingDiagnostics.error.includes('No listing'), 'Expected search index diagnostics missing listing error.');
    ...
    assert.ok(typeof missingIdDiagnostics.error === 'string' && missingIdDiagnostics.error.includes('id or MLS id'), 'Expected search index diagnostics missing ID error.');
    return {
...
function assertMlsBatchMediaDiagnostics() {
    ...
    assert.equal(diagnostics.topLevelPhotoFieldCounts.PrimaryPhotoURL, 1, 'Expected MLS batch media diagnostics PrimaryPhotoURL count.');
    assert.equal(diagnostics.topLevelPhotoFieldCounts.LargePhotoURL, 1, 'Expected MLS batch media diagnostics LargePhotoURL count.');
    return diagnostics;
}
...
function assertEnqueueAlertPlan() {
    ...
    assert.throws(() => getEnqueueAlertPlan('   '), /alertId is required/, 'Expected enqueue alert plan to reject blank alertId.');
    return {
        queueName: plan.queueName,
        jobName: plan.jobName,
        jobId: plan.jobId,
...
function assertDeadLetterQueuePlan() {
    ...
    const payload = plan.data.payload;
...
async function assertControlState() {
    ...
    assert.equal(typeof payload.auth.configured, 'boolean', 'Expected control state auth configured flag.');
    ...
    assert.ok(typeof state.updatedBy === 'string' && state.updatedBy.length <= 120, 'Expected control state updatedBy to be bounded.');
    ...
    if (state.killSwitchActive === true) {
        assert.ok(warnings.some((warning) => warning.includes('Kill switch is active')), 'Expected control state kill-switch policy warning.');
    }
    if (mode === 'paused') {
        assert.ok(warnings.includes('Control mode is paused.'), 'Expected control state paused policy warning.');
    }
    if (state.areaCloud === false) {
        assert.ok(warnings.some((warning) => warning.includes('Area-cloud masking is off')), 'Expected control state map precision policy warning.');
    }
    if (state.privateLayer === true) {
        assert.ok(warnings.some((warning) => warning.includes('Private client layer is visible')), 'Expected control state private-layer policy warning.');
    }
    let unauthorizedTerminal = null;
    if (ADMIN_KEY) {
        const unauthorizedResponse = await fetch(`${BASE_URL}${path}`, {
        ...
        assert.ok(typeof unauthorizedPayload.command === 'string' && unauthorizedPayload.command.includes('inspect=policy'), 'Expected control state unauthorized command metadata to preserve query.');
        unauthorizedTerminal = unauthorizedPayload.terminal;
    }
    return {
        path,
...
async function assertIntakeSignals() {
    ...
    assert.ok(typeof payload.command === 'string' &&
        payload.command.includes('/api/admin/intake-signals') &&
        payload.command.includes('limit=6'), 'Expected intake signals command metadata to preserve limit query.');
    ...
    assert.equal(typeof payload.auth.configured, 'boolean', 'Expected intake signals auth configured flag.');
    ...
    assert.ok(typeof (isRecord(payload.readiness) ? payload.readiness.nextCommand : null) === 'string' &&
        String(isRecord(payload.readiness) ? payload.readiness.nextCommand : '').includes('limit=6'), 'Expected intake signals readiness command to preserve limit query.');
    ...
    assert.ok(typeof summary.hiddenPromotedInteractions === 'number' && summary.hiddenPromotedInteractions >= 0, 'Expected intake signals hiddenPromotedInteractions count.');
    assert.ok(readiness.level === 'ready' || readiness.level === 'watch' || readiness.level === 'blocked', 'Expected intake signals readiness level.');
    assert.ok(Array.isArray(readiness.gates), 'Expected intake signals readiness gates array.');
    assert.ok(readiness.gates.some((gate) => isRecord(gate) && gate.name === 'Signal Visibility'), 'Expected intake signals Signal Visibility gate.');
    assert.ok(readiness.gates.some((gate) => isRecord(gate) && gate.name === 'Promoted Interaction Handoff'), 'Expected intake signals Promoted Interaction Handoff gate.');
    assert.ok(readiness.gates.some((gate) => isRecord(gate) && gate.name === 'Alert Criteria'), 'Expected intake signals Alert Criteria gate.');
    ...
    assert.ok(typeof promotedPayload.command === 'string' &&
        promotedPayload.command.includes('limit=6') &&
        promotedPayload.command.includes('includePromotedInteractions=true'), 'Expected promoted intake signals command metadata to preserve includePromotedInteractions query.');
    ...
    assert.equal(promotedPayload.summary.total, promotedPayload.signals.length, 'Expected promoted intake signals summary total to match visible signals.');
    assert.equal(promotedPayload.summary.hiddenPromotedInteractions, 0, 'Expected promoted intake signals response to include promoted interactions instead of hiding them.');
    ...
    assert.ok(typeof boundedPayload.command === 'string' &&
        boundedPayload.command.includes('limit=999') &&
        boundedPayload.command.includes('includePromotedInteractions=yes'), 'Expected bounded intake signals command metadata to preserve raw query.');
    ...
    assert.equal(boundedPayload.summary.total, boundedPayload.signals.length, 'Expected bounded intake signals summary total to match visible signals.');
    let unauthorizedTerminal = null;
    if (ADMIN_KEY) {
        ...
        assert.ok(typeof unauthorizedPayload.command === 'string' &&
            ...
            unauthorizedPayload.command.includes('includePromotedInteractions=true'), 'Expected intake signals unauthorized command metadata to preserve query.');
        unauthorizedTerminal = unauthorizedPayload.terminal;
    }
    let detailReadiness = null;
    let detailInspectionSource = null;
    let unauthorizedDetailInspectionSource = null;
    let missingDetailReadiness = null;
    ...
    if (isRecord(firstSignal) && typeof firstSignal.id === 'string' && typeof firstSignal.kind === 'string') {
        ...
        assert.ok(typeof detailPayload.command === 'string' &&
            detailPayload.command.includes(`/api/admin/intake-signals/${firstSignalId}`) &&
            detailPayload.command.includes(`kind=${firstSignalKind}`), 'Expected intake signal detail command metadata to preserve kind query.');
        ...
        assert.equal(typeof detailPayload.auth.configured, 'boolean', 'Expected intake signal detail auth configured flag.');
        ...
        assert.ok(typeof (isRecord(detailPayload.readiness) ? detailPayload.readiness.nextCommand : null) === 'string' &&
            String(isRecord(detailPayload.readiness) ? detailPayload.readiness.nextCommand : '').includes(`kind=${firstSignalKind}`), 'Expected intake signal detail readiness command to preserve kind query.');
        const detailSignal = detailPayload.signal;
        const detailReadinessPayload = detailPayload.readiness;
        ...
        assert.ok(detailReadinessPayload.level === 'ready' || detailReadinessPayload.level === 'watch' || detailReadinessPayload.level === 'blocked', 'Expected intake signal detail readiness level.');
        assert.ok(Array.isArray(detailReadinessPayload.gates), 'Expected intake signal detail readiness gates array.');
        assert.ok(detailReadinessPayload.gates.some((gate) => isRecord(gate) && gate.name === 'Signal Visibility'), 'Expected intake signal detail Signal Visibility gate.');
        assert.ok(detailReadinessPayload.gates.some((gate) => isRecord(gate) && gate.name === 'Promotion State'), 'Expected intake signal detail Promotion State gate.');
        assert.ok(detailReadinessPayload.gates.some((gate) => isRecord(gate) && gate.name === 'Alert Criteria'), 'Expected intake signal detail Alert Criteria gate.');
        assert.ok(detailReadinessPayload.gates.some((gate) => isRecord(gate) && gate.name === 'Review State'), 'Expected intake signal detail Review State gate.');
        ...
        const missingPayload = (await missingResponse.json().catch(() => null));
        ...
        assert.equal(typeof missingPayload.auth.configured, 'boolean', 'Expected missing intake signal detail auth configured flag.');
        assert.equal(isRecord(missingPayload.readiness) ? missingPayload.readiness.level : null, 'blocked', 'Expected missing intake signal detail readiness to be blocked.');
        assert.ok(typeof missingPayload.command === 'string' &&
            missingPayload.command.includes('/api/admin/intake-signals/ops-smoke-missing-signal') &&
            missingPayload.command.includes(`kind=${firstSignalKind}`), 'Expected missing intake signal detail command metadata to preserve kind query.');
        ...
        if (ADMIN_KEY) {
            ...
            assert.equal(isRecord(unauthorizedPayload.readiness) ? unauthorizedPayload.readiness.terminal : null, 'Terminal 5', 'Expected intake signal detail unauthorized readiness terminal.');
            ...
            assert.ok(typeof unauthorizedPayload.command === 'string' &&
                ...
                unauthorizedPayload.command.includes(`kind=${firstSignalKind}`), 'Expected intake signal detail unauthorized command metadata to preserve kind query.');
...
async function assertCRMTasks() {
    ...
    assert.equal(typeof payload.auth.configured, 'boolean', 'Expected CRM task list auth configured flag.');
    ...
    assert.deepEqual(isRecord(payload.filters) ? payload.filters.effectiveStatuses : null, ['pending', 'reviewing'], 'Expected CRM task list active filter to expand to pending and reviewing statuses.');
    ...
    assert.ok(typeof (isRecord(payload.operations) ? payload.operations.intakeCommand : null) === 'string' &&
        String(payload.operations.intakeCommand).includes('/api/admin/intake-signals?limit=6') &&
        String(payload.operations.intakeCommand).includes('x-admin-key'), 'Expected CRM task list operations intake command to be bounded and protected.');
    assert.ok(typeof (isRecord(payload.operations) ? payload.operations.alertStatusCommand : null) === 'string' &&
        String(payload.operations.alertStatusCommand).includes('/api/process-alerts?limit=6'), 'Expected CRM task list operations alert status command to be bounded.');
    ...
    assert.equal(Number(audit.closureReviewReady) + Number(audit.closureReviewMissing), audit.closed, 'Expected CRM task audit closure counts to reconcile.');
    assert.ok(typeof audit.closureReviewCoveragePercent === 'number' &&
        audit.closureReviewCoveragePercent >= 0 &&
        audit.closureReviewCoveragePercent <= 100, 'Expected CRM task audit coverage percent to be bounded.');
    assert.ok(readiness.level === 'ready' || readiness.level === 'watch' || readiness.level === 'blocked', 'Expected CRM task list readiness level.');
    assert.ok(Array.isArray(readiness.gates), 'Expected CRM task list readiness gates array.');
    assert.ok(readiness.gates.some((gate) => isRecord(gate) && gate.label === 'Closure Audit'), 'Expected CRM task list Closure Audit readiness gate.');
    assert.ok(readiness.gates.some((gate) => isRecord(gate) && gate.label === 'Active Review'), 'Expected CRM task list Active Review readiness gate.');
    assert.ok(readiness.gates.some((gate) => isRecord(gate) && gate.label === 'Alert Criteria'), 'Expected CRM task list Alert Criteria readiness gate.');
    ...
    assert.ok(typeof filteredPayload.command === 'string' &&
        filteredPayload.command.includes('status=all') &&
        filteredPayload.command.includes('type=strategy_intake') &&
        filteredPayload.command.includes('limit=4'), 'Expected filtered CRM task list command metadata to preserve status, type, and limit.');
    ...
    assert.ok(typeof boundedPayload.command === 'string' &&
        ...
        boundedPayload.command.includes('limit=999'), 'Expected bounded CRM task list command metadata to preserve raw query.');
    ...
    let detailInspectionSource = null;
    let missingDetailInspectionSource = null;
    let unauthorizedListInspectionSource = null;
    let unauthorizedInspectionSource = null;
    ...
    if (ADMIN_KEY) {
        ...
        const unauthorizedListPayload = (await unauthorizedListResponse.json().catch(() => null));
        ...
        assert.equal(isRecord(unauthorizedListPayload.readiness) ? unauthorizedListPayload.readiness.terminal : null, 'Terminal 5', 'Expected CRM task list unauthorized readiness terminal.');
        ...
        assert.equal(typeof unauthorizedListPayload.auth.configured, 'boolean', 'Expected CRM task list unauthorized auth configured flag.');
        assert.equal(isRecord(unauthorizedListPayload.readiness) ? unauthorizedListPayload.readiness.level : null, 'blocked', 'Expected CRM task list unauthorized readiness to be blocked.');
        assert.ok(typeof unauthorizedListPayload.command === 'string' &&
            unauthorizedListPayload.command.includes('status=all') &&
            unauthorizedListPayload.command.includes('type=strategy_intake') &&
            unauthorizedListPayload.command.includes('limit=4'), 'Expected CRM task list unauthorized command metadata to preserve query.');
    ...
    if (isRecord(firstTask) && typeof firstTask.id === 'string') {
        ...
        assert.ok(typeof detailPayload.command === 'string' &&
            ...
            detailPayload.command.includes('inspect=detail'), 'Expected CRM task detail command metadata to preserve query.');
        ...
        assert.equal(typeof detailPayload.auth.configured, 'boolean', 'Expected CRM task detail auth configured flag.');
        ...
        assert.ok(isRecord(detailPayload.task) &&
            isRecord(detailPayload.task.operations) &&
            typeof detailPayload.task.operations.intakeCommand === 'string' &&
            detailPayload.task.operations.intakeCommand.includes('/api/admin/intake-signals?limit=6'), 'Expected CRM task detail operations intake command to be bounded.');
        const detailTask = detailPayload.task;
        ...
        assert.ok(detailTask.priority === 'high' || detailTask.priority === 'medium' || detailTask.priority === 'low' || detailTask.priority === 'unknown', 'Expected CRM task detail priority.');
        ...
        assert.equal(typeof missingPayload.auth.configured, 'boolean', 'Expected missing CRM task detail auth configured flag.');
        assert.ok(typeof missingPayload.command === 'string' &&
            missingPayload.command.includes('/api/admin/crm-tasks/ops-smoke-missing-task') &&
            missingPayload.command.includes('inspect=detail'), 'Expected missing CRM task detail command metadata to preserve query.');
        ...
        if (ADMIN_KEY) {
            ...
            assert.equal(typeof unauthorizedPayload.auth.configured, 'boolean', 'Expected CRM task detail unauthorized auth configured flag.');
            assert.ok(typeof unauthorizedPayload.command === 'string' &&
                unauthorizedPayload.command.includes(`/api/admin/crm-tasks/${firstTaskId}`) &&
                unauthorizedPayload.command.includes('inspect=detail'), 'Expected CRM task detail unauthorized command metadata to preserve query.');
...
async function assertMlsRetryStatus() {
    ...
    assert.ok(typeof (isRecord(payload.commands) ? payload.commands.retryStatus : null) === 'string' &&
        String(payload.commands.retryStatus).includes('/api/mls/retry'), 'Expected MLS retry status command.');
    assert.ok(typeof (isRecord(payload.commands) ? payload.commands.dryRunRetry : null) === 'string' &&
        String(payload.commands.dryRunRetry).includes('/api/mls/retry'), 'Expected MLS retry dry-run command.');
    ...
    if (ADMIN_KEY) {
        ...
        assert.ok(typeof unauthorizedPayload.command === 'string' &&
            ...
            unauthorizedPayload.command.includes('limit=6'), 'Expected MLS retry unauthorized command metadata to preserve query.');
...
async function assertMlsRetryDryRun() {
    ...
    assert.ok(typeof (isRecord(payload.commands) ? payload.commands.repeatDryRun : null) === 'string' &&
        String(payload.commands.repeatDryRun).includes('dryRun=true'), 'Expected MLS retry dry-run repeat command.');
    assert.ok(typeof (isRecord(payload.commands) ? payload.commands.liveRetry : null) === 'string' &&
        String(payload.commands.liveRetry).includes('execute=true'), 'Expected MLS retry dry-run live retry command guidance.');
...
async function assertDeadLetter() {
    ...
    assert.ok(Array.isArray(isRecord(payload.filters) ? payload.filters.states : null) &&
        payload.filters.states.includes('waiting') &&
        payload.filters.states.includes('delayed') &&
        payload.filters.states.includes('failed'), 'Expected dead-letter filters to preserve requested open states.');
    ...
    assert.ok(typeof (isRecord(payload.commands) ? payload.commands.deadLetterOpen : null) === 'string' &&
        String(payload.commands.deadLetterOpen).includes('/api/admin/dead-letter'), 'Expected dead-letter open command.');
    ...
    assert.ok(typeof filteredPayload.command === 'string' &&
        ...
        filteredPayload.command.includes('limit=4'), 'Expected filtered dead-letter command metadata to preserve sourceQueue and limit.');
    ...
    const boundedPath = '/api/admin/dead-letter?sourceQueue=mls-sync%20%24bad%23chars&state=completed&limit=999';
    const boundedPayload = await fetchJson(boundedPath);
    ...
    assert.ok(typeof boundedPayload.command === 'string' &&
        ...
        boundedPayload.command.includes('limit=999'), 'Expected bounded dead-letter command metadata to preserve raw query.');
    let unauthorizedTerminal = null;
    if (ADMIN_KEY) {
        ...
        assert.ok(typeof unauthorizedPayload.command === 'string' &&
            ...
            unauthorizedPayload.command.includes('limit=4'), 'Expected dead-letter unauthorized command metadata to preserve query.');
        unauthorizedTerminal = unauthorizedPayload.terminal;
    }
    return {
        path,
...
async function assertAlertStatus() {
    ...
    assert.equal(typeof payload.auth.configured, 'boolean', 'Expected process-alerts auth configured flag.');
    ...
    assert.equal(payload.notificationReadiness.terminal, 'Terminal 5', 'Expected process-alerts notification readiness terminal metadata.');
    assert.equal(payload.notificationReadiness.route, '/api/process-alerts', 'Expected process-alerts notification readiness route metadata.');
    assert.equal(typeof payload.notificationReadiness.nextCommand, 'string', 'Expected process-alerts notification readiness next command metadata.');
    assert.ok(Array.isArray(payload.notificationReadiness.blockerCodes), 'Expected process-alerts notification readiness blocker code summary.');
    assert.ok(Array.isArray(payload.notificationReadiness.blockerEnvVars), 'Expected process-alerts notification readiness blocker env var summary.');
    assert.ok(Array.isArray(payload.notificationReadiness.blockedBy), 'Expected process-alerts notification readiness blockedBy array.');
    assert.ok(isRecord(payload.notificationReadiness.commands), 'Expected process-alerts notification readiness commands metadata.');
    assert.equal(isRecord(payload.notificationReadiness.commands)
        ? payload.notificationReadiness.commands.notificationReadiness
        : null, 'npm run check:notification-readiness', 'Expected process-alerts notification readiness command hint.');
    const notificationBlockers = payload.notificationReadiness.blockedBy;
    const notificationBlockerCodes = payload.notificationReadiness.blockerCodes;
    const notificationBlockerEnvVars = payload.notificationReadiness.blockerEnvVars;
    const structuredNotificationEnvVars = Array.from(new Set(notificationBlockers
        .filter(isRecord)
        .flatMap((blocker) => (Array.isArray(blocker.envVars) ? blocker.envVars : []))
        .filter((envVar) => typeof envVar === 'string')));
    assert.equal(notificationBlockerCodes.length, notificationBlockers.filter(isRecord).length, 'Expected process-alerts notification blocker code summary count to match structured blockers.');
    assert.equal(notificationBlockerEnvVars.length, structuredNotificationEnvVars.length, 'Expected process-alerts notification blocker env summary count to match structured blocker env vars.');
    for (const blocker of notificationBlockers.filter(isRecord)) {
        assert.ok(notificationBlockerCodes.includes(blocker.code), 'Expected process-alerts notification blocker summary to include every structured blocker code.');
    }
    for (const envVar of structuredNotificationEnvVars) {
        assert.ok(notificationBlockerEnvVars.includes(envVar), 'Expected process-alerts notification blocker env summary to include every structured blocker env var.');
    }
    const firstNotificationBlocker = notificationBlockers.find(isRecord);
    if (firstNotificationBlocker) {
        assert.ok(notificationBlockerCodes.includes(firstNotificationBlocker.code), 'Expected process-alerts notification blocker summary to include first blocker code.');
        ...
        assert.ok(firstNotificationBlocker.envVars.every((envVar) => typeof envVar === 'string'), 'Expected process-alerts notification blocker env vars to be strings.');
        for (const envVar of firstNotificationBlocker.envVars) {
            assert.ok(notificationBlockerEnvVars.includes(envVar), 'Expected process-alerts notification blocker env summary to include first blocker env vars.');
        }
        assert.equal(firstNotificationBlocker.nextCommand, 'npm run check:property-inquiry-notification:readiness', 'Expected process-alerts notification blocker next command.');
    }
    ...
    const failedRows = payload.failedAlertRows;
    ...
    assert.ok(typeof (isRecord(payload.commands) ? payload.commands.status : null) === 'string' &&
        String(payload.commands.status).includes('/api/process-alerts'), 'Expected process-alerts status command.');
    assert.ok(typeof (isRecord(payload.commands) ? payload.commands.dryRun : null) === 'string' &&
        String(payload.commands.dryRun).includes('/api/process-alerts'), 'Expected process-alerts dry-run command.');
    assert.ok(typeof (isRecord(payload.commands) ? payload.commands.deadLetter : null) === 'string' &&
        String(payload.commands.deadLetter).includes('/api/admin/dead-letter'), 'Expected process-alerts dead-letter command.');
    assert.ok(typeof (isRecord(payload.commands) ? payload.commands.queueDashboard : null) === 'string' &&
        String(payload.commands.queueDashboard).includes('run:queue-dashboard'), 'Expected process-alerts queue dashboard command.');
    assert.ok(typeof (isRecord(payload.commands) ? payload.commands.failedRowsInspection : null) === 'string' &&
        String(payload.commands.failedRowsInspection).includes('/api/process-alerts'), 'Expected process-alerts failed-row inspection command.');
    assert.ok(typeof (isRecord(payload.commands) ? payload.commands.failedRowsScript : null) === 'string' &&
        String(payload.commands.failedRowsScript).includes('run:alerts:failed'), 'Expected process-alerts failed-row script command.');
    assert.ok(typeof (isRecord(payload.commands) ? payload.commands.markTestFailedRowsSkipped : null) === 'string' &&
        String(payload.commands.markTestFailedRowsSkipped).includes('run:alerts:failed:skip-test'), 'Expected process-alerts test-row cleanup command.');
    assert.ok(typeof (isRecord(payload.commands) ? payload.commands.scriptDryRun : null) === 'string' &&
        String(payload.commands.scriptDryRun).includes('run:alerts:dry'), 'Expected process-alerts dry-run script command.');
    assert.ok(typeof (isRecord(payload.commands) ? payload.commands.scriptLive : null) === 'string' &&
        String(payload.commands.scriptLive).includes('run:alerts:live'), 'Expected process-alerts live script command.');
    ...
    assert.equal(dryRunPayload.notificationReadiness.terminal, 'Terminal 5', 'Expected process-alerts dry-run notification readiness terminal metadata.');
    assert.ok(Array.isArray(dryRunPayload.notificationReadiness.blockedBy), 'Expected process-alerts dry-run notification blockedBy array.');
    ...
    assert.ok(typeof (isRecord(dryRunPayload.commands) ? dryRunPayload.commands.live : null) === 'string' &&
        String(dryRunPayload.commands.live).includes('execute=true'), 'Expected process-alerts dry-run live command guidance.');
    ...
    if (ADMIN_KEY) {
        ...
        assert.ok(typeof unauthorizedPayload.command === 'string' &&
            ...
            unauthorizedPayload.command.includes('limit=6'), 'Expected process-alerts unauthorized command metadata to preserve query.');
        ...
        assert.equal(unauthorizedPayload.notificationReadiness.terminal, 'Terminal 5', 'Expected process-alerts unauthorized notification readiness terminal metadata.');
        assert.ok(Array.isArray(unauthorizedPayload.notificationReadiness.blockedBy), 'Expected process-alerts unauthorized notification blockedBy array.');
        assert.ok(isRecord(unauthorizedPayload.auth), 'Expected process-alerts unauthorized auth metadata.');
        assert.equal(typeof unauthorizedPayload.auth.configured, 'boolean', 'Expected process-alerts unauthorized auth configured flag.');
...
async function assertLaunchReadiness() {
    const payload = await new Promise((resolve, reject) => {
        ...
        child.on('exit', (code) => {
            try {
                ...
                reject(new Error(`Launch readiness command failed to return expected JSON: ${errorMessage(error)}${stderr ? ` stderr=${stderr.slice(0, 500)}` : ''}`));
            }
        });
    });
    ...
    assert.equal(isRecord(payload.commands) ? payload.commands.propertyInquiryReadiness : null, 'npm run check:property-inquiry-notification:readiness', 'Expected property inquiry readiness command hint.');
    assert.equal(isRecord(payload.commands) ? payload.commands.notificationReadiness : null, 'npm run check:notification-readiness', 'Expected notification readiness summary command hint.');
    assert.equal(isRecord(payload.commands) ? payload.commands.strictNotificationReadiness : null, 'npm run check:notification-readiness:strict', 'Expected strict notification readiness command hint.');
    assert.equal(isRecord(payload.commands) ? payload.commands.strictNotificationReadinessContract : null, 'npm run check:notification-readiness:strict-contract', 'Expected strict notification readiness contract command hint.');
    ...
    assert.ok(propertyInquiryGate.level === 'ready' || propertyInquiryGate.level === 'watch' || propertyInquiryGate.level === 'blocked', 'Expected property inquiry launch gate level.');
    assert.ok(Array.isArray(propertyInquiryGate.checks), 'Expected property inquiry launch checks.');
    const recipientCheck = propertyInquiryGate.checks.find((check) => isRecord(check) && check.name === 'PROPERTY_INQUIRY_NOTIFY_TO');
...
async function assertSavedSearchAlertNotificationReadiness() {
    const payload = await new Promise((resolve, reject) => {
        ...
        child.on('exit', (code) => {
            try {
                ...
                assert.equal(isRecord(parsed.commands) ? parsed.commands.savedSearchAlertReadiness : null, 'npm run check:alert-notification-readiness', 'Expected saved-search alert readiness command hint.');
                assert.equal(isRecord(parsed.commands) ? parsed.commands.notificationReadiness : null, 'npm run check:notification-readiness', 'Expected notification readiness command hint.');
                assert.equal(isRecord(parsed.commands) ? parsed.commands.strictNotificationReadiness : null, 'npm run check:notification-readiness:strict', 'Expected strict notification readiness command hint.');
                assert.equal(isRecord(parsed.commands) ? parsed.commands.strictNotificationReadinessContract : null, 'npm run check:notification-readiness:strict-contract', 'Expected strict notification readiness contract command hint.');
                assert.equal(isRecord(parsed.commands) ? parsed.commands.launchReadiness : null, 'npm run check:launch-readiness', 'Expected launch readiness command hint.');
                assert.equal(isRecord(parsed.commands) ? parsed.commands.savedSearchAlertDryRun : null, 'npm run run:alerts:dry -- --limit 50', 'Expected saved-search alert dry-run command hint.');
                assert.ok(code === 0 || code === 1, `Expected saved-search alert readiness to exit 0 or 1, got ${code ?? 'unknown'}.`);
                resolve(parsed);
            }
            catch (error) {
                reject(new Error(`Saved-search alert readiness command failed to return expected JSON: ${errorMessage(error)}${stderr ? ` stderr=${stderr.slice(0, 500)}` : ''}`));
            }
        });
    });
    ...
    assert.ok(readinessLevel === 'ready' || readinessLevel === 'watch' || readinessLevel === 'blocked', 'Expected saved-search alert readiness level.');
    const failedRowsCheck = payload.checks.find((check) => isRecord(check) && check.name === 'AlertQueue failed rows');
...
async function assertPropertyInquiryNotificationReadiness() {
    const payload = await new Promise((resolve, reject) => {
        ...
        child.on('exit', (code) => {
            try {
                ...
                assert.equal(parsed.nextCommand, 'npm run check:property-inquiry-notification:readiness', 'Expected blocked property inquiry readiness next command.');
                assert.ok(isRecord(parsed.commands), 'Expected property inquiry readiness commands metadata.');
                assert.equal(isRecord(parsed.commands) ? parsed.commands.propertyInquiryReadiness : null, 'npm run check:property-inquiry-notification:readiness', 'Expected property inquiry readiness command hint.');
                assert.equal(isRecord(parsed.commands) ? parsed.commands.notificationReadiness : null, 'npm run check:notification-readiness', 'Expected notification readiness command hint.');
                assert.equal(isRecord(parsed.commands) ? parsed.commands.strictNotificationReadiness : null, 'npm run check:notification-readiness:strict', 'Expected strict notification readiness command hint.');
                assert.equal(isRecord(parsed.commands) ? parsed.commands.strictNotificationReadinessContract : null, 'npm run check:notification-readiness:strict-contract', 'Expected strict notification readiness contract command hint.');
                assert.equal(isRecord(parsed.commands) ? parsed.commands.launchReadiness : null, 'npm run check:launch-readiness', 'Expected launch readiness command hint.');
                ...
                reject(new Error(`Property inquiry readiness command failed to return expected JSON: ${errorMessage(error)}${stderr ? ` stderr=${stderr.slice(0, 500)}` : ''}`));
            }
        });
    });
    ...
    const readinessLevel = isRecord(payload.readiness) ? payload.readiness.level : null;
    assert.ok(readinessLevel === 'ready' || readinessLevel === 'watch' || readinessLevel === 'blocked', 'Expected property inquiry readiness level.');
    const recipientCheck = payload.checks.find((check) => isRecord(check) && check.name === 'PROPERTY_INQUIRY_NOTIFY_TO');
    assert.ok(isRecord(recipientCheck), 'Expected direct property inquiry recipient check.');
    assert.ok(recipientCheck.status === 'pass' || recipientCheck.status === 'warn' || recipientCheck.status === 'fail', 'Expected direct property inquiry recipient check status.');
    return {
        readiness: readinessLevel,
...
async function assertNotificationReadinessSummary() {
    const payload = await new Promise((resolve, reject) => {
        ...
        child.on('exit', (code) => {
            try {
                ...
                reject(new Error(`Notification readiness summary command failed to return expected JSON: ${errorMessage(error)}${stderr ? ` stderr=${stderr.slice(0, 500)}` : ''}`));
            }
        });
    });
    ...
    assert.equal(isRecord(payload.commands) ? payload.commands.notificationReadiness : null, 'npm run check:notification-readiness', 'Expected notification readiness summary command hint.');
    assert.equal(isRecord(payload.commands) ? payload.commands.strictNotificationReadiness : null, 'npm run check:notification-readiness:strict', 'Expected strict notification readiness summary command hint.');
    assert.equal(isRecord(payload.commands) ? payload.commands.strictNotificationReadinessContract : null, 'npm run check:notification-readiness:strict-contract', 'Expected strict notification readiness contract command hint.');
    assert.equal(isRecord(payload.commands) ? payload.commands.launchReadiness : null, 'npm run check:launch-readiness', 'Expected launch readiness command hint in notification summary.');
    ...
    assert.equal(propertyInquiryResult.nextCommand, 'npm run check:property-inquiry-notification:readiness', 'Expected property-inquiry summary child next command.');
    ...
    assert.equal(isRecord(savedSearchResult.commands) ? savedSearchResult.commands.savedSearchAlertReadiness : null, 'npm run check:alert-notification-readiness', 'Expected saved-search summary child readiness command hint.');
    assert.equal(isRecord(savedSearchResult.commands) ? savedSearchResult.commands.strictNotificationReadinessContract : null, 'npm run check:notification-readiness:strict-contract', 'Expected saved-search summary child strict contract command hint.');
    assert.equal(isRecord(propertyInquiryResult.commands) ? propertyInquiryResult.commands.propertyInquiryReadiness : null, 'npm run check:property-inquiry-notification:readiness', 'Expected property-inquiry summary child readiness command hint.');
    assert.equal(isRecord(propertyInquiryResult.commands) ? propertyInquiryResult.commands.strictNotificationReadinessContract : null, 'npm run check:notification-readiness:strict-contract', 'Expected property-inquiry summary child strict contract command hint.');
    ...
    const recipientFailedCheck = propertyInquiryResult.failedChecks.find((check) => isRecord(check) && check.name === 'PROPERTY_INQUIRY_NOTIFY_TO');
...
async function assertNotificationReadinessDryRunGuard({ strictMode = false } = {}) {
        ...
        child.on('exit', (code) => {
            try {
                ...
                if (strictMode) {
                    ...
                    assert.equal(code, 0, `Expected dry-run notification readiness summary to exit 0 for operator inspection, got ${code ?? 'unknown'}.`);
                }
                resolve(parsed);
            }
            catch (error) {
                reject(new Error(`Dry-run notification readiness summary command failed to return expected JSON: ${errorMessage(error)}${stderr ? ` stderr=${stderr.slice(0, 500)}` : ''}`));
            }
        });
    });
    assert.ok(isRecord(payload.readiness), 'Expected dry-run notification readiness summary metadata.');
    assert.equal(payload.readiness.level, 'blocked', 'Expected dry-run notification readiness summary to block.');
    assert.equal(payload.success, !strictMode, 'Expected dry-run notification summary success to reflect strict fail-closed mode.');
    ...
    const propertyInquiryDryRunCheck = propertyInquiryResult.failedChecks.find((check) => isRecord(check) && check.name === 'PROPERTY_INQUIRY_NOTIFICATION_DRY_RUN');
    const aggregateDryRunCheck = aggregateResult.failedChecks.find((check) => isRecord(check) && check.name === 'PROPERTY_INQUIRY_NOTIFICATION_DRY_RUN');
    ...
    return {
        readiness: payload.readiness.level,
