...
const repairMigration = fs.readFileSync("prisma/migrations/20260725142500_seller_lead_uuid_schema_reconciliation/migration.sql", "utf8");
const parityMigration = fs.readFileSync("prisma/migrations/20260722210000_repair_seller_lead_schema_parity/migration.sql", "utf8");
const gioMigration = fs.readFileSync("prisma/migrations/20260725143000_gio_wave3_additive_persistence_foundation/migration.sql", "utf8");
assert.match(schema, /model SellerLead \{[\s\S]*?id\s+String\s+@id\s+@default\(dbgenerated\("gen_random_uuid\(\)"\)\)\s+@db\.Uuid[\s\S]*?\}/, "SellerLead.id must use the governed UUID strategy.");
assert.match(schema, /sellerLeadId\s+String\?\s+@map\("leadid"\)\s+@db\.Uuid/, "CRMTask.leadid must be modeled as a UUID-compatible SellerLead foreign key.");
assert.match(schema, /sellerLead\s+SellerLead\?\s+@relation\(fields: \[sellerLeadId\], references: \[id\], onDelete: Cascade\)/, "CRMTask sellerLead relation must reflect the existing production FK behavior.");
...
console.log("[seller-lead-schema-safety] ok: UUID id strategy, UUID-compatible SellerLead FK, fail-closed migration, no SellerLead ID rewrite, governed empty-directory disposition, and dormant GIO posture verified.");
