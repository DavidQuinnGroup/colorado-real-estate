...
function printDryRun() {
    ...
    for (const listing of testListings) {
        console.log(`${listing.property.mlsId}: ${listing.property.address}, ${listing.property.city}, ${listing.property.state} ` +
            `at ${listing.property.price} with ${listing.photos.length} photo(s).`);
    }
}
function getUpdateData(property) {
    const { id: _id, ...updateData } = property;
    return updateData;
}
async function replacePhotos(propertyId, photos) {
...
async function seedListing(listing, skipIndex) {
...
async function main() {
    ...
    console.log(`REIE test property seed complete: database=${summary.database}, photos=${summary.photos}, ` +
