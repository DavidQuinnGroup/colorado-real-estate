import { ALERT_JOB_ATTEMPTS, ALERT_JOB_BACKOFF_DELAY_MS, ALERT_QUEUE_NAME, ALERT_REMOVE_ON_COMPLETE_AGE_SECONDS, ALERT_REMOVE_ON_COMPLETE_COUNT, ALERT_REMOVE_ON_FAIL_AGE_SECONDS, ALERT_REMOVE_ON_FAIL_COUNT, alertQueue, } from '../lib/queue/alertQueue.js';
...
function getDiagnosticMessage(error, queueName) {
    const aggregateMessage = getAggregateErrorMessage(error);
    if (aggregateMessage)
        return `Redis connection failed for ${queueName}: ${aggregateMessage}`;
    const code = getErrorCode(error);
    if (code)
        return `Redis connection failed for ${queueName}: ${code}`;
    if (error instanceof Error && error.message.trim()) {
        const queuedError = queueErrorMessages.get(queueName);
        if (queuedError && error.message === 'Connection is closed.')
            return queuedError;
...
function getQueueHealth(counts) {
...
function buildDryRunRetryCommand(queueName, limit = 10) {
...
function buildRetryStatusCommand(queueName, limit = 10) {
    const params = new URLSearchParams({
        queue: queueName,
        limit: String(limit),
    });
...
function buildLiveRetryCommand(queueName, limit = 10) {
    const params = new URLSearchParams({
        queue: queueName,
        dryRun: 'false',
        execute: 'true',
...
function buildWorkerStartCommand(queueName) {
    if (queueName === MLS_SYNC_QUEUE_NAME)
        return 'npm run run:worker:mls';
    if (queueName === MLS_PAGE_QUEUE_NAME)
        return 'npm run run:worker:mls-page';
    if (queueName === ALERT_QUEUE_NAME)
        return 'npm run run:worker:alerts';
    return 'npm run run:worker';
}
function buildBusyQueueNextCommand(queues) {
    ...
    if ((mlsSync?.counts.waiting || 0) > 0)
        return buildWorkerStartCommand(MLS_SYNC_QUEUE_NAME);
    if ((mlsPage?.counts.waiting || 0) > 0)
        return buildWorkerStartCommand(MLS_PAGE_QUEUE_NAME);
...
function buildDeadLetterBySourceQueueCommand(queueName, limit = 25) {
    const params = new URLSearchParams({
...
async function inspectQueue(definition, options) {
    const counts = await withTimeout(`${definition.name} getJobCounts`, options.timeoutMs, definition.queue.getJobCounts('waiting', 'active', 'delayed', 'completed', 'failed', 'paused'));
    const result = {
