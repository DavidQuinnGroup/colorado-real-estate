...
/* PRICE FORMATTER */
/* ----------------------------- */
const formatPrice = (value) => `$${(value / 1000000).toFixed(2)}M`;
/* ----------------------------- */
/* CITY COLORS */
/* ----------------------------- */
const cityColors = {
...
/* CHART COMPONENT */
/* ----------------------------- */
function Chart({ metric, title, selectedCities, isDark }) {
    ...
    return (<div className="border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 rounded-xl p-6 shadow-lg">
      ...
      <div className="w-full h-[350px]">
        ...
        <recharts_1.ResponsiveContainer width="100%" height="100%">
          ...
          <recharts_1.LineChart data={combinedData} syncId="market">

            <defs>
              {selectedCities.map((city) => (<linearGradient key={city} id={`gradient-${city}`} x1="0" y1="0" x2="0" y2="1">
            ...
            <recharts_1.XAxis dataKey="month" stroke={textColor} tickLine={false} axisLine={false}/>

            <recharts_1.YAxis stroke={textColor} tickLine={false} axisLine={false} tickFormatter={(value) => metric === "price" ? formatPrice(value) : value}/>

            <recharts_1.Tooltip formatter={(value) => metric === "price" ? formatPrice(Number(value ?? 0)) : value} cursor={{ stroke: textColor, strokeWidth: 1 }} contentStyle={{
            ...
            {selectedCities.map((city) => marketData[city] ? (<recharts_1.Area key={`area-${city}`} type="monotone" dataKey={city} stroke="none" fill={`url(#gradient-${city})`}/>) : null)}

            {selectedCities.map((city) => marketData[city] ? (<recharts_1.Line key={`line-${city}`} type="monotone" dataKey={city} stroke={cityColors[city]} strokeWidth={3} dot={false}/>) : null)}
...
function MarketChart() {
    ...
    const [selectedCities, setSelectedCities] = (0, react_1.useState)(["Boulder"]);
    ...
    return (<div className="space-y-10">
      ...
      <div className="flex flex-wrap justify-center gap-3">
        ...
        {cities.map((city) => {
            ...
            return (<button key={city} onClick={() => toggleCity(city)} className={"px-4 py-2 rounded-lg border text-sm transition " +
                    (active
      ...
      <Chart metric="price" title="Median Price Trend" selectedCities={selectedCities} isDark={isDark}/>

      <div className="grid md:grid-cols-2 gap-8">

        <Chart metric="inventory" title="Inventory Trend" selectedCities={selectedCities} isDark={isDark}/>

        <Chart metric="dom" title="Days on Market Trend" selectedCities={selectedCities} isDark={isDark}/>
