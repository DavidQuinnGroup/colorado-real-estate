...
export default function NeighborhoodProduct3Experience({ model }: NeighborhoodProduct3ExperienceProps) {
  return (
    <section
      ...
      className="mx-auto w-full max-w-7xl px-6 pt-8 md:px-12"
      aria-labelledby="neighborhood-product-3-heading"
    >
      <div
        className="overflow-hidden rounded-[8px] border border-cyan-100/20 bg-[#071017]"
      ...
      >
        ...
        <div className="border-b border-white/10 bg-cyan-100/[0.055] p-5 md:p-6">
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
            ...
            <span className={`inline-flex min-h-8 items-center rounded-[6px] border px-3 py-2 text-[10px] font-black uppercase tracking-[0.14em] ${stateClass(model.evidenceState)}`}>
          ...
          >
            {[
              ...
              <Link
                key={label}
                href={href}
                className="inline-flex min-h-10 min-w-0 items-center justify-center rounded-[6px] border border-white/10 bg-white/[0.055] px-2 py-2 text-center text-[9px] font-black uppercase tracking-[0.08em] text-white/70 focus:outline-none focus:ring-2 focus:ring-cyan-100/70"
              >
                {label}
              </Link>
            ))}
        ...
        <div className="grid gap-px bg-white/10 md:grid-cols-3">
          {model.profile.map((item) => (
            <article
              key={item.label}
              className="bg-[#071017] p-4 md:p-5"
              data-testid="neighborhood-product-3-decision-profile-item"
              data-neighborhood-product-3-profile-state={item.state}
            >
              <span className={`inline-flex rounded-[5px] border px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.12em] ${stateClass(item.state)}`}>
        ...
        >
          <div className="grid gap-px bg-white/10 lg:grid-cols-[0.74fr_1.26fr]">
            <div className="bg-[#071017] p-5 md:p-6">
              <p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.22em] text-cyan-100/76">
              ...
              {model.constellation.map((dimension) => (
                ...
                >
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <span className={`inline-flex shrink-0 rounded-[5px] border px-2 py-1 text-[9px] font-black uppercase tracking-[0.12em] ${confidenceClass(dimension.evidence)}`}>
          ...
          <div className="overflow-x-auto border-t border-white/10 bg-[#071017] p-4" data-testid="neighborhood-product-3-constellation-table">
        ...
        >
          <div className="grid gap-px bg-white/10 md:grid-cols-[0.75fr_1.25fr]">
            <div className="bg-[#071017] p-5 md:p-6">
              <p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.22em] text-cyan-100/76">
            ...
            <div className="bg-[#071017] p-5 md:p-6">
              ...
              <Link
                ...
                className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-[6px] bg-white/[0.07] px-4 py-2 text-[10px] font-black uppercase tracking-[0.12em] text-white no-underline transition hover:bg-white/[0.12] focus:outline-none focus:ring-2 focus:ring-cyan-100/70"
              >
                City market context
          ...
          <div className="grid gap-px bg-white/10 md:grid-cols-2">
            {model.propertyContext.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className="group bg-[#071017] p-5 transition hover:bg-white/[0.045]"
        ...
        >
          ...
          <div className="grid gap-px bg-white/10 md:grid-cols-5">
            {model.checklist.map((item) => (
              <article key={item.label} className="bg-[#071017] p-4" data-testid="neighborhood-product-3-verification-item">
