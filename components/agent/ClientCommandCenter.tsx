...
import { clientCommandCenterSections, clientCommandCenterSectionsFromIntent, formatInformationValue, humanize, propertyLabel, type ClientCaseSummary, type ClientCommandCenterSectionId, type InformationWorkspaceSummary, type OutputSummary } from './clientCommandCenterTypes';
...
export default function ClientCommandCenter({ clientCaseId }: { clientCaseId: string }) {
  ...
  return <main className={styles.page} data-testid="client-command-center"><div className={styles.container}>
    ...
    <div className={styles.sectionList}>
      {section('overview', 'Overview', `${clientCase.parties.length} people · ${clientCase.properties.length} properties · ${clientCase.transactions.length} transactions`, launch('View information', informationHref), <div className={styles.overviewGrid}><div><span>People</span><strong>{clientCase.parties.length || 'None recorded'}</strong></div><div><span>Properties</span><strong>{clientCase.properties.length || 'None recorded'}</strong></div><div><span>Transactions</span><strong>{clientCase._count?.transactions || 'None recorded'}</strong></div><div><span>Readiness</span><strong>{currentInformation?.value ? 'Available below' : 'Open Readiness to review'}</strong></div></div>)}
