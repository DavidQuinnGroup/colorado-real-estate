...
export function ClientCommandCenterVisualFixture() {
  const sections: Array<readonly [string, string, boolean]> = [
    ['Overview', '1 person · 0 properties · 1 transaction', true],
    ['People', 'Jordan Avery', false],
    ['Objectives and goals', 'Buyer decision · Financial strategy', false],
    ['Properties', 'No properties linked.', false],
    ...
    ['Transactions', '1 draft transaction.', false],
  ...
  return <main className={`atlas-ds-root atlas-ds-shell-agent ${styles.page}`} data-testid="client-command-center-visual-fixture"><div className={styles.container}>
    <header className={styles.commandHeader}><div className={styles.headerLinks}><a className={styles.utilityLink} href="#fixture"><Home aria-hidden="true" size={16} />Workspace Home</a></div><p className={styles.eyebrow}>Internal certification fixture</p><h1 className={styles.commandTitle}>Jordan Avery</h1><div className={styles.headerMeta}><span>Active</span><span>Jordan Avery</span><span>Updated Sep 18, 2026</span></div></header>
