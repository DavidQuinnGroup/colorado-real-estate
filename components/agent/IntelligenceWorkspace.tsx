...
function scopedHref(href: string, clientCaseId: string | null) {
  return clientCaseId ? `${href}?clientCaseId=${encodeURIComponent(clientCaseId)}` : href;
}

export function IntelligenceContextNavigation({ current }: { current: IntelligenceArea }) {
  const clientCaseId = useSearchParams().get('clientCaseId');
  return (
    <nav aria-label="Intelligence work areas" className={styles.contextNavigation} data-testid="agent-intelligence-context-navigation">
      <div className={styles.contextNavigationHeading}>
        <Link className={styles.contextHubLink} href={scopedHref('/agent/prepare', clientCaseId)}>Intelligence</Link>
      ...
      <div className={styles.contextNavigationLinks}>
        {workAreas.map((area) => (
          <Link aria-current={current === area.key ? 'page' : undefined} className={styles.contextNavigationLink} href={scopedHref(area.href, clientCaseId)} key={area.href}>
    ...
    <main className={styles.page} data-testid="agent-intelligence-workspace" data-agent-only="true" data-persistence="false" data-customer-data="false" data-provider-activity="false">
      <div className={styles.content}>
        ...
        <nav aria-label="Intelligence work areas" className={styles.contextNavigation} data-testid="agent-intelligence-context-navigation">
          ...
          <div className={styles.contextNavigationLinks}>
            {workAreas.map((area) => <Link className={styles.contextNavigationLink} href={scopedHref(area.href, clientCaseId)} key={area.href}>{area.title.replace(' Intelligence', '')}</Link>)}
          </div>
        </nav>
        ...
        <section aria-label="Intelligence work areas" className={styles.workAreaGrid}>
          {workAreas.map((area) => {
            ...
            return (
              <Link className={`atlas-ds-surface atlas-ds-surface-glass ${styles.workArea}`} href={scopedHref(area.href, clientCaseId)} key={area.href}>
                <div>
