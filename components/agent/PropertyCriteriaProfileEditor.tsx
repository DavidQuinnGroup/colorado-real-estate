...
export default function PropertyCriteriaProfileEditor({ context, onProfileChange }: { context: PropertyCriteriaContext; onProfileChange?: (profile: PropertyCriteriaProfile) => void }) {
  const [profile, setProfile] = useState(() => createPropertyCriteriaProfile(context));
  const commitProfile = (next: PropertyCriteriaProfile) => {
    onProfileChange?.(next);
    return next;
  };
  const updateRange = (field: 'bedrooms' | 'bathrooms' | 'squareFeet' | 'garageSpaces' | 'yearBuilt' | 'lotSquareFeet') => (update: { min?: number | null; max?: number | null; intent?: PropertyCriteriaIntent }) => setProfile((current) => commitProfile(updatePropertyCriteriaRange(current, field, update)));
  const updateChoice = (field: 'propertyTypes' | 'basement' | 'outdoorSpace' | 'stories' | 'condition') => (values: readonly string[], intent: PropertyCriteriaIntent) => setProfile((current) => commitProfile(updatePropertyCriteriaChoice(current, field, values as never[], intent)));
