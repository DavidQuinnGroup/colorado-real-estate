...
function RangeField({ label: fieldLabel, value, onChange, unit }: { label: string; value: PropertyCriteriaProfile['bedrooms']; onChange: (update: { min?: number | null; max?: number | null; intent?: PropertyCriteriaIntent }) => void; unit?: string }) {
  return <div className="border border-white/10 bg-black/10 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-medium text-white">{fieldLabel}</p><IntentSelect value={value.intent} onChange={(intent) => onChange({ intent })} /></div><div className="mt-3 grid grid-cols-2 gap-2"><label className="text-xs text-slate-400">Minimum<input aria-label={`${fieldLabel} minimum`} type="number" min="0" value={value.min ?? ''} onChange={(event) => onChange({ min: number(event.target.value) })} className="mt-1 min-h-10 w-full border border-white/15 bg-black/20 px-2 text-sm text-white" /></label><label className="text-xs text-slate-400">Maximum<input aria-label={`${fieldLabel} maximum`} type="number" min="0" value={value.max ?? ''} onChange={(event) => onChange({ max: number(event.target.value) })} className="mt-1 min-h-10 w-full border border-white/15 bg-black/20 px-2 text-sm text-white" /></label></div>{unit ? <p className="mt-2 text-xs text-slate-500">{unit}</p> : null}</div>;
}
...
function ChoiceField({ label: fieldLabel, values, selected, intent, onChange }: { label: string; values: readonly string[]; selected: readonly string[]; intent: PropertyCriteriaIntent; onChange: (values: readonly string[], intent: PropertyCriteriaIntent) => void }) {
  return <div className="border border-white/10 bg-black/10 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-medium text-white">{fieldLabel}</p><IntentSelect value={intent} onChange={(nextIntent) => onChange(selected, nextIntent)} /></div><div className="mt-3 flex flex-wrap gap-2">{values.map((value) => { const checked = selected.includes(value); return <label key={value} className={`cursor-pointer border px-2 py-1 text-xs ${checked ? 'border-cyan-200/70 bg-cyan-200/10 text-white' : 'border-white/10 text-slate-300'}`}><input className="sr-only" type="checkbox" checked={checked} onChange={() => onChange(checked ? selected.filter((item) => item !== value) : [...selected, value], intent)} />{label(value)}</label>; })}</div></div>;
}
