  ...
  return <main className="min-h-screen bg-[#071014] px-5 py-6 text-slate-100 sm:px-8 sm:py-8 lg:px-12" data-testid="agent-market-update-preparation-experience" data-agent-only="true" data-session-only="true" data-persistence="false" data-customer-data="false" data-recipient-selection="false" data-communication-execution="false" data-provider-activity="false" data-admin-inheritance="false">
    <div className="mx-auto max-w-6xl">
      ...
      {preparation && preparation.state !== 'NOT_READY' ? <div className="mt-8 space-y-5" data-testid="agent-market-update-package" data-market-update-state={preparation.state}>
        <section className="border border-cyan-200/20 bg-cyan-100/[0.06] p-5 sm:p-6"><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-cyan-100/80">Executive market summary</p><h2 className={`mt-2 ${projectAtlasTitleHierarchy.briefingTitle}`}>{MARKETS.find((market) => market.id === preparation.input.marketId)?.label} market update</h2><p className="mt-3 max-w-3xl text-base leading-7 text-slate-200">{preparation.executiveSummary.text}</p></section>
        <section className="grid gap-5 lg:grid-cols-2"><div className="border border-white/10 bg-white/[0.035] p-5 sm:p-6"><div className="flex items-center gap-3"><ClipboardList className="h-5 w-5 text-cyan-100" aria-hidden="true" /><h2 className={projectAtlasTitleHierarchy.briefingSection}>Key market observations</h2></div><dl className="mt-5 grid gap-3">{preparation.observations.map((item) => <div key={item.evidenceId} className="border border-white/10 bg-black/15 p-4"><dt className="text-xs text-slate-400">{item.label}</dt><dd className="mt-2 text-xl font-semibold text-white">{item.value}</dd><dd className="mt-3 text-sm leading-6 text-slate-300">{item.plainLanguageDescription.text}</dd><dd className="mt-3 text-xs leading-5 text-amber-100">Semantic state: {item.semanticState}. {item.authoritativeDocumentationRequired}</dd><dd className="mt-3 border-t border-white/10 pt-3 text-xs leading-5 text-slate-400">As of {item.sourceDate}. Source: {item.sourceId ?? 'Repository-local market context'}. Evidence: {item.evidenceId}. Freshness: {item.freshness}. Verification: professional review required.</dd></div>)}</dl></div><div className="border border-amber-200/20 bg-amber-100/[0.05] p-5 sm:p-6"><div className="flex items-center gap-3"><ShieldCheck className="h-5 w-5 text-amber-100" aria-hidden="true" /><h2 className={projectAtlasTitleHierarchy.briefingSection}>What these measures help explain</h2></div><p className="mt-5 text-sm leading-6 text-slate-300">{preparation.evidenceSynthesis.text}</p><h3 className={`mt-6 ${projectAtlasTitleHierarchy.briefingSubsection}`}>What could change the interpretation</h3><p className="mt-3 text-sm leading-6 text-slate-300">{preparation.whatCouldChangeInterpretation.text}</p><ul className="mt-5 space-y-3 text-sm leading-6 text-slate-300">{preparation.interpretationLimits.map((item) => <li key={item.text}>{item.text}</li>)}</ul></div></section>
        <section className="grid gap-5 lg:grid-cols-2"><div className="border border-white/10 bg-white/[0.035] p-5 sm:p-6"><h2 className={projectAtlasTitleHierarchy.briefingSection}>Why it matters to this conversation</h2><p className="mt-4 text-sm leading-6 text-slate-300">{preparation.audienceContext.text}</p><h3 className={`mt-6 ${projectAtlasTitleHierarchy.briefingSubsection}`}>Talking points</h3><ul className="mt-3 space-y-3 text-sm leading-6 text-slate-300">{preparation.talkingPoints.map((item) => <li key={item.text}>{item.text}</li>)}</ul></div><div className="border border-white/10 bg-white/[0.035] p-5 sm:p-6"><h2 className={projectAtlasTitleHierarchy.briefingSection}>Client-friendly explanations</h2><ul className="mt-4 space-y-3 text-sm leading-6 text-slate-300">{preparation.clientFriendlyExplanations.map((item) => <li key={item.text}>{item.text}</li>)}</ul><h3 className={`mt-6 ${projectAtlasTitleHierarchy.briefingSubsection}`}>Questions worth asking</h3><ol className="mt-3 space-y-3 text-sm leading-6 text-slate-300">{preparation.questionsWorthAsking.map((item, index) => <li key={item.text} className="grid grid-cols-[1.5rem_minmax(0,1fr)] gap-3"><span className="text-cyan-100">{index + 1}.</span>{item.text}</li>)}</ol></div></section>
        <section className="grid gap-5 lg:grid-cols-2"><div className="border border-white/10 bg-white/[0.035] p-5 sm:p-6"><h2 className={projectAtlasTitleHierarchy.briefingSection}>Sources, as-of dates &amp; freshness</h2><ul className="mt-4 space-y-3 text-sm leading-6 text-slate-300">{preparation.sourceFreshness.map((item) => <li key={item.evidenceId} className="border-b border-white/10 pb-3 last:border-0"><span className="font-medium text-white">{item.metric}: </span>{item.market}; source {item.sourceId ?? 'Repository-local market context'}; source date {item.sourceDate}; ATLAS observed {item.atlasObservedDate}; freshness {item.freshness}; verification {item.verificationStatus}.</li>)}</ul><h3 className={`mt-6 ${projectAtlasTitleHierarchy.briefingSubsection}`}>Verification checkpoints</h3><ul className="mt-3 space-y-3 text-sm leading-6 text-slate-300">{preparation.verificationCheckpoints.map((item) => <li key={item.text}>{item.text}</li>)}</ul></div><div className="border border-emerald-200/20 bg-emerald-100/[0.05] p-5 sm:p-6"><div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-emerald-100" aria-hidden="true" /><h2 className={projectAtlasTitleHierarchy.briefingSection}>Agent next actions</h2></div><ol className="mt-4 space-y-3 text-sm leading-6 text-slate-300">{preparation.agentNextActions.map((item, index) => <li key={item.text} className="grid grid-cols-[1.5rem_minmax(0,1fr)] gap-3"><span className="text-emerald-100">{index + 1}.</span>{item.text}</li>)}</ol></div></section>
        <section className="border border-white/10 bg-white/[0.025] p-5 sm:p-6"><div className="flex items-center gap-3"><FileText className="h-5 w-5 text-cyan-100" aria-hidden="true" /><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-cyan-100/70">Optional human-review language</p><h2 className={`mt-1 ${projectAtlasTitleHierarchy.briefingSection}`}>Draft market update language</h2></div></div><p className="mt-5 max-w-4xl whitespace-pre-wrap text-sm leading-7 text-slate-200">{preparation.optionalDraftLanguage?.text}</p><p className="mt-5 border-t border-white/10 pt-4 text-sm leading-6 text-slate-400">This language is not sent, published, saved, or addressed to a recipient. Review and adapt it personally before any external use.</p></section>
