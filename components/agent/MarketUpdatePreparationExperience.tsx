  ...
  return <main className="min-h-screen bg-[#071014] px-5 py-6 text-slate-100 sm:px-8 sm:py-8 lg:px-12" data-testid="agent-market-update-preparation-experience" data-agent-only="true" data-session-only="true" data-persistence="false" data-customer-data="false" data-recipient-selection="false" data-communication-execution="false" data-provider-activity="false" data-admin-inheritance="false">
    <div className="mx-auto max-w-6xl">
      ...
      <section className="mt-8 border border-white/10 bg-white/[0.035] p-5 sm:p-6" aria-labelledby="market-update-selection-heading">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-cyan-100/70">Update context</p><h2 id="market-update-selection-heading" className="mt-1 text-lg font-semibold text-white">Choose what to prepare</h2></div><p className="text-sm text-slate-400">Selections remain editable after preparation.</p></div>
        <div className="mt-6 grid gap-6 lg:grid-cols-2">
          <fieldset><legend className="text-sm font-medium text-white">Market</legend><div className="mt-3 grid gap-2 sm:grid-cols-2">{MARKETS.map((market) => <label key={market.id} className={selectClass(marketId === market.id)}><span>{market.label}</span><input type="radio" name="market-update-market" value={market.id} checked={marketId === market.id} onChange={() => setMarketId(market.id)} className="h-4 w-4 accent-cyan-200" /></label>)}</div></fieldset>
          <fieldset><legend className="text-sm font-medium text-white">Audience</legend><div className="mt-3 grid gap-2 sm:grid-cols-2">{MARKET_UPDATE_AUDIENCES.map((item) => <label key={item} className={selectClass(audience === item)}><span>{audienceLabels[item]}</span><input type="radio" name="market-update-audience" value={item} checked={audience === item} onChange={() => setAudience(item)} className="h-4 w-4 accent-cyan-200" /></label>)}</div></fieldset>
          <fieldset><legend className="text-sm font-medium text-white">Purpose</legend><div className="mt-3 grid gap-2">{MARKET_UPDATE_PURPOSES.map((item) => <label key={item} className={selectClass(purpose === item)}><span>{purposeLabels[item]}</span><input type="radio" name="market-update-purpose" value={item} checked={purpose === item} onChange={() => setPurpose(item)} className="h-4 w-4 accent-cyan-200" /></label>)}</div></fieldset>
          <fieldset><legend className="text-sm font-medium text-white">Topics to emphasize</legend><div className="mt-3 grid gap-2">{MARKET_UPDATE_TOPICS.map((item) => <label key={item} className={selectClass(topics.includes(item))}><span>{topicLabels[item]}</span><input type="checkbox" checked={topics.includes(item)} onChange={() => toggleTopic(item)} className="h-4 w-4 accent-cyan-200" /></label>)}</div></fieldset>
        </div>
