...
export default function ClientAuthorizationWorkspace() {
  ...
  async function recover(authorization: Authorization) {
    setBusy(true); setStatus(''); setOneTimeLink(null);
    try {
      const response = await fetch('/api/agent/client-authorizations', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'RECOVER_SECURE_CONFIRMATION_CAPABILITY', authorizationId: authorization.id, recoveryKey: `ATLAS_SECURE_CONFIRMATION_RECOVERY_${authorization.id}` }) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || 'The secure-link recovery could not be completed.');
      if (body.confirmationToken) setOneTimeLink(`${window.location.origin}/client-authorization/access?token=${body.confirmationToken}`);
      await load();
      setStatus(body.confirmationToken ? 'Replacement secure link ready. The inaccessible unused capability was revoked before this link was issued.' : 'The controlled recovery was already completed; its raw secure link cannot be displayed again.');
    } catch (error) { setStatus(error instanceof Error ? error.message : 'The secure-link recovery could not be completed.'); }
    finally { setBusy(false); }
  }

  return <main className="min-h-screen px-5 py-6 text-[var(--atlas-text-primary)] sm:px-8 sm:py-8 lg:px-12" data-testid="client-authorization-workspace" data-agent-only="true" data-client-facing="false">
    <div className="mx-auto max-w-6xl">
      <header className="border-b border-[color:color-mix(in_srgb,var(--atlas-profile-border-subtle)_34%,transparent)] pb-6"><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[var(--atlas-information-system-status)]">Project Atlas / Agent Workspace</p><h1 className="mt-2 text-2xl font-semibold text-[var(--atlas-text-primary)] sm:text-3xl">Client Authorization</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--atlas-text-secondary)]">Profile-governed authorization history. This Foundation V1 surface is synthetic certification only; it does not collect client authorization, release documents, or execute an external action.</p></header>
      ...
      <section className="mt-6 space-y-4" aria-label="Client authorization history">
        ...
        {authorizations.map((authorization) => { const terms = scope(authorization); const currentCapability = activeCapability(authorization); const isHistorical = ['SUPERSEDED', 'REVOKED', 'DECLINED', 'CONSUMED'].includes(authorization.status); const isCurrent = !isHistorical && !authorization.supersededByAuthorization; return <article key={authorization.id} className={`atlas-authorization-record ${isHistorical ? 'atlas-authorization-record-historical' : ''} ${isCurrent ? 'atlas-authorization-record-current' : ''}`} data-testid="client-authorization-record" data-authorization-status={authorization.status} data-authorization-history={isHistorical ? 'true' : 'false'}>
          ...
          {authorization.status === 'PENDING_CONFIRMATION' && currentCapability && !oneTimeLink && <aside className="atlas-issued-capability" aria-label="Issued secure link status"><KeyRound size={17} aria-hidden="true" /><div><p className="font-semibold text-cyan-50">Secure link issued and active</p><p className="mt-1 text-sm leading-6 text-slate-300">One use remains. The raw bearer link was shown only at issuance and cannot be safely redisplayed after leaving that view. Expires {dateTime(currentCapability.expiresAt)}.</p><button type="button" disabled={busy} onClick={() => recover(authorization)} className="atlas-action atlas-action-secondary mt-3" data-testid="client-authorization-recover-capability"><KeyRound size={15} aria-hidden="true" />Recover lost secure link</button><p className="mt-2 text-xs leading-5 text-slate-400">Use only under controlled authorization. Recovery revokes this unused capability before issuing one replacement against the same frozen request.</p></div></aside>}
