...
export default function ClientAuthorizationWorkspace() {
  ...
  const recordUse = (authorization: Authorization) => action('RECORD_SYNTHETIC_USE', { input: { profileKey: authorization.profile.profileKey, profileVersion: authorization.profile.profileVersion, principalRefs: authorization.principals.map((principal) => principal.principalRef), actionClass: 'SYNTHETIC_INFORMATION_DISCLOSURE', purpose: 'CERTIFY_CLIENT_AUTHORIZATION_GOVERNANCE', recipientClass: 'SYNTHETIC_CERTIFICATION_RECIPIENT', recipientRef: scope(authorization).recipientRef, requestedDataClasses: ['SYNTHETIC_NON_SENSITIVE_DATA'] }, downstreamReference: `ATLAS_SYNTHETIC_USE_${authorization.id}`, clientMutationKey: `ATLAS_SYNTHETIC_USE_${authorization.id}` });
