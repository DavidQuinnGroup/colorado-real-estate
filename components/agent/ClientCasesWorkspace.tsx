...
export default function ClientCasesWorkspace({ clientCaseId }: { clientCaseId?: string }) {
  ...
  if (clientCaseId) return <main className="px-5 py-8 sm:px-8 lg:px-12" data-testid="client-case-detail"><div className="mx-auto max-w-6xl">
    ...
    {!clientCase ? <p className="mt-8 text-slate-300">Loading Client Case...</p> : <>
      ...
      <section className="mt-6 flex flex-wrap gap-3"><Link href="/agent" className="atlas-action atlas-action-secondary"><Home size={15} />Workspace Home</Link>{clientCase.status === 'ACTIVE' ? <button disabled={busy} onClick={() => void action('ARCHIVE', { clientCaseId: clientCase.id })} className="atlas-action atlas-action-destructive"><Archive size={15} />Archive Case</button> : <button disabled={busy} onClick={() => void action('REACTIVATE', { clientCaseId: clientCase.id })} className="atlas-action atlas-action-primary"><RotateCcw size={15} />Reactivate Case</button>}</section>
    </>}
  </div></main>;
