...
export default function ClientCasesWorkspace({ clientCaseId }: { clientCaseId?: string }) {
  ...
  if (clientCaseId) return <main className={styles.page} data-testid="client-case-detail"><div className={styles.container}>
    ...
    {!clientCase ? <p className="mt-8 text-slate-300">Loading Client Case...</p> : <>
      <header className={styles.header}><p className={styles.eyebrow}>Project Atlas / Client Work</p><h1 className={styles.title}>{clientCase.displayName}</h1><p className={styles.statusLine}>Status: {clientCase.status}</p><div className={styles.actionRow}>{['Buyer','Seller','Financial Strategy','Intelligence','Transactions','Outputs'].map((label, index) => <Link key={label} href={[ '/agent/prepare/buyer','/agent/prepare/seller','/agent/strategy','/agent/prepare/market','/agent/transactions','/agent/outputs'][index] + `?clientCaseId=${encodeURIComponent(clientCase.id)}`} className="atlas-action atlas-action-secondary">{label}</Link>)}<Link href={`/agent/clients/${encodeURIComponent(clientCase.id)}/readiness`} className="atlas-action atlas-action-secondary">Readiness</Link></div><p className={styles.copy}>Client Case context is Agent-private and keeps advisory relationships distinct from transactions, authorizations, outputs, and professional evidence.</p></header>
