...
export default function ClientCasesWorkspace({ clientCaseId }: { clientCaseId?: string }) {
  ...
  if (clientCaseId) return <main className="px-5 py-8 sm:px-8 lg:px-12" data-testid="client-case-detail"><div className="mx-auto max-w-6xl">
    ...
    {!clientCase ? <p className="mt-8 text-slate-300">Loading Client Case...</p> : <>
      <header className="mt-6 border-b border-white/10 pb-6"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-cyan-100/70">Project Atlas / Client Work</p><h1 className="mt-2 text-2xl font-semibold text-white sm:text-3xl">{clientCase.displayName}</h1><p className="mt-3 text-sm text-cyan-100">Status: {clientCase.status}</p><div className="mt-4 flex flex-wrap gap-2">{['Buyer','Seller','Financial Strategy','Intelligence','Transactions','Outputs'].map((label, index) => <Link key={label} href={[ '/agent/prepare/buyer','/agent/prepare/seller','/agent/strategy','/agent/prepare/market','/agent/transactions','/agent/outputs'][index] + `?clientCaseId=${encodeURIComponent(clientCase.id)}`} className="atlas-action atlas-action-secondary">{label}</Link>)}</div><p className="mt-4 text-sm text-slate-300">Client Case context is Agent-private and keeps advisory relationships distinct from transactions, authorizations, outputs, and professional evidence.</p></header>
