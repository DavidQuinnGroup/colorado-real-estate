...
export default function ClientCasesWorkspace({ clientCaseId }: { clientCaseId?: string }) {
  ...
  if (clientCaseId) return <main className="px-5 py-8 sm:px-8 lg:px-12" data-testid="client-case-detail"><div className="mx-auto max-w-6xl">
    ...
    {!clientCase ? <p className="mt-8 text-slate-300">Loading Client Case...</p> : <>
      <header className="mt-6 border-b border-white/10 pb-6"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-cyan-100/70">Project Atlas / Client Work</p><h1 className="mt-2 text-2xl font-semibold text-white sm:text-3xl">{clientCase.displayName}</h1><p className="mt-3 text-sm text-cyan-100">Status: {clientCase.status}</p><div className="mt-4 flex flex-wrap gap-2">{['Buyer','Seller','Financial Strategy','Intelligence','Transactions','Outputs'].map((label, index) => <Link key={label} href={[ '/agent/prepare/buyer','/agent/prepare/seller','/agent/strategy','/agent/prepare/market','/agent/transactions','/agent/outputs'][index] + `?clientCaseId=${encodeURIComponent(clientCase.id)}`} className="atlas-action atlas-action-secondary">{label}</Link>)}<Link href={`/agent/clients/${encodeURIComponent(clientCase.id)}/readiness`} className="atlas-action atlas-action-secondary">Readiness</Link></div><p className="mt-4 text-sm text-slate-300">Client Case context is Agent-private and keeps advisory relationships distinct from transactions, authorizations, outputs, and professional evidence.</p></header>
      ...
      <section className="mt-6 border border-white/10 p-5"><h2 className="text-lg font-semibold text-white">Linked work</h2>{clientCase.transactions?.length ? <ul className="mt-4 space-y-2 text-sm text-slate-300">{clientCase.transactions.map((transaction) => <li key={transaction.id}><Link href={`/agent/transactions/${transaction.id}?clientCaseId=${encodeURIComponent(clientCase.id)}`} className="font-medium text-cyan-100 hover:text-white">{transaction.label}</Link> <span className="text-slate-500">{transaction.side} / {transaction.status} / {transaction.stage}</span></li>)}</ul> : <p className="mt-3 text-sm text-slate-400">No linked Transactions yet. Output, Professional Input, and financial scenario relationships are deferred to their owning foundations.</p>}</section>
      <section className="mt-6 flex flex-wrap gap-3" aria-label="Case lifecycle actions">{clientCase.status === 'ACTIVE' ? <button disabled={busy} onClick={() => void action('ARCHIVE', { clientCaseId: clientCase.id })} className="atlas-action atlas-action-destructive"><Archive size={15} />Archive Case</button> : <button disabled={busy} onClick={() => void action('REACTIVATE', { clientCaseId: clientCase.id })} className="atlas-action atlas-action-primary"><RotateCcw size={15} />Reactivate Case</button>}</section>
    </>}
  </div></main>;
