...
import {
  inspectInternalPreviewAdapter,
  invokeInternalPreviewAdapter,
} from "@/lib/internal-preview/previewAdapter";
...
export async function GET(request: NextRequest) {
  ...
  try {
    return NextResponse.json({
      success: true,
      module: "internal-preview-adapter",
      schemaVersion: 1,
      access: "internal_admin",
      inspection: await inspectInternalPreviewAdapter(),
    });
  } catch (error) {
    return NextResponse.json(
      {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await invokeInternalPreviewAdapter({
      execute,
      invocationId,
    });
