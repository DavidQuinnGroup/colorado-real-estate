...
async function assertSelectedAlert() {
  ...
  assert(
    row.clickedAt === null || Number.isFinite(Date.parse(row.clickedAt)),
    'Expected selected row clickedAt to be null before proof or a valid timestamp after CLICK-001 closure.',
  );
