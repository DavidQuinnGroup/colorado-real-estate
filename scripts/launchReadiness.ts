...
type ReadinessLevel = 'ready' | 'watch' | 'blocked';
...
function checkResendApiKey(scope: string): CheckResult {
...
function checkSender({ warnOnFallback }: { warnOnFallback: boolean }): CheckResult {
...
function checkReplyTo(scope: string, missingDetail?: string): CheckResult {
...
function checkPublicUrl(scope: string): CheckResult {
...
function checkPropertyInquiryRecipient(): CheckResult {
...
function checkPropertyInquiryDryRunDisabled(): CheckResult {
...
function getGateLevel(checks: CheckResult[]): ReadinessLevel {
  if (checks.some((check) => check.status === 'fail')) return 'blocked';
  if (checks.some((check) => check.status === 'warn')) return 'watch';
  return 'ready';
}

function getGateSummary(name: string, level: ReadinessLevel) {
  if (level === 'blocked') return `${name} has required blockers.`;
  if (level === 'watch') return `${name} is usable with operator review.`;
  return `${name} is ready.`;
}

function buildGate(name: string, checks: CheckResult[]): Gate {
...
function getBlockedBy(gates: Gate[]): ReadinessBlocker[] {
...
function getOverallReadiness(gates: Gate[]) {
