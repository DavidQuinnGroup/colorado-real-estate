...
type DigestUser = {
  id: string;
  email: string;
  alertQueue: DigestAlert[];
};

type ValidDigestAlert = DigestAlert & {
...
type DigestResult = {
  userId: string;
  email: string;
  status: 'sent' | 'skipped' | 'preview' | 'failed';
  listings: number;
  alerts: number;
  reason: string;
};
...
function readDefaultLimit() {
  if (!process.env.DIGEST_RUN_LIMIT) return DEFAULT_LIMIT;
  return parseBoundedInteger(process.env.DIGEST_RUN_LIMIT, 'DIGEST_RUN_LIMIT', 1, MAX_LIMIT);
}

function parseArgs(argv: string[]): DigestOptions | null {
...
function isDigestListing(value: unknown): value is DigestListing {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false;

  const listing = value as DigestListing;
...
function normalizeDigestListing(listing: DigestListing): DigestListing {
  return {
    ...listing,
...
function partitionAlerts(alerts: DigestAlert[]) {
  const valid: ValidDigestAlert[] = [];
  const invalid: DigestAlert[] = [];
...
async function markAlerts(alertIds: string[], status: string) {
...
async function markInvalidAlertsFailed(alerts: DigestAlert[], dryRun: boolean) {
...
async function claimPendingAlerts(alerts: ValidDigestAlert[]) {
  const claimed: ValidDigestAlert[] = [];
...
function buildResult(user: DigestUser, status: DigestResult['status'], listings: number, alerts: number, reason: string): DigestResult {
  return {
    userId: user.id,
    email: user.email,
    status,
...
async function sendUserDigest(user: DigestUser, dryRun: boolean): Promise<DigestResult> {
  if (!user.email.trim()) {
...
function printResult(results: DigestResult[], options: DigestOptions, scanned: number) {
