...
import {
  ATTENTION_ITEM_RULES,
  EXECUTIVE_WORKSPACE_CALCULATION_VERSION,
  MATERIAL_CHANGE_RANKING_RULES,
...
function assertCommandCenterComposition() {
  ...
  assert.equal(
    new Set(payload.attentionItems.map((item) => item.stableId)).size,
    payload.attentionItems.length,
  );
...
console.log(
  "[enterprise-executive-workspace-safety] ok: command center, daily brief, ranking, attention, evidence, provenance, auth, UI, and mutation boundaries passed.",
);
