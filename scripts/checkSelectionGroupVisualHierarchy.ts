...
function main() {
  ...
  assert.match(titleHierarchy, /selectionGroup: 'text-base font-semibold leading-6 text-white'/);
  assert.match(globalStyles, /\[data-agent-only="true"\] fieldset > legend:not\(\.sr-only\)/);
  for (const declaration of ['margin-bottom: 0.75rem', 'font-size: 1rem', 'font-weight: 600', 'line-height: 1.5']) {
  ...
  for (const [name, experience, heading] of [
    ...
    assert.match(experience, new RegExp(`text-lg font-semibold text-white">${heading}<`), `${name} must retain its stronger selection heading.`);
  }
