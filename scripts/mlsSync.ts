...
const MLS_SYNC_MAX_RATE_DELAY_MS = 60_000;
const MLS_SYNC_MAX_PAGE_SIZE = 100;
const MLS_SYNC_MAX_PAGES = 100;
const MLS_SYNC_MAX_START_PAGE = 1_000_000;
const MLS_SYNC_DEFAULT_PAGE_TIMEOUT_MS = 30_000;
const MLS_SYNC_MAX_PAGE_TIMEOUT_MS = 120_000;
...
function parseNumber(value: string | undefined, name: string, min: number, max: number) {
...
function buildSyncOptions(options: CliOptions): SyncMLSGridOptions {
  return {
...
function buildOutput(
  options: CliOptions,
  syncOptions: SyncMLSGridOptions,
  notes: string[] = [],
  diagnostics: string[] = [],
  summary?: unknown,
): RunnerOutput {
...
function writeOutput(options: CliOptions, output: RunnerOutput) {
  if (options.json) {
