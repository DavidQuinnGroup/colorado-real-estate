...
type SearchSchemaModule = typeof import('../lib/typesense/schema.js');
...
function parseArgs(argv: string[]): ReindexOptions | null {
...
function getSchemaSummary(schema: CollectionCreateSchema) {
  ...
  return `${schema.name}: ${fields.length} fields, ${facetCount} facets, ${sortableCount} sortable fields, default sort ${
    schema.default_sorting_field || 'none'
  }`;
}

function getCanonicalRuleSummary(
  SEARCH_SCHEMA_FIELD_RULES: SearchSchemaModule['SEARCH_SCHEMA_FIELD_RULES'],
  SEARCH_SCHEMA_QUERY_FIELD_NAMES: SearchSchemaModule['SEARCH_SCHEMA_QUERY_FIELD_NAMES'],
  SEARCH_SCHEMA_FILTER_FIELD_NAMES: SearchSchemaModule['SEARCH_SCHEMA_FILTER_FIELD_NAMES'],
  SEARCH_SCHEMA_SORT_FIELD_NAMES: SearchSchemaModule['SEARCH_SCHEMA_SORT_FIELD_NAMES'],
) {
...
function getInspectionFailureDetail(inspection: CollectionInspection) {
...
function assertValidSchema(
  ...
  formatSearchSchemaValidationError: SearchSchemaModule['formatSearchSchemaValidationError'],
) {
...
function assertValidRetrievedCollection(
  collection: RetrievedTypesenseCollection,
  formatSearchSchemaValidationError: SearchSchemaModule['formatSearchSchemaValidationError'],
) {
...
async function inspectCollection(
  typesense: SearchSchemaModule['typesense'],
  schema: CollectionCreateSchema,
  formatSearchSchemaValidationError: SearchSchemaModule['formatSearchSchemaValidationError'],
...
function logInspection(inspection: CollectionInspection) {
  if (inspection.status === 'missing') {
...
async function deleteCollection(typesense: SearchSchemaModule['typesense'], collectionName: string) {
  try {
...
function getSchemaByName(searchSchemas: CollectionCreateSchema[], collectionName: string) {
  return searchSchemas.find((schema) => schema.name === collectionName);
}

async function validateCanonicalSchemas(schemaModule: SearchSchemaModule) {
  const {
...
async function inspectSearchCollections(schemaModule: SearchSchemaModule) {
...
async function ensureSearchCollections(schemaModule: SearchSchemaModule, reset: boolean) {
  const { formatSearchSchemaValidationError, searchSchemas, typesense } = schemaModule;
  ...
  const inspections = reset
    ? searchSchemas.map((schema) => ({ schemaName: schema.name, status: 'missing' as const }))
...
async function runCheck(schemaModule: SearchSchemaModule) {
...
async function main() {
  ...
  console.log(
    `Typesense reindex starting: check=${options.check}, reset=${options.reset}, collectionsOnly=${
      options.collectionsOnly
    }, batchSize=${options.batchSize ?? DEFAULT_BATCH_SIZE}, maxRecords=${options.maxRecords ?? 'all'}.`,
  );
