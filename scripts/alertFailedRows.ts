...
type AlertFailedRowsOptions = {
  execute: boolean;
  limit: number;
  markTestSkipped: boolean;
};
...
function getBoundedInteger(value: string, fallback: number, min: number, max: number) {
  const parsed = Number(value);
  if (!Number.isFinite(parsed)) return fallback;
  return Math.max(min, Math.min(Math.floor(parsed), max));
}

function parseOptions(argv: string[]): AlertFailedRowsOptions {
  const options: AlertFailedRowsOptions = {
    execute: false,
    limit: DEFAULT_LIMIT,
    ...
    JSON.stringify(
      {
        ...
        rows: inspectedRows,
        nextCommand:
          eligibleRows.length > 0 && !shouldUpdate
            ? 'npm run run:alerts:failed:skip-test'
