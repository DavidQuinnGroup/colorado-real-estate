...
import {
  EIA_MODEL_OWNERSHIP,
  assertExplicitClassification,
  assertNoMixedAggregation,
  buildKpiObservationIdempotencyKey,
} from "../lib/enterprise-kpi/persistence.js";

const schema = fs.readFileSync("prisma/schema.prisma", "utf8");
const migration = fs.readFileSync(
  "prisma/migrations/20260718164000_eia_wave1_persistence_foundation/migration.sql",
  "utf8",
);
...
assert.equal(
  key,
  "EIA-KPI-OBS|TEST|FIXTURE|EIF-SPRINT-5|LEARNING_SYSTEM|KPI-PLAT-001|NO_PERIOD_START|NO_PERIOD_END|NO_OBSERVED_AT|NO_SOURCE_STATE|EIA-1.0-test",
);
...
assert.ok(schema.includes("dataOrigin           EIADataOrigin"));
assert.ok(schema.includes("environment          EIAEnvironment"));
...
console.log(
  "[enterprise-intelligence-persistence-safety] ok: models, provenance, fixture/live separation, idempotency, indexes, additive SQL, auth posture, public exposure, and activation guardrails passed.",
);
