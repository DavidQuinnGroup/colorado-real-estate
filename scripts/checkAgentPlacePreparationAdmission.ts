...
assert.ok(middleware.includes("pathname === '/agent' || pathname.startsWith('/agent/')") && middleware.includes('buildAgentLoginRedirect'), 'The exact Agent place route must use the centralized Agent login redirect.');
assert.ok(intelligenceLanding.includes("href: '/agent/prepare/place'") && intelligenceLanding.includes('Location Intelligence'), 'The authorized Location Preparation capability must be visible through Intelligence.');
