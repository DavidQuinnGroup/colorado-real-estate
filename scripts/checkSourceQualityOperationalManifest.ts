...
const PRIOR_FIVE_ENTRY_FINGERPRINTS = new Map([
...
assert.equal(valid.manifest.entries.length, 6);
...
for (const [sourceId, entryFingerprint] of PRIOR_FIVE_ENTRY_FINGERPRINTS) {
...
assert.equal(withoutMls.manifest?.entries.length, 5);
...
assert.equal(withoutAssessor.manifest?.entries.length, 5);
...
assert.equal(withoutMunicipal.manifest?.entries.length, 5);
...
assert.equal(withoutTreasurer.manifest?.entries.length, 5);
...
assert.equal(assembly.assembly.sourceCount, 6);
...
if (report.classification === 'FAIL_CLOSED') throw new Error('Report must accept six-source operational manifest output.');
assert.equal(report.report.sourceCount, 6);
