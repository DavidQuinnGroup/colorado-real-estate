...
assert.equal(valid.manifest.entries.length, 13);
...
assert.equal(withoutMls.manifest?.entries.length, 12);
...
assert.equal(withoutAssessor.manifest?.entries.length, 12);
...
assert.equal(withoutMunicipal.manifest?.entries.length, 12);
...
assert.equal(withoutTreasurer.manifest?.entries.length, 12);
...
assert.equal(withoutAccela.manifest?.entries.length, 12);
...
assert.equal(withoutCityOpenData.manifest?.entries.length, 12);
...
assert.equal(withoutCityPortal.manifest?.entries.length, 12);
...
assert.equal(withoutRecorder.manifest?.entries.length, 12);
...
assert.equal(withoutAddressPoints.manifest?.entries.length, 12);
...
assert.equal(withoutParkBoundaries.manifest?.entries.length, 12);
...
assert.equal(withoutParcelGis.manifest?.entries.length, 12);
...
assert.equal(assembly.assembly.sourceCount, 13);
...
if (report.classification === 'FAIL_CLOSED') throw new Error('Report must accept thirteen-source operational manifest output.');
assert.equal(report.report.sourceCount, 13);
