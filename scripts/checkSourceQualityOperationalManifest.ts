...
assert.equal(valid.manifest.entries.filter((entry) => entry.sourceId === BROOMFIELD_COUNTY_TREASURER_SOURCE_ID).length, 1);
assert.equal(valid.manifest.entries.filter((entry) => entry.sourceId === DOUGLAS_COUNTY_TREASURER_SOURCE_ID).length, 1);
...
assert.deepEqual(registryOnlySources.map((source) => source.sourceId), ['SRC-BOULDER-PERMIT-CANDIDATES'], 'After Arapahoe Parcel GIS Manifest inclusion, registry-only sources must be limited to Permit Candidate.');
assert.equal(registryOnlySources.filter((source) => source.sourceId === DOUGLAS_COUNTY_TREASURER_SOURCE_ID).length, 0, 'Douglas Treasurer must be included after Manifest inclusion authorization.');
...
assert.equal(registryOnlySources.filter((source) => source.sourceId === BROOMFIELD_COUNTY_TREASURER_SOURCE_ID).length, 0, 'Broomfield Treasurer must be included after Manifest inclusion authorization.');
...
const weldTreasurerEvidence = convertWeldCountyTreasurerSourceQualityEvidence();
const rawWeldTreasurerEntry = SOURCE_QUALITY_OPERATIONAL_MANIFEST_DATA.entries.find((entry) => entry.sourceId === WELD_COUNTY_TREASURER_SOURCE_ID);
assert.ok(rawWeldTreasurerEntry);
assert.equal(rawWeldTreasurerEntry?.sourceId, WELD_COUNTY_TREASURER_SOURCE_ID);
assert.equal(rawWeldTreasurerEntry?.inclusionClass, 'STRUCTURED_EVIDENCE_WITH_KNOWN_GAPS');
assert.deepEqual(rawWeldTreasurerEntry?.linkages, weldTreasurerEvidence.linkages);
assert.deepEqual(rawWeldTreasurerEntry?.linkages, convertWeldCountyTreasurerSourceQualityEvidence().linkages);
assert.deepEqual(rawWeldTreasurerEntry?.expectedEvidenceClasses, ['CERTIFICATION']);
assert.strictEqual(rawWeldTreasurerEntry?.certificationReference, WELD_COUNTY_TREASURER_SOURCE_QUALITY_CERTIFICATION);
assert.equal(rawWeldTreasurerEntry?.reviewedAt, WELD_COUNTY_TREASURER_SOURCE_QUALITY_EVIDENCE_REVIEWED_AT);
assert.equal(rawWeldTreasurerEntry?.reviewAuthorityClass, 'DELEGATED_SOURCE_GOVERNANCE_REVIEW');
assert.deepEqual(rawWeldTreasurerEntry?.limitationCodes, []);
assert.equal(normalizeWeldCountyTreasurerSourceQualityEvidence().result, 'INSUFFICIENT_EVIDENCE');
...
assert.equal(WELD_COUNTY_TREASURER_SOURCE_QUALITY_FIREWALL.customerDisplayAuthority, 'CUSTOMER_DISPLAY_NOT_GRANTED_BY_EVIDENCE_PACKAGE');
assert.equal(WELD_COUNTY_TREASURER_SOURCE_QUALITY_FIREWALL.legalUse, 'LEGAL_USE_NOT_APPROVED_BY_EVIDENCE_PACKAGE');
assert.equal(WELD_COUNTY_TREASURER_SOURCE_QUALITY_FIREWALL.publicSourceFallacy, 'PUBLIC_OR_GOVERNMENT_SOURCE_NOT_UNRESTRICTED_OR_VERIFIED_OR_COMPLETE');
...
const weldTreasurerManifestEntry = valid.manifest.entries.find((entry) => entry.sourceId === WELD_COUNTY_TREASURER_SOURCE_ID);
assert.ok(weldTreasurerManifestEntry);
assert.equal(weldTreasurerManifestEntry?.entryFingerprint, validateSourceQualityOperationalManifest(SOURCE_QUALITY_OPERATIONAL_MANIFEST_DATA).manifest?.entries.find((entry) => entry.sourceId === WELD_COUNTY_TREASURER_SOURCE_ID)?.entryFingerprint);
...
const broomfieldTreasurerEvidence = convertBroomfieldCountyTreasurerSourceQualityEvidence();
const rawBroomfieldTreasurerEntry = SOURCE_QUALITY_OPERATIONAL_MANIFEST_DATA.entries.find((entry) => entry.sourceId === BROOMFIELD_COUNTY_TREASURER_SOURCE_ID);
assert.ok(rawBroomfieldTreasurerEntry);
assert.equal(rawBroomfieldTreasurerEntry?.sourceId, BROOMFIELD_COUNTY_TREASURER_SOURCE_ID);
assert.equal(rawBroomfieldTreasurerEntry?.inclusionClass, 'STRUCTURED_EVIDENCE_WITH_KNOWN_GAPS');
assert.deepEqual(rawBroomfieldTreasurerEntry?.linkages, broomfieldTreasurerEvidence.linkages);
assert.deepEqual(rawBroomfieldTreasurerEntry?.linkages, convertBroomfieldCountyTreasurerSourceQualityEvidence().linkages);
assert.deepEqual(rawBroomfieldTreasurerEntry?.expectedEvidenceClasses, ['CERTIFICATION']);
assert.strictEqual(rawBroomfieldTreasurerEntry?.certificationReference, BROOMFIELD_COUNTY_TREASURER_SOURCE_QUALITY_CERTIFICATION);
assert.equal(rawBroomfieldTreasurerEntry?.reviewedAt, BROOMFIELD_COUNTY_TREASURER_SOURCE_QUALITY_EVIDENCE_REVIEWED_AT);
assert.equal(rawBroomfieldTreasurerEntry?.reviewAuthorityClass, 'DELEGATED_SOURCE_GOVERNANCE_REVIEW');
assert.deepEqual(rawBroomfieldTreasurerEntry?.limitationCodes, []);
assert.equal(normalizeBroomfieldCountyTreasurerSourceQualityEvidence().result, 'INSUFFICIENT_EVIDENCE');
...
const broomfieldTreasurerManifestEntry = valid.manifest.entries.find((entry) => entry.sourceId === BROOMFIELD_COUNTY_TREASURER_SOURCE_ID);
assert.ok(broomfieldTreasurerManifestEntry);
assert.equal(broomfieldTreasurerManifestEntry?.entryFingerprint, validateSourceQualityOperationalManifest(SOURCE_QUALITY_OPERATIONAL_MANIFEST_DATA).manifest?.entries.find((entry) => entry.sourceId === BROOMFIELD_COUNTY_TREASURER_SOURCE_ID)?.entryFingerprint);
assert.notEqual(broomfieldTreasurerManifestEntry?.entryFingerprint, valid.manifest.entries.find((entry) => entry.sourceId === ARAPAHOE_COUNTY_TREASURER_SOURCE_ID)?.entryFingerprint);
assert.notEqual(broomfieldTreasurerManifestEntry?.entryFingerprint, valid.manifest.entries.find((entry) => entry.sourceId === ADAMS_COUNTY_TREASURER_SOURCE_ID)?.entryFingerprint);
assert.notEqual(broomfieldTreasurerManifestEntry?.entryFingerprint, jeffersonTreasurerManifestEntry?.entryFingerprint);
assert.notEqual(broomfieldTreasurerManifestEntry?.entryFingerprint, weldTreasurerManifestEntry?.entryFingerprint);
assert.notEqual(broomfieldTreasurerManifestEntry?.entryFingerprint, larimerTreasurerManifestEntry?.entryFingerprint);
...
const arapahoeParcelGisEntry = valid.manifest.entries.find((entry) => entry.sourceId === ARAPAHOE_COUNTY_PARCEL_GIS_SOURCE_ID);
assert.ok(arapahoeParcelGisEntry);
assert.equal(arapahoeParcelGisEntry?.inclusionClass, 'STRUCTURED_EVIDENCE_WITH_KNOWN_GAPS');
assert.deepEqual(arapahoeParcelGisEntry?.linkages, convertArapahoeCountyParcelGisSourceQualityEvidence().linkages);
assert.strictEqual(arapahoeParcelGisEntry?.certificationReference, ARAPAHOE_COUNTY_PARCEL_GIS_SOURCE_QUALITY_CERTIFICATION);
assert.equal(arapahoeParcelGisEntry?.reviewedAt, ARAPAHOE_COUNTY_PARCEL_GIS_SOURCE_QUALITY_EVIDENCE_REVIEWED_AT);
...
assert.equal(withoutMls.manifest?.entries.length, 26);
...
assert.equal(withoutAssessor.manifest?.entries.length, 26);
...
assert.equal(withoutMunicipal.manifest?.entries.length, 26);
...
assert.equal(withoutTreasurer.manifest?.entries.length, 26);
...
assert.equal(withoutAccela.manifest?.entries.length, 26);
...
assert.equal(withoutCityOpenData.manifest?.entries.length, 26);
...
assert.equal(withoutCityPortal.manifest?.entries.length, 26);
...
assert.equal(withoutRecorder.manifest?.entries.length, 26);
...
assert.equal(withoutAddressPoints.manifest?.entries.length, 26);
...
assert.equal(withoutParkBoundaries.manifest?.entries.length, 26);
...
assert.equal(withoutParcelGis.manifest?.entries.length, 26);
...
assert.equal(withoutAdamsAssessor.manifest?.entries.length, 26);
...
assert.equal(withoutArapahoeAssessor.manifest?.entries.length, 26);
...
assert.equal(withoutBroomfieldAssessor.manifest?.entries.length, 26);
...
assert.equal(withoutJeffersonAssessor.manifest?.entries.length, 26);
...
assert.equal(withoutLarimerAssessor.manifest?.entries.length, 26);
...
assert.equal(withoutWeldAssessor.manifest?.entries.length, 26);
...
assert.equal(assembly.assembly.sourceCount, 27);
...
assert.equal(assembly.assembly.summaries.find((summary) => summary.source.sourceId === ARAPAHOE_COUNTY_PARCEL_GIS_SOURCE_ID)?.classification, 'INSUFFICIENT_EVIDENCE');
...
assert.equal(assembly.assembly.summaries.find((summary) => summary.source.sourceId === BROOMFIELD_COUNTY_TREASURER_SOURCE_ID)?.classification, 'INSUFFICIENT_EVIDENCE');
assert.equal(assembly.assembly.summaries.find((summary) => summary.source.sourceId === DOUGLAS_COUNTY_TREASURER_SOURCE_ID)?.classification, 'INSUFFICIENT_EVIDENCE');
const report = composeSourceQualityReport(assembly.assembly.summaries);
assert.notEqual(report.classification, 'FAIL_CLOSED');
if (report.classification === 'FAIL_CLOSED') throw new Error('Report must accept twenty-seven-source operational manifest output.');
assert.equal(report.report.sourceCount, 27);
...
assert.ok(report.report.insufficientEvidenceSources.includes(BROOMFIELD_COUNTY_TREASURER_SOURCE_ID));
assert.ok(report.report.insufficientEvidenceSources.includes(DOUGLAS_COUNTY_TREASURER_SOURCE_ID));
...
assert.ok(report.report.insufficientEvidenceSources.includes(ARAPAHOE_COUNTY_PARCEL_GIS_SOURCE_ID));
...
assert.ok(data.includes('ARAPAHOE_COUNTY_PARCEL_GIS_SOURCE_ID'));
assert.equal(data.includes("'SRC-ARAPAHOE-COUNTY-PARCEL-GIS'"), false);
