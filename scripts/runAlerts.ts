...
type AlertRunResult = Awaited<ReturnType<typeof processAlertQueue>>;
...
function readDefaultLimit() {
  if (!process.env.ALERT_RUN_LIMIT) return DEFAULT_LIMIT;
...
function parseArgs(argv: string[]): RunAlertsOptions | null {
  const options: RunAlertsOptions = {
...
function getRecommendation(result: AlertRunResult) {
  if (result.failed > 0) return 'Review failed alerts before running a larger or live batch.';
  if (result.dryRun && result.scanned > 0) return 'Dry-run preview completed. Review rows before rerunning with --live.';
...
function buildExecutionPlan(options: RunAlertsOptions, result: AlertRunResult) {
...
function printResult(options: RunAlertsOptions, result: AlertRunResult) {
  const commands = buildCommands(options.limit);
