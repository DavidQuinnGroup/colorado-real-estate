...
import {
  DEFAULT_MARKETABLE_STATUS,
  getDefaultStatusFilter,
  getPrimarySearchPhoto,
  getSearchQualitySummary,
  hasExplicitStatusFilter,
  isDefaultMarketableStatus,
  normalizeSearchPhotos,
  sortListingsForLaunchQuality,
} from '../lib/search/listingQuality.js';

type FixtureListing = {
  id: string;
  status: string;
  price: number;
  photos?: Array<{ id?: string; url?: string | null; order?: number }>;
...
function defaultPublicResults(fixtures: FixtureListing[]) {
...
function assertFixtureContracts() {
  ...
  assert.deepEqual(
    normalizedPhotos.map((photo) => photo.url),
    ['https://cdn.example.com/listing-a.jpg'],
    'Expected photo normalization to remove empty, duplicate, and malformed URLs.',
  );
...
async function main() {
  ...
  console.log(
    '[search-listing-quality] ok: default Active contract, explicit Sold support, provider-equivalent filters, photo fallback state, fallback metadata, and public diagnostic copy verified with stable fixtures.',
  );
}
