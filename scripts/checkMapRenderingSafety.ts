...
async function main() {
  ...
  assert(searchMap.includes('data-marker-state="${state || \'default\'}"'), 'Search map markers must expose selected/hovered/default state metadata.');
  assert(searchMap.includes('aria-pressed="${isSelected ? \'true\' : \'false\'}"'), 'Search map markers must expose selected state accessibly.');
  assert(searchMap.includes("setSelectedId(property.id);"), 'Search map marker clicks must select the property.');
  assert(searchMap.includes('setHoveredId(null);'), 'Search map marker clicks must clear hover state after selection.');
  assert(searchMap.includes("marker.on('mouseover'"), 'Search map may preserve supplemental hover synchronization.');
  assert(!searchMap.includes('marker.bindPopup(buildPopupHtml(property)'), 'Search map must not bind the old hover-dependent actionable property popup.');
  assert(!searchMap.includes('marker.openPopup()'), 'Search map marker preview action must not depend on Leaflet popup opening.');
