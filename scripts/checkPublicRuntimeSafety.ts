...
for (const filePath of adminCredentialReaders) {
  ...
  assert.equal(
    /searchParams\.get\(["']adminKey["']\)/.test(source),
    false,
    `${filePath} must not accept admin credentials from query parameters.`,
  );
}
...
console.log(
  "[public-runtime-safety] ok: public DDL removed, missing-schema failures are safe, query admin credentials are rejected, and admin namespaces are protected.",
);
