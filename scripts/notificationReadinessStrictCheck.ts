...
function getResult(payload: JsonRecord, name: string) {
  const results = Array.isArray(payload.results) ? payload.results : [];
  return results.find((result) => isRecord(result) && result.name === name);
}

function getGate(payload: JsonRecord, name: string) {
  const gates = Array.isArray(payload.gates) ? payload.gates : [];
  return gates.find((gate) => isRecord(gate) && gate.name === name);
}

function hasFailedCheck(result: unknown, name: string) {
  if (!isRecord(result) || !Array.isArray(result.failedChecks)) return false;
  return result.failedChecks.some((check) => isRecord(check) && check.name === name && check.status === 'fail');
}

function hasCheckDetail(result: unknown, name: string, status: string, detail: string) {
  if (!isRecord(result) || !Array.isArray(result.checks)) return false;

  return result.checks.some(
    (check) =>
      isRecord(check) && check.name === name && check.status === status && String(check.detail || '') === detail,
  );
}

function hasBlockedBy(result: unknown, code: string, envVar: string) {
  if (!isRecord(result) || !Array.isArray(result.blockedBy)) return false;
...
function runLaunchReadiness(env: NodeJS.ProcessEnv) {
  ...
  assert.equal(
    result.status,
    expectedExitCode,
    `Expected launch readiness exit ${expectedExitCode} when readiness is ${String(readinessLevel)}.`,
  );
...
function runStrictSummary(env: NodeJS.ProcessEnv) {
  ...
  assert.equal(
    result.status,
    expectedExitCode,
    `Expected strict summary exit ${expectedExitCode} when readiness is ${String(readinessLevel)}.`,
  );
  assert.equal(
    payload.success,
    readinessLevel !== 'blocked',
    'Expected strict summary success to fail closed only when readiness is blocked.',
  );
...
function main() {
  ...
  assert.ok(
    hasFailedCheck(dryRunEnvResult.propertyInquiryResult, 'PROPERTY_INQUIRY_NOTIFICATION_DRY_RUN'),
    'Expected dry-run strict property-inquiry child to expose dry-run failed check.',
  );
  assert.ok(
    hasBlockedBy(
      dryRunEnvResult.propertyInquiryResult,
      'property_inquiry_dry_run_enabled',
      'PROPERTY_INQUIRY_NOTIFICATION_DRY_RUN',
    ),
    'Expected dry-run strict property-inquiry child to expose structured dry-run blocker.',
  );
  assert.ok(
    hasFailedCheck(dryRunEnvResult.aggregateResult, 'PROPERTY_INQUIRY_NOTIFICATION_DRY_RUN'),
    'Expected dry-run strict aggregate child to expose dry-run failed check.',
  );
  assert.ok(
    hasBlockedBy(
  ...
  assert.ok(
    hasCheckDetail(
      noReplyToLaunchResult.propertyInquiryGate,
      'RESEND_REPLY_TO_EMAIL',
      'warn',
      'Not configured; property inquiry emails will reply directly to the lead email.',
    ),
    'Expected aggregate launch readiness to explain property-inquiry reply-to fallback behavior.',
  );
